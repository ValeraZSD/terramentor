...
stores everything in one SQLite file in a folder you can open from Settings, and talks to a
language model at an endpoint **you** configure. There is no vendor and no account, and
...
- **Only pages served from a name you could plausibly be reaching the app by may call `/api`**
  ...
  Tailscale tailnet), and, always, the very origin the request itself arrived on, which covers
  ...
  list, so a foreign page cannot use it to get in. A request carrying neither header is
...
- **Course and deck archives are bounded where they are decompressed.** Every ZIP-based input
  ...
  inflated through the same reader under a 300 MB total and discarded; the same bytes inflate
  to the same size in the parser, so a file that passes cannot make it allocate more.
...
- **What comes back is read as material, never as instruction.** A fetched page is the one
  thing in the conversation that neither you nor this app wrote, and the model picked it out
  of search results. It arrives labelled as reference material, with the model told that a
...
- **Nothing the model says writes to your library.** What the assistant proposes (a mastery
  check, a flashcard, a note) is a button under its answer, and only your press adds it.
