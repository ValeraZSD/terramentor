...
//
// The status endpoint IS the launcher's SINGLE-INSTANCE probe: before starting
// a server the launcher asks every candidate port whether a Terramentor on the
// same library is already there, and if so opens a window at it and exits.
// That is what makes a second double-click open a second window instead of a
// second server on a second port with an empty, bewildering library.
//
// Which is why it answers above the auth gate, and why it answers with as
// little as it can. On the deployment SECURITY.md recommends — a password set,
// reachable over a tailnet — anything in this answer is readable by every
// device on that tailnet with one unauthenticated GET, so it must not name the
// account or the file the library is in: the database is not encrypted at
// rest. The library is a HASH (`libraryId` in paths.js), which compares the
// same and says nothing, and everything the settings panel draws is served
// from `/api/desktop/info` on the protected router.
//
...
import { libraryId } from './paths.js';
...
/**
 * What the launcher recorded when it had to fall back to the default library
 * folder with no pointer to honour (desktop/launcher.js sets
 * `TERRAMENTOR_LIBRARY_FALLBACK` only when the launcher's own note says the
 * previous launch resolved somewhere else — the shape of a lost pointer).
 * Pure, and read once by index.js so the gate can inject the answer instead
 * of reaching into this process's environment.
 */
export function libraryFallbackFromEnv(env = process.env) {
    if (!env.TERRAMENTOR_LIBRARY_FALLBACK) return null;
    try {
        const v = JSON.parse(env.TERRAMENTOR_LIBRARY_FALLBACK);
        if (!v || typeof v.lastDir !== 'string' || !v.lastDir
            || typeof v.pointerPath !== 'string' || !v.pointerPath) return null;
        return { lastDir: v.lastDir, pointerPath: v.pointerPath };
    } catch { return null; }
}
 ...
 * `libraryFallback` is what the launcher recorded about a lost pointer, and
 * `libraryEmpty` the fact that joins it to a visible warning: an empty
 * library is also what a genuine first run looks like, so neither half alone
 * may speak.
 */
export function createDesktop({ dataDir, dbPath, version, port, getSetting, setSetting, onShutdown, libraryFallback = null, libraryEmpty = () => false, now = Date.now }) {
    const state = {
        startedAt: now(),
        lastPing: 0,
        lastBye: 0,
    ...
    /**
     * What the launcher's probe needs, and nothing else.
     *
     * This answer is PUBLIC — above the auth gate — because the two callers
     * have no session to offer: the launcher is not a browser, and the quit
     * screen polls while the app is shutting down, locked or not. So it is
     * built from the other end: each field is here because something decides
     * with it BEFORE there is a session, and everything else moved to
     * `/api/desktop/info` behind the gate.
     *
     *   libraryId        is this the library I am about to open? (a hash —
     *                    `dataDir` itself named the account and the path of an
     *                    unencrypted database to anything that could reach the
     *                    port, which over a tailnet is every device on it)
     *   windowMode
     *   autostartWindow  how a window opens, and whether one opens at all.
     *   trayHosted       Chromium takes the first two as launch flags and the
     *                    third decides whether a sign-in run may stay hidden;
     *                    all three are read before a window exists to ask.
     */
    const probeStatus = () => ({
        desktop: true,
        app: 'terramentor',
        libraryId: libraryId(dataDir),
        windowMode: windowMode(),
        autostartWindow: autostartWindow(),
        trayHosted,
    });

    /** Everything about this install, for the panel that shows it. Behind the
     *  auth gate: it names where the library is, which file the database is,
     *  and which process is serving it. */
    const status = () => ({
        ...probeStatus(),
        version,
        pid: process.pid,
        ...
        dataDir,
        dbPath,
        // Two separate facts, and `trayHosted` is the other one, up in the
        // probe: whether the app stays up without a window, and WHY. A panel
        // that draws a switch for something the tray icon already decides is
        ...
        // Only when both halves agree. `libraryFallback` alone would warn a
        // person whose pointer was lost months ago and who has since built a
        // library in the default folder on purpose; `libraryEmpty` alone
        // would warn every genuine first run. Together they name the one
        // situation that reads as lost work: this launch fell back to the
        // default folder, it is empty, and last time the library was
        // somewhere else.
        libraryFallback: libraryFallback && libraryEmpty()
            ? { ...libraryFallback, lastDirExists: existsSync(libraryFallback.lastDir) }
            : null,
    });
    ...
    const pub = Router();
    pub.get('/api/desktop/status', (req, res) => res.json(probeStatus()));
    // The heartbeat stays public too, and deliberately keeps no gate of its
    // own: the locked screen is a window, and a window that cannot say it is
    // there is one the shutdown rule counts as closed — so a locked app left
    // open would stop itself. It answers `{ok:true}` and nothing else, and it
    // cannot stop the app: `bye` only stamps `lastBye` once the last client is
    // gone, and a real window is registered under a client id nobody else has.
    // What someone on the tailnet CAN do is keep an idle instance alive, which
    // is a state the app offers as a setting (`desktop_keep_running`).
    ...
    // The whole picture, for the settings panel and the lost-library card.
    // Mounted below `requireAuth` in index.js, which is the difference between
    // this and the probe above.
    protectedRouter.get('/api/desktop/info', (req, res) => res.json(status()));
    ...
    protectedRouter.post('/api/desktop/restore-library', async (req, res) => {
        // Write the pointer back to where the launcher last saw the library.
        // Like open-data-dir, this takes no input from the request: the path
        // was recorded at launch, and a caller who could name an arbitrary
        // directory here would be writing files anywhere the server can.
        if (!libraryFallback) {
            return res.status(400).json({ error: 'This launch did not record a library to restore' });
        }
        if (!existsSync(libraryFallback.lastDir)) {
            return res.status(400).json({ error: 'That folder is not there right now' });
        }
        try {
            // Lazily, like the autostart plan above: the Docker image ships
            // `server`, `dist` and `public` and nothing else, and this module
            // is imported by index.js on every deployment.
            const { libraryPointerFile } = await import('../desktop/lib.js');
            writeFileSync(libraryFallback.pointerPath, libraryPointerFile(libraryFallback.lastDir), 'utf8');
        } catch (e) {
            return res.status(500).json({ error: e?.message || String(e) });
        }
        // Takes effect on the next launch: the launcher reads the pointer
        // before it starts the server, and this one already opened its
        // library. The card tells the reader to quit and start again.
        res.json({ ok: true, lastDir: libraryFallback.lastDir, pointerPath: libraryFallback.pointerPath });
    });
