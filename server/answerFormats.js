 ...
 *
 * A question's `type` used to be a closed set of three (multiple choice,
 * true/false, short answer), each one normalised by hand in two reducers
 * (`normalizeFeedQuestion`, `finalizeQuiz`), described by hand in three prompts,
 * rendered by hand in three surfaces and graded by hand in two — so a fourth
 * format meant touching ten places, and the app measured every subject with
 * "pick one of four". A mastery engine for cooking, code, chemistry and law
 * cannot ask everything as a multiple-choice question: writing the function IS
 * the skill, ordering the steps IS the skill.
 *
...
export const ANSWER_FORMATS = {
     ...
     * This one grades locally, so a calculation is no longer the only typed
     * thing an assessment can carry.
     *
