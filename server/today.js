...
import { calculatePace, daysUntil, getValidStudyDates, readStudyDays } from './scheduling.js';
...
// The `completed_at` a node gets when it is set to completed, as the right-hand
// side of an UPDATE (one `?`, the new timestamp). A node that is ALREADY
// completed keeps the day it was finished: a retaken mastery check is not a
// second finish, and the journey replay and the completion screen are ordered
// by this date. SQLite evaluates it against the row as it was before the UPDATE.
export const COMPLETED_AT_ON_COMPLETE = "CASE WHEN status = 'completed' AND completed_at IS NOT NULL THEN completed_at ELSE ? END";

/**
 * What node ids ARE, for a surface acting on ids a model proposed: the real
 * title and project, the status, and `kind` — `topic | section | note | stage`.
 * A mastery check is taken on a topic only, a card hangs on a topic or a
 * deck's stage, and a completed topic's check is a retake. Unknown ids are
 * simply absent.
 */
export function nodeLabels(ids) {
    const wanted = (Array.isArray(ids) ? ids : []).map(Number).filter(Number.isInteger).slice(0, 20);
    if (!wanted.length) return [];
    return db.prepare(`
        SELECT n.id, n.title, n.project_id as projectId, p.name as projectName,
               n.status, COALESCE(p.status, 'active') as projectStatus,
               CASE WHEN n.is_note = 1 THEN 'note'
                    WHEN NOT (${LEAF_NODE}) THEN 'section'
                    WHEN NOT (${WORK_LEAF}) THEN 'stage'
                    ELSE 'topic' END as kind
        FROM nodes n JOIN projects p ON p.id = n.project_id
        WHERE n.id IN (${wanted.map(() => '?').join(',')})
    `).all(...wanted);
}

/**
 * May a mastery check be drawn on this node? A check is taken on a TOPIC: on a
 * section it would write a quiz for the whole section and a pass would close
 * the section itself; a note is reading material and a deck's stage is a place
 * in its card order. Returns `{ status }`, or `{ refused, code, httpStatus }`.
 */
export function masteryCheckTarget(nodeId) {
    const node = db.prepare(`SELECT n.status, (${WORK_LEAF}) as work FROM nodes n WHERE n.id = ?`).get(Number(nodeId));
    if (!node) return { refused: 'That topic no longer exists.', code: 'not_found', httpStatus: 404 };
    if (!node.work) return { refused: 'A mastery check is taken on a topic, not on a section, a note or a deck stage.', code: 'not_a_topic', httpStatus: 400 };
 ...
 *     evidence (a mastery check, a paper attempt, a placement probe) still appears
