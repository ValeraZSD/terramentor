...
// of the year. The small cut the UI strokes — `src/components/BrandMark.tsx` —
// is the `currentColor` copy (`tools/brand.mjs` records why it exists); its
// hollow pages cull the globe through the same `CULL_BOX`/`pageHoles` the real
// mark clips with, and are drawn from the holes so the two cannot drift.
//
...
/** The two page polygons as holes punched in a clip — shared with BrandMark,
 * whose UI cut strokes the mark hollow and must cull the globe behind the
 * pages exactly where the real mark does. */
export const pageHoles = () => `${pathOf(placedPage('right'))} ${pathOf(placedPage('left'))}`;

/** The clip's outer box, drawn well outside the artwork's own bounds so that
 * the clip never touches the globe's edge — only the holes may cut anything. */
export const CULL_BOX = 'M-400 -400 H800 V800 H-400 Z';
...
export function markArt({ d = 46, stroke = 2.2, paper = '#ffffff', ink = '#000000', grid = true } = {}) {
    ...
    const clip = `<clipPath id="${id}">`
        + `<path clip-rule="evenodd" d="${CULL_BOX} ${pageHoles()}"/>`
        + '</clipPath>';
 ...
 *              ink blots: measured by `brand.mjs --check`, a grid line turns 46%
 *              of the pixels it touches into ink at 16px against 66% at 64px, so
 *              at 16px it is a smudge more often than a line. The chosen cut is
 *              kept from 20px up and dropped
