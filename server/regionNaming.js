 ...
 * the name.
 *
 * The model that wrote the name is deliberately NOT in this key, though the
 * `node_embeddings` sidecar keys on its model and must: two models' vector
 * spaces are incomparable, so a vector from the wrong one is simply wrong. A
 * name is not a vector. It is validated prose about a list of titles, and one
 * written by a different model is not invalid, only written by a different
 * model. Keying on the model would hand the map's names to the chat-model
 * setting, for reasons that have nothing to do with the atlas: 91 of the real
 * library's 115 regions hold a good name — "Physics", "Life Sciences",
 * "Mathematics", "Japanese Learning" — that would drop out of reach, and every
 * bubble would fall back to the medoid this module exists to replace.
 *
...
/**
 * The hash scheme rows written before `regionSignature` carry, kept ONLY so
 * those rows can be adopted. A hash is one-way and every name in an existing
 * library is stored under it, so without this, changing the key would discard
 * every stored name.
 */
