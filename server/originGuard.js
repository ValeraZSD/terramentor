...
/** `host[:port]` → `[name, port]`; a bracketed IPv6 literal keeps its brackets. */
function splitHostPort(h) {
    if (h.startsWith('[')) {
        const end = h.indexOf(']');
        return [h.slice(0, end + 1), h.slice(end + 1).replace(/^:/, '')];
    }
    const cut = h.lastIndexOf(':');
    if (cut > 0 && !h.slice(0, cut).includes(':')) return [h.slice(0, cut), h.slice(cut + 1)];
    return [h, ''];
}

/**
 * A host as an origin would write it: lowercased, the scheme's default port
 * dropped. `Host: study.example.com:443` and `Origin: https://study.example.com`
 * name one place, and this is what lets them compare as strings.
 */
function canonicalHost(host, protocol) {
    const [name, port] = splitHostPort(String(host || '').trim().toLowerCase());
    if (!name) return '';
    const dflt = protocol === 'https:' ? '443' : '80';
    return port && port !== dflt ? `${name}:${port}` : name;
}

/**
 * The scheme the browser used for THIS request, or null when the request
 * object cannot say (the CORS callback runs without one). A proxy's
 * X-Forwarded-Proto wins because it is the scheme the browser saw; otherwise
 * a TLS socket says https and a plain one http.
 */
function requestProtocol(req) {
    const fwd = String(req.headers['x-forwarded-proto'] || '').split(',')[0].trim().toLowerCase();
    if (fwd === 'http' || fwd === 'https') return `${fwd}:`;
    if (req.socket) return req.socket.encrypted ? 'https:' : 'http:';
    return null;
}
...
export function isAllowedOrigin(origin, req) {
    ...
    // The origin this request arrived on — the standalone app calling itself,
    // the Vite dev server (its proxy keeps the browser's Host, so
    // localhost:5173 asks as localhost:5173), and a device that typed this
    // server's address directly all land here. The WHOLE origin has to match:
    // scheme, name and port. The name alone is not enough, because another web
    // service on another port of this same machine is another origin with the
    // same name, and with the auth gate off (the default) a simple POST from
    // such a page would run before CORS hid the answer. The scheme is checked
    // when the request can say what it was; the CORS callback cannot, and there
    // the port still decides.
    if (req) {
        const proto = requestProtocol(req);
        if (proto === null || proto === parsed.protocol) {
            const here = canonicalHost(req.headers.host, proto || parsed.protocol);
            if (here && here === canonicalHost(parsed.host, parsed.protocol)) return true;
        }
    }
    ...
    // arrives carrying X-Forwarded-Proto.
    //
    // What stops a page forging that header is NOT that it is forbidden — it is
    // not on the Fetch spec's forbidden list, and `fetch()` will set it happily.
    // It is that setting it makes the request non-simple, so the browser sends a
    // CORS preflight first, and `cors()` reflects nothing for an origin
    // `isAllowedOrigin` rejects — the preflight fails and the real request is
    // never sent. That is a load-bearing dependency in the other direction: the
    // moment something is added to ALLOWED_ORIGINS, or the cors mount stops
    // consulting this function, this line stops being safe. Any other cross-host
    // origin is one entry in ALLOWED_ORIGINS away.
