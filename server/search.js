 ...
 * separator. An allowlist \u2014 `a-z0-9`, Latin-1 Supplement
 * and Extended-A, CJK ideographs \u2014 silently deletes every other
 * alphabet before the length filter runs, so a query in one of them becomes zero
 * tokens and `searchAll` returns an empty result for every table. Cyrillic
 * (a Russian course), kana (hiragana and katakana are NOT inside
 * \u4e00-\u9fff, so a Japanese deck is findable by kanji but not by the
 * reading), Greek, Hebrew, Arabic, Hangul \u2014 all unsearchable, and indis-
 * tinguishable from "nothing matched".
 *
 * `\p{L}` and `\p{N}` need the `u` flag; they cover the accented Latin the
 * ranges above were there for, so nothing that worked before changes.
 */
