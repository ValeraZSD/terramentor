...
// The kinds of evidence BKT accepts. This list used to live as a CHECK constraint
// on mastery_evidence; the constraint was dropped (see the migration in
// database.js) because SQLite cannot ALTER one, and every new evidence kind since
// silently 500'd against the stale constraint instead. Validated here rather than
// per-endpoint because this function is the single write path — feed.js, index.js
// and paper.js all funnel through it.
//
// Not all evidence is equal: checkMasteryEligibility's raw-score clause honours
// only 'quiz' | 'boss_fight' | 'paper'. 'drill' and 'flashcard' sharpen the
...
// it might.
export const VALID_EVIDENCE_TYPES = ['quiz', 'flashcard', 'boss_fight', 'drill', 'paper', 'placement'];
 ...
 *   (b) it has at least one real assessment (quiz/boss_fight/paper) of
 *       >= MIN_GATE_QUESTIONS questions answered at >= `bossPass` raw accuracy.
 *
 ...
 * ~0.67). Gating purely on BKT made "passing" a Boss Fight fail to complete the
 ...
 * @param {number} bossPass  - Minimum raw accuracy for assessment evidence (default 0.8)
 * @returns {{ eligible, mastery_score, evidence_count }}
 */
export function checkMasteryEligibility(nodeId, threshold = 0.85, bossPass = 0.8) {
    const record = getNodeMastery(nodeId);
    ...
    const strongEvidence = db.prepare(`
        ...
        WHERE node_id = ?
          AND evidence_type IN ('quiz', 'boss_fight', 'paper')
          AND total >= ?
          AND (score * 1.0 / total) >= ?
        LIMIT 1
    `).get(nodeId, MIN_GATE_QUESTIONS, bossPass);
...
export function getProjectMasteryStats(projectId) {
    return db.prepare(`
        SELECT
            ...
            (SELECT COUNT(*) FROM mastery_evidence me WHERE me.node_id = nm.node_id AND me.evidence_type = 'boss_fight') as boss_fight_count
 ...
 *    `decay_days`). Topics cleared only by a raw Boss Fight while BKT stayed below
 ...
 */
export function getNodeMasteryDetail(nodeId, { threshold = 0.85, bossPass = 0.8 } = {}) {
    const record = getNodeMastery(nodeId);
    ...
    const eligibility = checkMasteryEligibility(nodeId, threshold, bossPass);
