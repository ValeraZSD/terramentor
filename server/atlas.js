...
async function loadTopics({ includeArchived = false } = {}) {
    const rows = db.prepare(`
        SELECT n.id, n.title, n.status, n.project_id, n.parent_id,
               ...
               -- LEAF_NODE's test: a section heading is on the map too, and
               -- is not a topic the course counts.
               (SELECT COUNT(*) FROM nodes c WHERE c.parent_id = n.id AND c.is_note = 0) = 0 AS is_leaf
        FROM vec_nodes v
        ...
        WHERE n.is_note = 0
          ...
          --
          -- The test is per node (server/nodeRole.js): testing "is this project
          -- a deck" also excludes the subdecks the deck's AUTHOR named — 32
          -- real topics in one imported deck, 6 in another, measured
          -- 2026-09-09 — from a map of what the learner knows.
          AND ${TOPIC_NODE}
        ORDER BY n.id
    `).all();
...
function summarise(region, id, threshold, positions, lineage, namedLabel = null, nameSource = null) {
    ...
    const topics = region.members.map((m, i) => {
        ...
        return {
            ...
            // A topic the course COUNTS (no non-note children), as opposed
            // to a section heading drawn on the map beside its topics.
            leaf: !!m.is_leaf,
