 ...
 *
 * A course written outside the app used to have nowhere to put practice except
 * `description`, so a real one shipped 4,315 questions as markdown prose with
 ...
 */
export const READ_PROJECT_FIELDS = ['name', 'description', 'color', 'icon', 'content_language', 'version', 'uuid'];
export const READ_NODE_FIELDS = ['title', 'description', 'notes', 'status', 'is_note', 'uuid', 'children', 'questions', 'resources'];
...
export function normalizeImportProject(project, { isSupportedLanguage }) {
    ...
    // An unrecognised code used to become '' in silence, which means "follow the
    // material" — i.e. a shared Dutch course would have every future lesson
    // authored in whatever language the importer's model guessed from the
    // titles. Same outcome, but now it says so.
 ...
 * @returns {{nodes: object[], warnings: string[], count: number, questionCount: number}}
 */
export function normalizeImportTree(nodes, deps = {}) {
    ...
    return { nodes: tree, warnings: warnings.list(), count, questionCount: questionBudget.total };
}
