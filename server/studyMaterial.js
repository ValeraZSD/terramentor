 ...
 * once must be indistinguishable from the one the Boss Fight makes for one, or
...
import { buildNodeContext, AI_PROMPTS, aiProvenance } from './ai.js';
...
export function finalizeQuiz(nodeId, aiResponse, includeGhosts, { decayDays = 14, questionType = null } = {}) {
    ...
    // A saved quiz is reused by every Boss Fight on its topic, and the gate must
    ...
    questions = questions.reduce((acc, q) => {
        ...
        // MORE: this quiz can be a Boss Fight. It catches two options that are
    ...
    // Boss Fight gate, which must stay a pure assessment of its own node).
    if (includeGhosts) {
        try {
    ...
    const result = db.prepare('INSERT INTO quizzes (node_id, title, questions, generated_by) VALUES (?, ?, ?, ?)')
        .run(nodeId, `Quiz - ${new Date().toLocaleDateString()}`, JSON.stringify(questions), aiProvenance());
 ...
 * this path — the Boss Fight, the one assessment that decides whether a topic
 ...
 * answered equally by 515 and 525, and a live Boss Fight shipped both as options
...
export async function vetQuiz(nodeId, quiz, { signal, onProgress } = {}) {
    ...
    if (!kept.length) {
        // Every question was unsound. Leaving the row behind would hand the Boss
        // Fight a saved quiz it can never use (the modal only regenerates when it
