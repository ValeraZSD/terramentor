...
export function parseTemplate(fmt) {
    ...
    // named directly because the signal used to be `<script>` ANYWHERE in the
    // template, and that is a proxy which fails in both directions: a script
    // cannot change which field lands on which side, which is the only thing
    // this module reads, and decks attach them for ordinary presentation.
    // Measured on two real decks: one MathJax loader vetoed 63 of a maths
    // deck's notes, and a three-line autoplay snippet vetoed a phrasebook's
    // 4,317 -- every card it had -- reported as "unsupported note type".
...
export function templateSides(tmpl, fieldNames, hasContent, textOf = null) {
    ...
    // recalling the gloss. Everything but `front` used to be flattened into
    // `extras` — one answer-side block — and the client then tried to recover
    // which of those lines had been on the question by matching the word against
    // the sentence. That is a guess standing in for something the file states
    // outright, and it failed exactly where a guess fails: する conjugates to
    // します, shares no prefix with its own dictionary form, and lost its
    // sentence. These indices stay in `extras` as well, because a template
