...
export async function readMediaIndex(zip) {
    ...
    if (!entry) return { map: new Map(), format: 'none' };
    ...
    if (isZstd) {
        const names = decodeMediaEntries(Buffer.from(zstdDecompressSync(raw)));
        const map = new Map();
...
export async function extractMedia(zip, wanted, { onProgress } = {}) {
    ...
    for (const name of wanted) {
        ...
        try {
            ...
            if (compressed) buf = Buffer.from(zstdDecompressSync(buf));
        } catch {
            skipped.push({ name, reason: MEDIA_SKIP_REASONS.UNREADABLE });
            continue;
        }

        if (buf.length > MAX_MEDIA_BYTES) { skipped.push({ name, reason: MEDIA_SKIP_REASONS.TOO_LARGE }); continue; }
        if (bytes + buf.length > MAX_MEDIA_TOTAL_BYTES) { skipped.push({ name, reason: MEDIA_SKIP_REASONS.BUDGET }); continue; }

        const type = sniffMediaType(buf);
        if (!type) { skipped.push({ name, reason: MEDIA_SKIP_REASONS.UNSUPPORTED }); continue; }

        const { hash, size } = mediaStorage.put(buf);
        bytes += size;
        files.set(name, { hash, size, kind: type.kind, mime: type.mime, filename: name });
