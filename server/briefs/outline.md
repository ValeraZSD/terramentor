...
**`new_per_day` is for a course that ships flashcards** (see section 2): how many new cards a
day it asks the learner to take on. Omit it in this pass; there are no cards yet.
...
| `flashcards` | Flashcard[] | **omit; do not use it in this pass.** Spaced-repetition cards the app schedules on that topic: `{front, back, extra?, extra_front?, media?}`, media by file name and only inside a `.studyvault` bundle. Like questions, written against material that does not exist yet. |
...
The real teaching, 600–2000 words per leaf with derivations, worked examples and failure modes,
...
title still tells the learner what to look for, but the link is gone. The drop is a safeguard,
not a judgement of your output: a shared course arrives from someone you do not know, and its
links render as real links.
