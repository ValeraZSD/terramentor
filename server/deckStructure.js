 ...
 *
 * A flat deck has no structure of its own. Imported as ONE node, a 1.5k-card
 * deck is a project whose entire curriculum is a single topic named after it,
 * holding 1,501 flashcards: the tree draws one empty category card saying "No
 * items yet", the dashboard offers "0% complete" against a denominator of 1,
 * and "Your Journey" is a progress bar that can only ever read 0/1 or 1/1.
 * Every other part of the engine keys on a node — the feed picks FOCUS nodes,
 * mastery is per node, checkpoints are node boundaries — so a deck with one
 * node gets none of it, and new cards are introduced with nothing to say which
 * ones came first.
 *
