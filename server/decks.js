...
export function deckCounts(projectId, { now = new Date() } = {}) {
    ...
    //
    // It used to be derived from the card row: `review_count = 1`, reviewed
    // today, `stability IS NOT NULL`. Every clause of that was a workaround for
    // not having a review log, and the (re)learning ladder broke it in the
    ...
    // this count: the more thoroughly a card was learned, the less it counted,
    // and a learner who finished every ladder was handed a fresh twenty the
    // moment they closed the session. Measured on a real deck the day
    // it was found: 22 cards genuinely met, 20 counted, the missing two being
    // exactly the two answered more than once.
    //
    ...
    //
    // `source = 'app'` does the job `stability IS NOT NULL` used to. The
    // importer stamps `last_reviewed` as the moment of import for every card
    ...
    // under `source = 'anki'`. A card first met months ago elsewhere was not
