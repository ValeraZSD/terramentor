...
function nextMissing(nodeId) {
    ...
    // practice set written earlier — is asked from that bank after each part
    // (feed.js), so no per-part question is authored for it: that was a model
    // call per part spent writing a question beside sixty vetted ones.
    const banked = ownedQuestions(nodeId) >= MIN_GATE_QUESTIONS;
    for (let i = 1; i <= parts.length; i++) {
        const lesson = bySeq.get(i * 2 - 1);
        if (!lesson) return { type: 'lesson', i, parts, plan };
        if (!banked && !bySeq.has(i * 2) && !skippedQ.includes(i)) return { type: 'question', i, parts, plan, lesson };
    }
