...
import {
    ...
    nodeLabels,
    masteryCheckTarget,
    COMPLETED_AT_ON_COMPLETE,
} from './today.js';

import { composeFeed, buildFeedHeader, consumeFeedItem, replaceSpecInContent } from './feed.js';
import { drawFromNode, drawFromQuiz, logAsked, parseSavedKey, masteryCheckSize, PRACTICE_SESSION_SIZE, MAX_CHECK_SIZE } from './questionLog.js';
import { formatSourceContext, resolveCitations } from './citations.js';
import { webSearchEnabled } from './webContext.js';
import {
    chatTools, runToolRounds, runToolCalls, lateResultsBlock,
    extractToolTail, createTailGuard, toolTailRule, MAX_CALLS_PER_TURN,
    wireTools, parseToolArgs, nativeToolRule, runNativeAgentTurn,
    isToolRefusalError, NATIVE_MAX_ROUNDS, hasWebTool,
} from './aiTools.js';
import { SEARCH_BACKENDS, searchKeysStatus, setSearchBackendKey, testSearchBackend } from './searchBackends.js';
...
import { bulkCandidates, startBulk, bulkStatus, cancelBulk, MAX_BULK_NODES } from './bulkGen.js';
...
import { storeMediaBuffer, MAX_MEDIA_BYTES } from './ankiMedia.js';
...
import { extractText, MAX_FILE_BYTES, assertZipSafe, readZipEntry } from './extract.js';
...
import { createAppIconRouter, manifestColors } from './appIcon.js';
...
import { projectProgress, topicFraction, topicWeight, TAUGHT_SQL } from './progress.js';
import { projectCompletion, markCelebrated } from './completion.js';
import { readSettable, settingsBefore, assistantSettingsBlock } from './assistantSettings.js';
...
// nobody asked to be walked through. It is a checklist of the six things that
 ...
 */
/**
 * `<id>` for `/models/<id>/endpoints`, or null if the id cannot be one.
 *
 * A model id carries a slash of its own (`vendor/model`) and must keep it, so
 * only the SEGMENTS are encoded — and `encodeURIComponent('..')` is `'..'`,
 * because a dot is an unreserved character. That left `?model=../../x`
 * building `<base>/models/../../x/endpoints`, which `fetch` normalises into
 * `<base>/../x` — a different path on the provider the learner configured,
 * requested with the learner's API key on it. A segment that means "somewhere
 * else" is refused rather than encoded, which is the only answer: there is no
 * escaping that survives the URL normaliser.
 */
 ...
 */
async function buildSourceContext(nodeId, projectId, message, { limit = 3, useVault = true, useLibrary = false, pageContext = '', history = [], emit, signal, native = false } = {}) {
    let chunks = [];
    try {
    ...
    const tools = chatTools({ web: webSearchEnabled(), library: useLibrary });
    ...
    return {
        text: (text ? `\n\n${text}` : '') + looking + gap + toolTailRule(tools),
 ...
 */
async function runLateLookups({ tail, tools, calls, items, context, message, system, history, emit, signal, answer }) {
    ...
    if (wanted.length) {
        ...
        const { added } = await runToolCalls({ wanted, tools, calls, items, context, emit });
        const failed = wanted.some(c =>
            calls.find(d => d.tool === c.tool && d.arg === c.arg)?.summary === 'the search failed');
...
app.post('/api/ai/chat', async (req, res) => {
    ...
    try {
        ...
        // judged in its exchange rather than on its own.
        ...
        if (nodeId) {
            ...
            if (node) ({ text: ragContext, sources: ragSources, calls: toolCalls = [], tools: ragTools, items: ragItems, context: ragLooked } =
                await buildSourceContext(nodeId, node.project_id, message, { useVault: useRag, history }));
        }
        ...
        const { system, user } = AI_PROMPTS.tutor(context, ragContext, message, getUiLanguage(), { web: hasWebTool(ragTools) });
        ...
        if (ragTools.length && aiResponse.trim()) {
            ...
            if (tail.calls.length) {
                ...
                aiResponse = tail.head + continued;
...
async function runChatTurn({ nodeId, projectId, message, useRag, emit, signal, pageContext = '', ragProjectId = null }) {
    ...
    if (isGlobal) {
        ...
        ({ system, user } = AI_PROMPTS.tutor(context, ragContext, message, getUiLanguage(), { web: hasWebTool(ragTools) }));
    }
...
app.get('/api/nodes/:nodeId/mastery-check/draw', (req, res) => {
    ...
    const { bankSize, questions } = drawFromNode(nodeId, size);
    ...
    res.json({ size, bankSize, status: target.status, questions: questions.map(q => ({ ...q.question, quizId: q.quizId, index: q.index })) });
});
...
app.get('/api/ai/quizzes/:quizId/draw', (req, res) => {
    ...
    const { nodeId, bankSize, questions } = drawFromQuiz(quizId, size);
    if (nodeId == null) return res.status(404).json({ error: 'Quiz not found' });
    res.json({ size, bankSize, questions: questions.map(q => ({ ...q.question, index: q.index })) });
});
...
app.post('/api/ai/quizzes/:quizId/attempt', (req, res) => {
    ...
    try {
        ...
        const entries = Array.isArray(asked)
            ? asked.map(a => ({ quizId, index: a?.index, correct: a?.correct }))
...
//
// A BACKGROUND TASK, like the author endpoint above it, and not a bare SSE
...
app.get('/api/ai/creation-status', (req, res) => {
    ...
    `);
    const generations = [];
    for (const handle of activeGenerations.values()) {
    ...
    for (const row of rows) {
        try {
            ...
            for (const q of parsed) {
                ...
                const model = q.generated_by || rowModel;
                out.push(model ? { ...q, generated_by: model } : q);
            }
...
const exportedFlashcards = (nodeId, mediaNames = null) => {
    ...
    return rows.map(r => {
        // The uuid is the card's identity across machines (COURSE-UPDATES.md):
        // what lets a later edition update this card instead of adding a twin.
...
app.get('/api/export/:projectId/bundle', async (req, res) => {
    ...
    // Same opt-in contract as the plain JSON export: the artifact's own
    // comment calls it "the shape a future marketplace publish would ship",
    // so private `notes` are as opt-in here as there. Default off — the
    ...
    try {
        ...
        res.setHeader('Content-Disposition',
            contentDisposition(`${project.name || 'project'}.studyvault`, { inline: false }));
...
app.post('/api/import/bundle', rejectOversizedBody(208 * 1024 * 1024), bundleUpload.single('bundle'), async (req, res) => {
    ...
    try {
        ...
        // sharing them). This reads the archive's own declared sizes, which its
        // author wrote, so it is a pre-filter and not the bound: every entry
        // below is read through `readZipEntry`, which stops inflating at a
        ...
        if (!raw) return res.status(400).json({ error: 'Invalid bundle: missing manifest.json' });
    ...
    let project, nodes, warnings, cardCount = 0;
    try {
    ...
    const charge = (n) => {
        inflatedBytes += n;
        if (inflatedBytes > BUNDLE_ZIP_LIMITS.maxTotalBytes) {
            throw new Error(`this bundle expands to more than ${Math.round(BUNDLE_ZIP_LIMITS.maxTotalBytes / 1024 / 1024)} MB and is being refused as a compression bomb`);
        }
    };
    try {
    ...
    try {
        ...
        return res.status(400).json({ error: `Failed to restore card media: ${e.message}` });
    }
    ...
    try {
        ...
        res.json({ ...newProject, cardCount, mediaCount: mediaByName.size, warnings });
    } catch (err) {
...
app.post('/api/projects/:projectId/mastery/mastery-check', (req, res) => {
    ...
    try {
        ...
        const result = updateMasteryFromAttempt(nodeId, score, total, 'mastery_check', {
            questions,
            type: 'mastery_check',
        });
        ...
        const passed = score / total >= checkPass;
        // pass_threshold lets the UI show the *configured* bar instead of a
        // hardcoded percentage.
        res.json({ ...result, passed, raw_score_pct: Math.round((score / total) * 100), pass_threshold: checkPass });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.get('/api/feed', (req, res) => {
    try {
        ...
        const { items, exhausted, transfers } = composeFeed({
            limit, excludeKeys, gate, nodeId: scopedNode, projectId: scopedProject,
        });
        const header = buildFeedHeader();
        res.json({
            date: header.date, header, items, exhausted, transfers,
...
app.post('/api/feed/consume', (req, res) => {
    try {
        ...
        // A saved question served by the feed is remembered as asked, so the
        // stream moves through the bank instead of re-serving its head.
        const saved = kind === 'question' ? parseSavedKey(key) : null;
        if (saved) logAsked([{ ...saved, correct: result && typeof result === 'object' ? !!result.correct : null }], 'feed');
