/**
 * Bulk study-material generation: mastery checks and flashcards, many topics at once.
 *
 ...
 *
 * `mastery_check`, named for what the rows ARE: a node's saved questions are its
 * mastery check bank (`MasteryGateModal` reads every non-ghost question the node
 * has and only generates when it finds none), so generating them ahead of time
 * is pre-loading the gate — at 5 questions, while the gate itself asks for 10
 * and needs `MIN_GATE_QUESTIONS` to count as an assessment at all. "Quiz" is
 * the name that makes the expensive path look optional and the useful one
 * invisible, so the kind is not called that; `quiz` is still accepted from
 * older clients.
 */
export const BULK_KINDS = ['mastery_check', 'flashcards'];

/** Older clients (and the previous dialog) say `quiz`; it means the same rows. */
export function normalizeKind(kind) {
    return kind === 'quiz' ? 'mastery_check' : kind;
}

/** How long a call of each kind took last time, so the dialog can say "~6 min". */
const AVG_SETTING = { mastery_check: 'bulk_avg_ms_mastery_check', flashcards: 'bulk_avg_ms_flashcards' };
/** Used until a run of that kind has actually been measured on this machine. */
const AVG_FALLBACK_MS = { mastery_check: 75000, flashcards: 35000 };
...
/**
 * Milliseconds a call of each kind has been taking on this machine, falling
 * back to a shipped guess for a kind never run here.
 *
 * Nothing SHOWS this. It is the seed for a live run's "~x left" while that run
 * has not yet timed a call of its own, and it is deliberately not served to the
 * dialog: the footer used to price an unstarted job from it, which is a
 * wall-clock promise a job that pauses for the tutor cannot keep.
 */
export function bulkEstimates() {
    const out = {};
    for (const kind of BULK_KINDS) {
        const raw = Number(getSetting(AVG_SETTING[kind]));
        out[kind] = raw > 0 ? Math.round(raw) : AVG_FALLBACK_MS[kind];
    }
    return out;
}
 ...
 * skipped forever, while its mastery check could never reach `MIN_GATE_QUESTIONS`
...
export function bulkCandidates(projectId) {
    ...
    // What the mastery check will actually find: every non-ghost question the node
...
export function startBulk({ projectId, nodeIds, kinds, questionCount = 5, cardCount = 8, skipExisting = true }) {
    ...
    if (!wanted.length) {
        const err = new Error('Pick at least one of: mastery check, flashcards.');
        err.status = 400;
        throw err;
    }
    ...
    job = {
        ...
        avgFor(kind) {
            ...
            return bulkEstimates()[kind] ?? null;
        },
    };
    ...
    try {
        for (const target of j.targets) {
            for (const kind of target.todo) {
                ...
                j.handle?.update({
                    percent: j.percent(),
                    message: `${j.done + 1}/${j.total} · ${target.title} — ${kind === 'mastery_check' ? 'mastery check' : 'flashcards'}`,
                    // This run's own measurements, so the dock shows the ETA the
                    // bulk dialog already had. Everywhere else the dock derives
                    // one from percent and elapsed; here there is a better number
                    // and it would be silly to throw it away at the one call site
                    // that has it.
                    etaMs: j.eta(),
                });
                ...
                try {
                    // One implementation, shared with the feed's look-ahead
                    // (feedGen's `material` tier) — material written for twenty
                    // topics at once, an hour before the learner arrives, or by
                    // the gate itself must all be the same rows, vetted the same
                    // way, or the cheap path quietly becomes the unchecked one.
                    // It never injects ghosts, for the reason stated there.
                    await generateMaterial(target.nodeId, kind, {
