...
/**
 * A library's identity, without saying where it is.
 *
 * The desktop launcher's single-instance probe has to decide "is the server on
 * this port serving the library I am about to open?" before there is a session
 * to ask with, so the answer is public (server/desktop.js). The data directory
 * itself cannot be that answer: on the configuration SECURITY.md recommends —
 * a password set, reachable over a tailnet — it would hand the account name
 * and the path of an unencrypted database to anyone who could reach the port.
 *
 * A hash compares exactly the same and spells out nothing. Normalised the way
 * the file system compares paths: resolved, and lower-cased on Windows.
 * Truncated to 16 hex characters — this distinguishes at most a handful of
 * directories on one machine, and a shorter string is one less thing on a wire.
 *
 * It is not a secret: somebody who already knows the account name can hash the
 * default path and compare. It is not meant to be — it is meant not to SAY the
 * name, which is the whole of what leaked.
 */
export function libraryId(dir, platform = process.platform) {
    if (!dir) return null;
    const norm = platform === 'win32' ? resolve(String(dir)).toLowerCase() : resolve(String(dir));
    return createHash('sha256').update(`terramentor-library:${norm}`).digest('hex').slice(0, 16);
}
