...
import { existsSync, readFileSync, statSync, readdirSync, unlinkSync } from 'fs';
...
// material, a schedule, boss fights) or `deck` (a card collection, almost always
// an Anki import).
//
...
// "Reasoning" panel survives a reload (it used to be in-memory only).
...
//   boss_fight_pass:   raw fraction of questions needed to pass a Boss Fight (0..1)
...
// called, so passing the Boss Fight could not clear a locked topic — structure
...
try {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS mastery_evidence (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      node_id INTEGER NOT NULL,
      evidence_type TEXT NOT NULL CHECK(evidence_type IN ('quiz', 'flashcard', 'boss_fight')),
      score REAL NOT NULL,
      total INTEGER NOT NULL,
      metadata TEXT,
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
// The original schema pinned it to ('quiz','flashcard','boss_fight'), but the
...
//
// The model is RECORDED here and is deliberately not part of the key. It used to
// be, borrowed from the `node_embeddings` sidecar where it is mandatory (two
// models' vector spaces are incomparable, so a vector from the wrong one is
// wrong). A NAME is not: it is validated prose about a set of titles, and one
// written by another model is not invalid, merely written by another model.
// Measured on the real library the day the chat model was switched: 91 of 115
// regions had a perfectly good name that the map could no longer see, so every
...
try {
  ...
  let backfilled = 0;
  for (const table of ['projects', 'nodes', 'resources']) {
...
// nothing backfills it. Tables that already carry a JSON metadata column
