...
export function generateDailyPlan(projectId, decayDays = 14) {
    ...
    // 4. The next few open leaves in curriculum order. There used to be a
    // dependency-edge condition here as well; the whole dependency layer was
    // removed on 2026-09-04, and `position` was always the real ordering.
    ...
    // 7. Get untested nodes (nodes with no quiz attempts)
    const untestedNodes = db.prepare(`
        ...
        WHERE n.project_id = ?
          ...
          AND n.status NOT IN ('completed', 'skipped')
          AND NOT EXISTS (
              SELECT 1 FROM quizzes q WHERE q.node_id = n.id
              AND EXISTS (
                  SELECT 1 FROM quiz_attempts qa WHERE qa.quiz_id = q.id
              )
          )
        LIMIT 5
    `).all(projectId);
