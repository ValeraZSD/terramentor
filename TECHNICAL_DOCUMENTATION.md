...
| `activity_log` | What the app DID: model calls, background jobs, projects created or deleted. Metadata only, with no titles, prompts or model output. Ring buffer, no FKs. See §12. |
...
every rewrite, leaving ghost copies of other topics' questions alone. Because
...
Notes are material attached to a topic (never scheduled, no status, zero
weight), so a topic whose only children are notes is still a leaf. Two mirrors must
...
0.05), passed from the stored feed row rather than from the client, because it decides
...
where one correct multiple-choice answer reaches 0.1. An assessment of
`MIN_GATE_QUESTIONS` or more takes the batch path instead, and a topic never measured
here enters it at the nominal `p_L0 = 0.3` (or its seeded prior, whichever is higher).
...
The check **draws** `mastery_check_size` questions (4–30, default 10) from every
quiz row the topic owns (`server/questionLog.js`): never asked first, then least
...
no probe propagates nothing and is taught from zero, since twelve questions cannot
...
above `MAX_GUESSABLE` (0.35) seeds **nothing**. That is a rule, not an arithmetic
...
3. **Visual coherence**: three verdicts, `ok`, `wrong` (remove the visual),
   `text-wrong` (the picture was right, and the *lesson* is what gets rewritten).
4. **Numeric audit of the lesson** (`auditLessonMath`, gated on `hasComputation`).

**Defects vs weaknesses.** A defect would mislead and is never served. A weakness
...
may call up to 12 read-only tools: `search_web` (only when "Let answers use the
web" is on), `find_in_library` (local search over projects, topics, saved links
...
Only pages and documents are numbered as citable sources.
...
`POST /assistant/cards`, deduplicated on its front) or a note for the Inbox; each
appears as a button under the answer. Undo on an added card removes it only while
...
| Mastery | `/nodes/:nodeId/mastery`, `/mastery/quiz`, `/mastery/drill`, `/nodes/:nodeId/drill-scores`, `/nodes/:nodeId/mastery-check/draw`, `/projects/:projectId/mastery` (a card's evidence has no route; it is written on the rating path, `server/cardEvidence.js`) |
| Assistant | `/ai/today-chat*`, `/assistant/cards` (POST adds a proposed card, DELETE undoes it only while it has never been reviewed) |
...
# Read your own library, so they are not in the suite:
