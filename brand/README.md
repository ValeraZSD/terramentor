...
library is drawn as a sphere with regions as caps on it
(`src/components/atlas/GlobeMap.tsx`), a course is traced over it as a
great circle, and the newest step is the brightest
...
Windows `.ico` and the macOS `.icns`, so that one file is the desktop icon too, which is why it carries a transparent margin rather than bleeding to the edge.
...
quoted, since either one silently drops the text and leaves the planet alone.
