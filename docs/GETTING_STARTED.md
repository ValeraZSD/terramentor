...
The home page is a **learning feed**, not a dashboard. You do not pick what to study — the app
decides, and you scroll.
...
- **Recall** — a question from a topic you already proved, resurfaced before you forget it.
- **Checkpoint** — the end of a topic, where you can complete it or take its Boss Fight.

Overdue work is served first. That is the whole catch-up mechanism: no nagging banner, the feed
simply gives you what you are behind on.
...
You can build a project by hand, or describe it and let the AI draft the whole curriculum.
...
Marking a topic complete can be intercepted by the **mastery gate**, which offers a **Boss
Fight** — a short quiz on that topic alone.

Three modes, in Settings → Learning:

- **Off** — completion is never questioned.
- **Advisory** (the default) — you are asked to prove it, but "mark done anyway" is always there.
- **Enforced** — completion needs proof.
...
- **Recall cards** — questions drawn from topics you already *proved*, once the estimate of your
  retention has decayed. Answering one refreshes both the estimate and the timer.
...
- **The assistant** (the speech-bubble icon, or press **a**) — one conversation that sees every
  ...
  screen. The per-topic **AI Tutor** lives inside a topic and goes deeper on that one thing.

Everything runs against a local model by default, and your data stays on this machine.
