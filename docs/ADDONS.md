...
|---|---|---|
| "a different theme" | **yes, fully** | data: the app's own colour ladder (11 RGB triplets, an accent, a light/dark axis). No CSS. |
...
| "new features that change the app substantially" | **yes, inside those boxes and never outside them** | anything expressible as the rows above, composed. An add-on may add a screen, a card face, a policy and a theme at once. |
...
filesystem access and command execution, and 2026 saw real malware delivered through exactly
that channel. VS Code is the same shape. Both are excellent products; neither makes the promise
this one does.

A community-managed catalogue makes this sharper, not softer. Nobody reviews a bundle, and a
review that did happen is a reading, not a security mechanism. **The sandbox is the
...
|---|---|:---:|---|---|
| **0** | **Declarative.** A manifest names *what*; the host does *how*. Themes, layout switches, resource types, search providers. | no | n/a | mechanism exists (`search_provider` is its only kind) |
| **1** | **Content packs.** Course templates, decks, drill banks, question banks, rubric sets, a tutor *voice*. | no | n/a | formats exist (import paths, ```drill); no packaging |
...
| n/a | **Host code.** Filesystem, network, the process. | yes | place 1 | **deliberately never built** |
...
`local`). `search_providers` stays where it is, since it predates this and works; a search-provider
...
| `write:settings:<key>` | change one named setting | the assistant's whitelist (`theme`, `theme_tint`, `accent_color`, `ui_scale`, `week_start_day`,
`ui_language`, `number_format`) is the model: visible instantly, undone in one tap; nothing that changes what the engine measures |
...
Ten kinds in four groups, chosen because each one is a request that has actually been made and
each maps to a mechanism that exists. Adding an eleventh is the moment to reread *where code may
run*.

### `theme` (Tier 0)

The app's theme *is* eleven RGB triplets (`--c-white`, `--c-slate-50` … `--c-slate-900`), an
accent pair (`--accent-rgb`, `--accent-fg-rgb`) and an axis (`light` | `dark`), which is what
`src/utils/themeRamp.ts` generates from a mode and a tint and writes onto `<html>`. A theme
part is that as JSON:
...
then does what a tint does today (writes the variables on `<html>`) and every surface,
...
the host's. It is shown in full on the install screen because it is prose the learner can
...
- **`card_face`** replaces the front/back rendering of flashcards on a project (or on cards
  ...
  rating buttons, the undo, the audio chain and the keyboard remain the host's, since a face that
...
|---|---|---|---|
| `feed.mix` | the candidate queues (lessons, questions, cards, recalls, as ids and metadata and never content) and the feed settings | an ordered list of ids | every id must be a candidate; length clamped; duplicates dropped |
...
not disable it on its own, since that would be the app acting on a network answer at startup.
...
1. **The `addons` table, the manifest validator, the install/enable/remove screen, and `theme`
   ...
   and the Settings UI to exist. A day or two. Ship one built-in theme through it (a shipped
   tint re-expressed as a part) so the mechanism is exercised on every install.
...
4. **Policies**, only when a second implementation of a slot is genuinely wanted. The isolate
   ...
   the request path.
5. **The registry**: a repository, an index and in-app browse, after steps 1–3 have shipped and
...
- **No CLA required for an add-on.** Add-ons are separate works with their own licence, and they are the best
