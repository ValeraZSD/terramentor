...
> **Using a different port?** Terramentor moves to 3002, 3003… if something else
> holds 3001. The port it actually bound is in Settings → General → *This
...
your own devices, the password is a second lock on your own front door — worth
having anyway, because a tailnet often grows a shared device later, and because
a phone is a thing that gets left on tables.
...
- Your data does not move. There is no sync, no account, no copy anywhere — the
