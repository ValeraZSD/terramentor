...
zip, unpack it, double-click. The app carries its own runtime, so your machine
needs nothing else, and there is no account.
...
which is the drive most likely to be full, since a library with a few imported decks
...
before the first start and the library lives there instead, so the whole app
...
That last point is the difference the icon makes: on Windows, *Keep running in
the background* is simply how the app behaves, so the panel says so instead of
drawing a control that cannot change it.
...
the server, since closing it to tidy the desktop would stop the app.
...
- **Server dependencies are scanned, then pruned.** `npm ci --omit=dev`, then
  ...
  `npm prune` removes the rest at lockfile versions, and the client's packages are
