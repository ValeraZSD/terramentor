...
Where the app offers to send you to look a topic up yourself. Settings → **Search links**. This
is not *Answering with the web*, which is the separate AI setting that grounds an answer in live
pages.
...
What is here is deliberately small: a name, an icon, and an `https` URL template the host fills
with the topic title when you click it.
...
1. **Protocol must be `https:`.** This blocks `javascript:` outright, because that URL ends up in an
...
Built-in providers can be disabled but not removed or replaced, since letting a third-party manifest
