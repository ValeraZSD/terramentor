...
**Why a CLA, stated honestly.** The plan is open core: the engine stays AGPL and local forever;
...
a non-trivial chunk, say so in the PR. Not because it is disallowed — it is not — but because
provenance matters for a copyleft project and it is easier to say now than to reconstruct later.
...
- **Match the surrounding code.** Comment density here is higher than most projects because the
  comments record *why*, usually a bug that was expensive to find. Keep that up — a comment
  explaining what a line does is noise, a comment explaining why it must be that way is the
  point.
- **Never hardcode `violet-*` or any accent colour.** Use the `accent` Tailwind colour; it is
  driven by a CSS variable and is per-project.
- **Input capability is not a width breakpoint.** Use the `can-hover:` / `touch:` variants and
  the helpers in `src/utils/platform.ts`. A phone in landscape clears `sm:`.
...
**A prerelease is the whole stable/unstable mechanism.** A tag carrying a suffix — `v1.1.0-rc.1`
— is flagged as a prerelease, which means GitHub's `/releases/latest` skips it. That endpoint is
...
The releases below 1.0.0 happened before this repository existed; `CHANGELOG.md` keeps them under
**Before 1.0** as a record of what the program could do, in the order it learned to do it. They
are history, not something to regenerate.
