...
console.log('--- the scanner catches the shape it was written for ---');
{
    ...
    const CASES = [
        ...
        ['a mention in a comment is not code',
            "// next_review <= datetime('now') compares as a string\nconst x = 1;", null],
    ];
...
console.log('\n--- the two renderings, on fixed strings ---');
{
    ...
    // …and reads the SAME CLOCK. This used to compare both to the second:
    // `NOW_ISO` and `new Date()` are two separate reads, so a second boundary
    // between them failed a suite in which nothing had been touched — the same
    // shape fsrs-gates.mjs already had to fix once ("the shape of every
    // assertion that reads the clock rather than being handed one").
    //
    // BRACKETED instead: the instant is handed to the assertion as a JS stamp
    // taken before and one taken after, and SQLite's answer has to fall between
    // them. There is no boundary to straddle. `GRAIN` is the instrument's
    // resolution, not a tolerance for flakiness: `%f` renders to the
    // millisecond off a julianday, so one instant can round one millisecond
    // either side of the JS stamp for the same instant — measured at exactly
    // ±1 ms, 1,020 times in 5,000 reads, and 0 in 5,000 outside the bracket
    // once the rounding is allowed for. It is still far tighter than the
    // second it replaces: `datetime('now')`, `'localtime'`, and even a clock
    // one SECOND slow are all refused by it (all four measured 2026-09-23).
    const GRAIN = 1;
    const before = new Date(Date.now() - GRAIN).toISOString();
    const sqlIso = db.prepare(`SELECT ${NOW_ISO} AS v`).get().v;
    const after = new Date(Date.now() + GRAIN).toISOString();
    ok('…and reads the same clock JS does, to the millisecond',
        before <= sqlIso && sqlIso <= after, `${before} <= ${sqlIso} <= ${after}`);
}
