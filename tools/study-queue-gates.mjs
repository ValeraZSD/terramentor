 ...
 *
 * There used to be two, and which one you got depended on which door you opened:
 * a deck screen served `deckQueue` (reviews owed, plus a rationed handful of new
 * cards), while a project's Review button and the cross-project session served
 * `/flashcards/due`, whose SQL counted every never-reviewed card as due. That is
 * the "1,483 cards due on day one" bug — it was fixed for imports and left in
 * place everywhere else, so on the real library (measured 2026-09-09) the
 * Physics project's Review button offered 615 cards in one session, 615 of them
 * never seen.
 *
