 ...
 *
 * Written 2026-09-15 after the Russian atlas read
 *
 ...
 * were a project's NAME, because the key was worded in the singular and the
 * translator read `{{projects}}` as one; and, underneath both, the fact that
 * English loaded no resources at all, so a count-based key could never fire its
 * `_one` form and the codebase had worked around that with the JSX "s".
 *
...
// CRLF (157 CR bytes, measured 2026-09-17) while the generator emits LF, so a
// byte comparison failed for every Windows contributor on a file that was
// perfectly correct — and CI was ubuntu-only, so it could not see it. Both
// halves are fixed: `gates.yml` now carries a windows-latest leg.
 ...
 *  Each one was read before it was listed; the list is per language, never global. */
...
// English's own singular is written by hand and was unchecked until 2026-09-15,
// which is how "1 sections" and "1 media files … were left out." stayed in the
// shipped file through a sweep that was looking for exactly that fault.
