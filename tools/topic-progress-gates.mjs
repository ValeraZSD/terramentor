...
// ---- a TAUGHT topic weighs one unit, whatever it carries -------------------
//
// The course bundle attaches a language course's listening clips to the topic
// they illustrate: sixty on one grammar topic, none on the next. By cards
// alone that topic would be sixty times its neighbour and the course's
// progress would be its listening cards met. A topic with something to
// teach — an overview of reading length, a note child, or a question bank —
// is one unit of its course; only a topic that is nothing but its cards
// weighs its cards.
db.prepare("UPDATE nodes SET description = ? WHERE id = ?").run('x'.repeat(1300), topics[2]);
p = projectProgress(courseId);
// Weights 1, 1, 1, 1: the ten-card topic is half met and worth one.
near('a topic with an overview to read weighs one, not its cards', p.fraction, (1 + 1 + 0.5 + 0) / 4);
db.prepare("UPDATE nodes SET description = '' WHERE id = ?").run(topics[2]);
db.prepare('INSERT INTO quizzes (node_id, title, questions) VALUES (?, ?, ?)').run(topics[2], 'q', '[{"question":"?","type":"true_false","correct_answer":"True"}]');
p = projectProgress(courseId);
near('…and so does a topic with a question bank', p.fraction, (1 + 1 + 0.5 + 0) / 4);
db.prepare('DELETE FROM quizzes WHERE node_id = ?').run(topics[2]);
db.prepare('INSERT INTO nodes (project_id, parent_id, title, position, is_note) VALUES (?, ?, ?, ?, 1)').run(courseId, topics[2], 'reference', 0);
p = projectProgress(courseId);
near('…and a topic with a note child', p.fraction, (1 + 1 + 0.5 + 0) / 4);
db.prepare('DELETE FROM nodes WHERE parent_id = ? AND is_note = 1').run(topics[2]);
p = projectProgress(courseId);
near('with nothing to teach it weighs its cards again', p.fraction, (1 + 1 + 5 + 0) / 13);
// The deck fixture above is untouched by the rule: its sections have no
// overview, no notes, no questions.
near('a deck section still weighs its cards', projectProgress(deckId).fraction, 60 / 110);
