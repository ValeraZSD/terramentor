...
//
// Why this gate exists at all. A `QuestionStem` with one branch — a stem
// through the markdown pipeline only when it carries a fenced VISUAL spec,
// everything else on the KaTeX-only path — leaves a graded question unable to
// show a photograph, which reads as a design decision and is not one. In a
// photo-based subject (a driving theory exam, a radiograph, a specimen) it
// forces every picture question to be demoted to a self-marked flashcard, the
// one surface that cannot clear the mastery gate.
//
