...
// The real molecule renderer too: its graph is what the molecule plan walks,
// so the molecules below are drawn by smiles-drawer itself, not written by hand.
const smilesOut = join(scratch, 'renderSmiles.mjs');
await esbuild.build({
...
const {
    ...
    ARRIVAL_BUDGET_MS, ARRIVAL_MIN_STEP_MS, arrivalScale, globalOrigin, partBox,
...
/** How long THIS part's stroke takes to draw itself, 0 if it does not draw. */
const drawOf = (el) => {
    const second = (el.style.animationDuration || '').split(',')[1];
    return second ? parseFloat(second) : 0;
};
...
console.log('\n--- reading order beats document order ---');
{
    ...
    // Eleven gaps: seven plain and four whole edge draws. Past the budget they
    // shrink together, so the step this diagram actually uses is derived, not
    // the constant — and the constant is what the two one-edge cases below pin.
    const scale = arrivalScale(7 * ARRIVAL_STEP_MS + 4 * ARRIVAL_EDGE_MS);
    const step = Math.round(ARRIVAL_STEP_MS * scale);
    check('the first delay is the lead and the step is the step',
        delayOf(parts[0]) === ARRIVAL_LEAD_MS && delayOf(parts[1]) - delayOf(parts[0]) === step,
        `${delayOf(parts[1]) - delayOf(parts[0])} vs ${step}`);
    check('the shell is told when the LAST part has landed',
        total === Math.max(...parts.map(delayOf)) + Math.max(ARRIVAL_PART_MS, ...parts.map(drawOf)), `${total}`);

    /* NODE, LINE, ARROW, NODE — and the arrow is the half that has shipped
       wrong twice. It was painted from the draw's first frame (a marker
       ignores `stroke-dasharray`), so it beat its own line; then the markers
       were switched off for the length of the draw and the gap before the
       target box stayed a COUNT OF STEPS — 2 × 90ms against a 560ms draw — so
       the box arrived while the stroke was a third of the way along and the ▸
       came last. The invariant that rules out both: whatever follows a part
       that draws itself starts no earlier than that part's stroke lands. */
    const drawn = parts.filter(p => drawOf(p) > 0);
    check('every edge in this diagram draws itself', drawn.length === 4, `${drawn.length}`);
    const late = parts.slice(0, -1).filter((p, i) =>
        drawOf(p) > 0 && delayOf(parts[i + 1]) < delayOf(p) + drawOf(p) - 1);
    check('nothing arrives before the line pointing at it has landed', late.length === 0,
        late.map(p => p.id).join(', '));
    check('…and not long after it either — the gap IS the draw',
        parts.slice(0, -1).every((p, i) =>
            drawOf(p) === 0 || Math.abs(delayOf(parts[i + 1]) - delayOf(p) - drawOf(p)) <= 1));
    check('an edge, then its label, then the box it points at',
        idx('L_0_1') + 1 === parts.findIndex(p => p.getAttribute('class') === 'edgeLabel')
        && idx('L_0_1') + 2 === idx('flowchart-B-0'));
    // The control: the shape this replaced. Two plain steps is nowhere near a
    // draw at any scale, so a beat counted in steps always lands the box first.
    check('a beat counted in STEPS would fail that — the pre-fix shape',
        2 * step < drawOf(parts[idx('L_0_1')]), `${2 * step} vs ${drawOf(parts[idx('L_0_1')])}`);
}
...
console.log('\n--- a molecule GROWS from one atom, and each label arrives with its bond ---');
{
    // jsdom has no SVGSVGElement check smiles-drawer can pass without these.
    globalThis.SVGSVGElement = dom.window.SVGSVGElement;
    globalThis.HTMLCanvasElement = dom.window.HTMLCanvasElement;
    // No canvas backend: smiles-drawer then estimates text size by arithmetic,
    // which is its own documented fallback. Answering null quietly skips
    // jsdom's "not implemented" warning on every label.
    dom.window.HTMLCanvasElement.prototype.getContext = () => null;
    const { default: renderSmiles } = await import(pathToFileURL(smilesOut).href);
    const palette = { fg: '#e2e8f0', bg: '#0f172a', muted: '#94a3b8', accentFg: '#f87171' };
    const draw = async (smiles) => {
        const host = dom.window.document.createElement('div');
        await renderSmiles(host, smiles, { width: 400, palette });
        return host;
    };
    const within = (p, q, tol = 0.6) => Math.hypot(p[0] - q[0], p[1] - q[1]) <= tol;
    const end = (el) => delayOf(el) + drawOf(el);
    let drawnBack = 0;

    for (const [name, smiles] of [['aspirin', 'CC(=O)OC1=CC=CC=C1C(=O)O'], ['benzene', 'c1ccccc1'], ['a salt', '[Na+].[Cl-]'], ['a stereocentre', 'C[C@H](N)C(=O)O']]) {
        const host = await draw(smiles);
        const svg = host.querySelector('svg');
        const graph = JSON.parse(svg.getAttribute('data-vb-molecule') || 'null');
        check(`${name}: the renderer leaves the molecule's graph on the svg`, !!graph && graph.v.length > 0, svg.getAttribute('data-vb-molecule')?.slice(0, 80));
        stampArrivalOrder(host, 'smiles');
        const shapes = Array.from(svg.querySelectorAll('line, polygon, path, circle, ellipse')).filter(s => !s.closest('defs, mask, clipPath'));
        const labels = Array.from(svg.querySelectorAll('.element')).map(t => t.parentElement);
        check(`${name}: every bond, ring and label is stamped — an unstamped part shows from the first frame`,
            [...shapes, ...labels].every(p => p.hasAttribute('data-vb-arrive')),
            [...shapes, ...labels].filter(p => !p.hasAttribute('data-vb-arrive')).map(tag).join(' '));

        // Where each stroke starts drawing, and where it lands.
        const strokes = shapes.filter(s => s.tagName.toLowerCase() === 'line' && /^draw/.test(s.getAttribute('data-vb-arrive')));
        const endsOf = (s) => {
            const a = [+s.getAttribute('x1'), +s.getAttribute('y1')], b = [+s.getAttribute('x2'), +s.getAttribute('y2')];
            return s.getAttribute('data-vb-arrive') === 'draw-back' ? [b, a] : [a, b];
        };
        const atoms = graph.v.filter(Boolean);
        const atomOf = (p) => atoms.find(v => within(v, p));
        // Only the axis strokes run atom to atom; a double bond's second line
        // is offset and shortened, so it is judged by the axis beside it.
        const axis = strokes.filter(s => endsOf(s).every(atomOf));
        if (axis.length) {
            const firstStart = Math.min(...axis.map(delayOf));
            const orphan = axis.filter(s => {
                const [from] = endsOf(s);
                if (delayOf(s) === firstStart) return false;
                return !axis.some(o => o !== s && within(endsOf(o)[1], from) && end(o) <= delayOf(s) + 1);
            });
            // The growth is CONNECTED: a bond leaves an atom the drawing has
            // already reached, never one that is still blank.
            check(`${name}: every bond starts from an atom a landed bond has reached`, orphan.length === 0,
                orphan.map(s => `${s.getAttribute('x1')},${s.getAttribute('y1')}`).join(' '));
        }
        // A label's atom: the graph vertex under its translate (the label is
        // shifted by half a glyph, never by a bond).
        const labelAtom = (l) => {
            const m = /translateX\(([-\d.e]+)px\)\s*translateY\(([-\d.e]+)px\)/.exec(l.getAttribute('style') || '');
            return m ? atoms.find(v => Math.hypot(v[0] - +m[1], v[1] - +m[2]) < 12) : undefined;
        };
        check(`${name}: every label sits on an atom of the graph`, labels.every(labelAtom), labels.filter(l => !labelAtom(l)).map(l => l.textContent).join(' '));
        // Every bond part with a point on that atom — a stroke's end, a
        // double bond's offset line, a wedge's corner. The earliest of them is
        // the bond that REACHES the atom (or, for the atom the drawing starts
        // at, the first bond to leave it).
        const pointsOf = (s) => {
            const t = s.tagName.toLowerCase();
            if (t === 'line') return [[+s.getAttribute('x1'), +s.getAttribute('y1')], [+s.getAttribute('x2'), +s.getAttribute('y2')]];
            if (t === 'polygon') { const n = s.getAttribute('points').split(/[\s,]+/).map(Number); return n.flatMap((_, i) => (i % 2 ? [] : [[n[i], n[i + 1]]])); }
            return [];
        };
        const bondDraw = Math.max(0, ...strokes.map(drawOf));
        const touching = (v) => shapes.filter(s => pointsOf(s).some(p => Math.hypot(p[0] - v[0], p[1] - v[1]) < 10));
        if (bondDraw > 0 && labels.length) {
            // A label arrives WHILE the bond reaching it lands — not in a
            // second pass after the whole skeleton.
            const late = labels.filter(l => {
                const parts = touching(labelAtom(l));
                if (!parts.length) return false; // a lone ion: judged below
                const reached = Math.min(...parts.map(delayOf));
                return delayOf(l) < reached - 1 || delayOf(l) > reached + bondDraw + 1;
            });
            check(`${name}: each label fades in while the bond reaching its atom lands`, late.length === 0,
                late.map(l => `${l.textContent}@${delayOf(l)}`).join(' '));
            // The control is the pre-fix plan: every label after the last
            // bond had started.
            const lastBondStart = Math.max(...strokes.map(delayOf));
            if (labels.length > 1 && strokes.length > 2) {
                check(`${name}: …so the labels are not one batch after the skeleton (the pre-fix shape)`,
                    labels.some(l => delayOf(l) < lastBondStart), labels.map(delayOf).join(' ') + ' vs ' + lastBondStart);
            }
        }
        drawnBack += strokes.filter(s => s.getAttribute('data-vb-arrive') === 'draw-back').length;
        const ring = shapes.find(s => s.tagName.toLowerCase() === 'circle' && s.getAttribute('fill') === 'none');
        if (ring) {
            check(`${name}: the aromatic circle draws once its ring has closed`,
                delayOf(ring) >= Math.max(...axis.map(end)) - 1, `${delayOf(ring)} vs ${Math.max(...axis.map(end))}`);
        }
        if (name === 'a salt') {
            const [first, second] = labels.map(delayOf).sort((a, b) => a - b);
            check('a salt: its two ions arrive one after the other, left to right', second > first, `${first} ${second}`);
        }
        if (name === 'benzene') {
            const starts = new Set(axis.map(delayOf));
            check('benzene: the ring grows both ways round at once — six bonds, three starts', starts.size === 3, [...starts].join(' '));
        }
    }
    // The connectivity check above is only a check on `back` if some bond
    // actually runs against its line's x1→x2; a set where none does would
    // pass it with the reversal broken.
    check('some bond in these molecules is drawn from its end, so the reversal is exercised', drawnBack > 0, `${drawnBack}`);
    const css = readFileSync(fileURLToPath(new URL('../src/index.css', import.meta.url)), 'utf8');
    const back = /@keyframes vb-edge-draw-back\s*\{([\s\S]*?)\n\}/.exec(css)?.[1] ?? '';
    check('a stroke drawn from its end runs stroke-dashoffset -1 → 0', /stroke-dashoffset:\s*-1/.test(back) && /stroke-dashoffset:\s*0/.test(back));
    check('…and the stylesheet keys that animation on draw-back', /\[data-vb-arrive="draw-back"\][^{]*\{[^}]*vb-edge-draw-back/.test(css));
}

console.log('\n--- a molecule with no graph on it: bonds left to right, then the atom labels ---');
{
