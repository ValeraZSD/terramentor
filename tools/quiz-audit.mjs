 ...
 *
 * Why this is needed on top of the generation-time check in `vetQuiz`: the mastery
 * check REUSES saved quizzes. `MasteryGateModal` only generates when a node has
 ...
 * frequency?" is answered equally by 515 Hz and 525 Hz, and a live mastery check
...
function normalizeRepaired(candidate, original) {
    ...
    // A repaired question is the SAME question, fixed: its identity (and so its
    // place in the ask-record) is the original's, whatever the model echoed.
    if (original.uuid) out.uuid = original.uuid;
