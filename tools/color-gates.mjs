...
//
// Two colour pickers can each look fine on their own and still disagree (a
// tick, a value field, a focus ring, a `ring-offset` whose gap is painted — a
// pale halo on Warm and on Black). No per-element check sees a disagreement,
// so half of this file is sameness, like control-gates.
//
// The other half is what a per-element check cannot see at all: a project's
...
// AND the dialog shows the result. Measured, not scanned: a source scan misses
// a divergence between two implementations, so these run the real functions
// over the real palettes.
//
...
/**
 * The same file with its prose removed: a scanner that reads the comments ends
 * up grading the argument for why `grid-cols-4 sm:` and `ring-offset` are
 * wrong, not the code.
 */
...
// drift from the arithmetic that ships.
...
// white by default), which is wrong on three of the four themes, so the
// selection mark is an `outline`, whose offset gap is transparent.
...
//
// "The class is `grid-cols-8` and nothing else" would hold only while the
// count is fixed. `ColorField` has a `dense` mode (half-width chips for the
// app icon's tile palette: sixteen across where there is room, eight where
// there is not), and a count that CHANGES is fine; the fault this guards
// against is a count that changes with the VIEWPORT. So: no responsive grid
// variant anywhere, and a count that is not the constant has to come from a
// measurement of the component's own box.
for (const rel of ['src/components/ui/ColorField.tsx', 'src/components/IconPicker.tsx']) {
    const src = code(rel);
...
//
// The chips DO take the slack now — eight fixed squares left 180-odd px of a
// 736px settings card empty, three times over, once per colour row. But they
// take it between a floor and a CEILING, which is the whole difference from
// the stripe this rule was written about: an unbounded `1fr` column takes
// whatever it is given, and sixteen swatches across a settings card with the
// value they belong to on a line underneath is what that looked like.
//
...

{
    ...
    ok('the chips grow, but between a floor and a ceiling',
        /minmax\(\$\{touch \? CHIP_TOUCH_REM : CHIP_REM\}rem, 1fr\)/.test(src)
        && /maxWidth:\s*`\$\{groupRem\(CHIP_MAX_REM\)\}rem`/.test(src),
        'an unbounded 1fr column beside the field is the stripe again');
    ok('a grown chip is still a square',
        /beside \? 'w-full aspect-square'/.test(src),
        'width without height turns eight swatches into eight bricks');
    // The palette group is the only thing in the row that grows. Two `flex-1`
    // children split the free space in the ratio of their grow factors — not
    // "fill the first, then the second" — which gave the group 272px to hold
    // 298px of chips: the tracks overflowed and the eyedropper was drawn on top
    // of the fifteenth swatch. Measured in Edge on a 736px card, 2026-09-21.
    ok('the value field does not compete with the chips for the slack',
        /shrink-0 basis-44/.test(src) && !/flex-1 basis-44/.test(src),
        'a growing value field is how the row ended before the card did');
    ok('…and the palette group carries a real floor, not just min-w-0',
        /minWidth:\s*`\$\{groupRem\(touch \? CHIP_TOUCH_REM : CHIP_REM\)\}rem`/.test(src),
        'min-w-0 lets a flex child be squeezed under its own content');
...
// A PROJECT swatch is darkened first — and the result must be a real colour,
// never a near-white: a pale input can start outside the band, and painting a
// white chip as chosen is a white button with a white label.
...
// enormous for anything pale: white moves 54. 25 points is the line, with
