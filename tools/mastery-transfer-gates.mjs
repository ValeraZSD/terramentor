...
const THRESHOLD = 0.85, BOSS_PASS = 0.8;
...
let el = checkMasteryEligibility(nNew, THRESHOLD, BOSS_PASS);
...
db.prepare('UPDATE node_mastery SET mastery_score = 0.97 WHERE node_id = ?').run(nNew);
el = checkMasteryEligibility(nNew, THRESHOLD, BOSS_PASS);
...
updateMasteryFromAttempt(nNew, 5, 6, 'boss_fight', { source: 'test' });
el = checkMasteryEligibility(nNew, THRESHOLD, BOSS_PASS);
...
el = checkMasteryEligibility(nBkt, THRESHOLD, BOSS_PASS);
...
el = checkMasteryEligibility(nPlain, THRESHOLD, BOSS_PASS);
