...
//
// The export button used to be one click that silently shipped the learner's
// private notes. It is now a two-step: open options, then export. That is new
// UI, and new UI in this project gets looked at rather than assumed — the
// browser pane is not displaying here, so this is the jsdom route instead
// (behaviour, never pixels).
//
