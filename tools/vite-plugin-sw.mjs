...
//
// A static `public/sw.js` ships byte-identical worker source on every build. A
// browser decides "is there a new worker?" by comparing those bytes, so it
// would never see one, the update prompt could not fire, and a single fixed
// cache name would accumulate every build's assets with nothing to tell them
// apart. See the long note at the top of `src/sw-template.js` for what a
// fixed cache name does to a slow launch.
//
