...
//
// Advisory, and not part of `npm test`: it reports, it does not gate. A dev aid,
// to run after a frontend
...
// ---------------------------------------------------------------------------
/**
 * One `className={…}` expression, split into the sets of classes that can
 * actually be on the element AT THE SAME TIME.
 *
 * A ternary's two arms never co-occur, and merging them invents pairs that
 * cannot exist. `DrillPlayer.tsx`'s Done button is the case that found this:
 * `missed.length === 0 ? 'bg-accent text-white …' : '… dark:text-slate-200 …'`
 * was read as one element and reported as slate-200 on the accent at 4.35:1 —
 * a WCAG AA failure the app has never been able to draw. `cx('a', c && 'b')` is
 * the opposite and must keep merging, since there the classes really do land
 * together, so only `?`/`:` splits.
 *
 * Nesting is handled by splitting each arm again, capped: past `MAX_VARIANTS`
 * the expression is complicated enough that the union is the safer answer — an
 * over-report is a nuisance, and a missed pair is a contrast bug that ships.
 */
const MAX_VARIANTS = 16;

function ternaryVariants(blob, depth = 0) {
  if (depth > 3) return [blob];
  // A top-level `?` — not one inside a string, a nested call or an optional
  // chain (`a?.b`), and not the `?` of a `??`.
  let q = null, paren = 0, brace = 0, bracket = 0, qmark = -1;
  for (let i = 0; i < blob.length; i++) {
    ...
    if (ch === '(') paren++; else if (ch === ')') paren--;
      ...
      && blob[i + 1] !== '.' && blob[i + 1] !== '?' && blob[i - 1] !== '?') { qmark = i; break; }
  }
  if (qmark === -1) return [blob];

  // The matching `:` for that `?`, skipping any nested ternary's own colon.
  let nested = 0;
  let colon = -1;
  q = null; paren = 0; brace = 0; bracket = 0;
  for (let i = qmark + 1; i < blob.length; i++) {
    const ch = blob[i];
    if (q) { if (ch === '\\') i++; else if (ch === q) q = null; continue; }
    if (ch === '"' || ch === "'" || ch === '`') { q = ch; continue; }
    if (ch === '(') paren++; else if (ch === ')') paren--;
    ...
    else if (!paren && !brace && !bracket) {
      if (ch === '?' && blob[i + 1] !== '.' && blob[i + 1] !== '?') nested++;
      else if (ch === ':') { if (nested) nested--; else { colon = i; break; } }
    }
  }
  if (colon === -1) return [blob];

  // The condition before `?` is a JS expression, not classes — its identifiers
  // fail `classify()` anyway — so each arm stands alone.
  const arms = [
    ...ternaryVariants(blob.slice(qmark + 1, colon), depth + 1),
    ...ternaryVariants(blob.slice(colon + 1), depth + 1),
  ];
  return arms.length > MAX_VARIANTS ? [blob] : arms;
}
...
function extractScopes(text) {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    for (const variant of ternaryVariants(blob)) {
      // pull class-like tokens (letters/digits/-/ /: and %), drop ${...} noise
      const tokens = variant.replace(/\$\{[^}]*\}/g, ' ').match(/[a-zA-Z][\w:/.\-\[\]]*/g) || [];
