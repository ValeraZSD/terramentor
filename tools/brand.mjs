...
/**
 * PIXEL COUNTS at the two ends of the tone range — solid ink and clean paper,
 * with everything between them deliberately uncounted. Comparing two renders
 * band by band says what a stroke DID: paper it took, ink it laid down, and by
 * subtraction the grey it left instead.
 */
async function bands(source, size) {
    const { ctx } = await rasterize(source, size);
    const { data } = ctx.getImageData(0, 0, size, size);
    let ink = 0, paper = 0;
    for (let i = 0; i < data.length; i += 4) {
        if (data[i + 3] <= 24) continue;
        const lum = 0.2126 * data[i] + 0.7152 * data[i + 1] + 0.0722 * data[i + 2];
        if (lum < 32) ink++; else if (lum >= 224) paper++;
    }
    return { ink, paper };
}
...
async function checks() {
    ...
    //    WHAT SHARE OF THE GRID ACTUALLY RESOLVES — not how much ink it adds.
    //
    //    How much ink the grid adds is a PROXY for the blot, and a proxy that
    //    the artwork can invert: the amount of grid a reader can see depends on
    //    how much of the globe the book covers, so "the grid adds proportionally
    //    more dark pixels at 16px than at 64px" reads +21% against +23% at the
    //    placement this mark actually ships, and fails — while the thing it
    //    stands for is as true as ever. Measure the claim itself.
    //
    //    Measured band by band, the grid takes 13 of the 16px picture's 256
    //    pixels off the paper and turns 6 of them into ink — the other 7 come
    //    back as grey. At 64px it takes 193 and inks 127. So a grid line at 16px
    //    is a smudge slightly more than half the time and a line two-thirds of
    //    the time at 64px, which is the whole reason there are two cuts. The
    //    counts are small at 16px and printed for that reason: a one-pixel
    //    difference is eight points, so the margin is deliberately wide and a
    //    near-miss here means look at the picture, not at the threshold.
    const gb = async (n) => bands(svgDoc(markArt({})), n);
    const nb = async (n) => bands(svgDoc(markArt({ grid: false })), n);
    const [gb16, nb16, gb64, nb64] = [await gb(16), await nb(16), await gb(64), await nb(64)];
    const resolved = (g, n) => (g.ink - n.ink) / Math.max(1, n.paper - g.paper);
    const [r16, r64] = [resolved(gb16, nb16), resolved(gb64, nb64)];
    ok('a grid line resolves at 64px and smudges at 16px',
        r64 - r16 > 0.1,
        `${(r16 * 100).toFixed(0)}% of the ${nb16.paper - gb16.paper} pixels it touches at 16px become ink, `
        + `against ${(r64 * 100).toFixed(0)}% of ${nb64.paper - gb64.paper} at 64px`);
