...
{
    // Cards that arrive carrying Anki history are stamped as reviewed at the
    // moment of import; counted as introduced that day, they would spend the
    // allowance on study that happened months ago elsewhere.
    ...
    // born today too, so a clause keyed on `created_at` cannot see them and the
    // deck would keep offering the full twenty after every session.
