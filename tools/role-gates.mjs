...
//
// The reserved slot goes to "this project has cards and no schedule", the
// property that actually justifies it: a collection that is never scheduled
// can never be overdue, so without a reserved slot it is invisible on every
// day that has real work.
