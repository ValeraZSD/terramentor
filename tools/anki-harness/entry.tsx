// Mounts the REAL AnkiImportModal so the redesigned preview step is exercised
// as a component rather than read as source.
...
import AnkiImportModal from '../../src/components/AnkiImportModal';

const act: any = (React as any).act || domAct;
(globalThis as any).__act = (fn: () => void) => act(fn);
(globalThis as any).__mount = (el: HTMLElement) => {
    const root = createRoot(el);
    act(() => { root.render(React.createElement(AnkiImportModal as any, { open: true, onClose: () => { } })); });
    return root;
};
