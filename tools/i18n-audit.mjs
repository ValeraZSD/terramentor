 ...
 *   node tools/i18n-audit.mjs nl de --dump temp/x.jsonl
 *                                        the findings, untruncated, one JSON
 *                                        object per line — the judgement-pass
 *                                        input; --list truncates at 80 chars
 *
...
for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '--dump') dumpPath = argv[++i] ?? 'i18n-audit-dump.jsonl';
    else if (!argv[i].startsWith('--')) only.push(argv[i]);
}
/** Full findings for --dump: {loc, col, key, src, value, …extra}. The printed
 * list truncates key and value, which is enough to count a column and not
 * enough to judge one — the whole point of the judgement pass is reading the
 * whole sentence. */
const dumpRows = [];
...
/** é → e: a stem test that survives the accent the English never had ("resúmenes" holds "resum" only once folded). */
 ...
 * pronoun; Polish sam/sama and Spanish listo/lista now wait the same way for
 * the verb beside them that makes them about the reader: "sam wskażesz",
 * "estás listo"), and French seul (it describes the drawing, not the reader).
 */
const GENDERED = {
    ...
    pl: [
        ...
        word('samemu|gotowy|gotowa|pewien|pewna|powinieneś|powinnaś|zadowolony|zadowolona|ciekaw|ciekawa|zmęczony|zmęczona|zalogowany|zalogowana|połączony|połączona'),
        // sam/sama on its own is "the same" (ta sama) or part of samochód more
        // often than "myself"; the 2sg verb after it is what makes it "you,
        // yourself" ("sam wskażesz"), and a Polish 2sg verb ends in -sz.
        word('sam[ao]?\\s+\\p{L}+sz'),
    ],
    es: [
        word('seguro|segura|preparado|preparada|bienvenido|bienvenida|honesto|honesta|sincero|sincera|conectado|conectada|registrado|registrada|cansado|cansada|dispuesto|dispuesta'),
        word('(?:yo|tú|ti|sí|usted)\\s+mism[oa]'),
        // listo/lista on its own is usually the NOUN lista ("la lista de
        // Recursos"); it is the reader's state only with estar.
        word('(?:estás|estoy)\\s+list[oa]'),
    ],
    pt: [
...
for (const file of readdirSync(localesDir).filter((f) => f.endsWith('.json'))) {
    ...
    for (const [norm, entries] of byValue) {
        ...
        if (distinct.length > 1) {
            findings.collision.push([distinct.join(' / '), norm]);
            dumpRows.push({ loc: code, col: 'collision', src: distinct.join(' / '), value: norm });
        }
    }
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        if (gendered && readerScope(code).test(src)) {
            ...
            if (hit) {
                findings.gender.push([key, hit[0]]);
                dumpRows.push({ loc: code, col: 'gender', key, src, value, hit: hit[0] });
            }
        }

        // Identical to the English, and long enough that it should not be.
        if (value.trim() === src.trim() && src.split(/\s+/).length >= 3) {
            findings.untranslated.push([key, value]);
            dumpRows.push({ loc: code, col: 'untranslated', key, src, value });
        }
        ...
        if (script && script.test(value)) {
            ...
            if (runs.length) {
                findings.latin.push([key, runs[0]]);
                dumpRows.push({ loc: code, col: 'latin', key, src, value, run: runs[0] });
            }
        }

        // A control sized for the English cannot hold this.
        if (src.length >= 12 && value.length > src.length * 2 && !/[.!?]$/.test(src)) {
            findings.long.push([key, `${src.length} to ${value.length}`]);
            dumpRows.push({ loc: code, col: 'long', key, src, value });
        }
        ...
        if (shape.length) {
            findings.shape.push([key, shape.join(', ')]);
            dumpRows.push({ loc: code, col: 'shape', key, src, value, faults: shape });
        }
    }
    ...
    for (const term of GLOSSARY) {
        ...
        for (const [key, value] of Object.entries(data)) {
            ...
            //
            // But the term is matched against the English with its
            // {{placeholders}} and dotted paths blanked, and a JSON sample key
            // is skipped whole: "in {{project}}" and "{ \"project\": … }" name
            // no product noun — eight findings in four locales were the
            // placeholder, not the prose.
            if (/^\s*\{/.test(src)) continue;
            const srcBare = src.replace(/\{\{[^{}]*\}\}/g, ' ').replace(/\b\w+\.\w+/g, ' ');
            if (!re.test(srcBare)) continue;
            const valBare = value.replace(/\{\{[^{}]*\}\}/g, ' ');
            if (fold(valBare).toLowerCase().includes(stemFold)) continue;
            ...
            if (re.test(valBare)) {
                findings.english.push([key, `${term} is "${canonical}" here: ${value}`]);
                dumpRows.push({ loc: code, col: 'english', key, src, value, term, canonical });
            } else {
                findings.renamed.push([key, `${term} is "${canonical}" here: ${value}`]);
                dumpRows.push({ loc: code, col: 'renamed', key, src, value, term, canonical });
            }
        }
    }
...
if (dumpPath) {
    writeFileSync(dumpPath, dumpRows.map((r) => JSON.stringify(r)).join('\n') + '\n');
    console.log(`dump: ${dumpRows.length} findings → ${dumpPath}`);
}
