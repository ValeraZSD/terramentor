...
if (coursePaths) {
    ...
    // Four of the eight nodes: the note and the pagination slice are not on
    // the map, and the root and "Wave patterns" are section headings — drawn
    // and journeyed through, but not topics the course counts (LEAF_NODE).
    check('a course counts its mapped TOPICS, not its headings', courses.find(c => c.id === course)?.topics === 4,
        String(courses.find(c => c.id === course)?.topics));
    check('a course counts only the finishes it can place',
        courses.find(c => c.id === course)?.finished === 1,
        String(courses.find(c => c.id === course)?.finished));
    check('the payload marks a heading as no leaf', topicById.get(cMid)?.leaf === false && topicById.get(cDeep)?.leaf === true);
    ...
    // ---- the reader's two dials ---------------------------------------------
    // Speed and dwell are separate numbers because they answer separate
    // complaints — one is "this is too long to sit through", the other is "I
    // cannot read the names". Tied together, a reader who wants to read the
    // names also gets an arrow crawling between them.
    //
    // The property that matters most is the FIRST one: at the shipped default
    // every beat here is the beat that was measured, to the bit. The dials were
    // threaded through arithmetic that half the atlas reads, and a default that
    // is a no-op only by luck is a replay that quietly changed for everyone.
    const { DEFAULT_PACE, flightMs, flightShare, SPEED_MIN, SPEED_MAX } = coursePaths;
    const { FLIGHT_SHARE: SHARE } = coursePaths;
    const mid = fakeTrace([0, 0], [0.3, 0]);
    const beat = stepDurationMs(mid, 2);
    check('the default pace is exactly the beat that was measured',
        stepDurationMs(mid, 2, DEFAULT_PACE) === beat
        && Math.abs(flightShare(mid, 2, DEFAULT_PACE) - SHARE) < 1e-12);
    check('twice the speed is half the beat',
        Math.abs(stepDurationMs(mid, 2, { speed: 2, holdMs: 0 }) - beat / 2) < 1e-9);
    check('half the speed is twice the flight too',
        Math.abs(flightMs(mid, 2, { speed: 0.5, holdMs: 0 }) - flightMs(mid, 2) * 2) < 1e-9);
    // A dwell buys time AT an arrival and nowhere else. If it shortened the
    // flight instead, "let me read the names" would make the arrow faster.
    check('a dwell is added to the beat and taken out of nothing',
        Math.abs(stepDurationMs(mid, 2, { speed: 1, holdMs: 500 }) - (beat + 500)) < 1e-9
        && flightMs(mid, 2, { speed: 1, holdMs: 500 }) === flightMs(mid, 2));
    check('...so the flying share falls as the dwell grows',
        flightShare(mid, 2, { speed: 1, holdMs: 500 }) < SHARE
        && flightShare(mid, 2, { speed: 1, holdMs: 500 }) > 0);
    check('the two dials do not interfere',
        Math.abs(stepDurationMs(mid, 2, { speed: 2, holdMs: 400 }) - (beat / 2 + 400)) < 1e-9);
    // A pace is read out of a database a different build wrote, so it is not a
    // control this code can see. Every one of these used to divide the beat
    // into an arrow that never lands — which on screen is a replay that has hung.
    check('a broken pace still lands the arrow', [
        { speed: 0, holdMs: 0 }, { speed: NaN, holdMs: NaN }, { speed: -3, holdMs: -100 },
        { speed: 1e9, holdMs: 1e9 }, {},
    ].every(p => {
        const ms = stepDurationMs(mid, 2, p);
        const fly = flightMs(mid, 2, p);
        return Number.isFinite(ms) && ms > 0 && Number.isFinite(fly) && fly > 0 && fly <= ms;
    }));
    // ---- the head changes shape at each end of a leg, never on one frame ----
    // A dot on the ground, an arrow in the air: swapped on the frame it took
    // off and the frame it landed, that was a blink at every single step.
    {
        const { headMorph, HEAD_MORPH_MS } = coursePaths;
        check('the head is all dot at take-off and at landing',
            headMorph(0, 900) === 0 && headMorph(1, 900) === 0 && headMorph(1e-6, 900) < 0.01 && headMorph(1 - 1e-6, 900) < 0.01);
        check('...and all arrow in the middle of a leg', headMorph(0.5, 900) === 1);
        check('the cross-over is timed in ms, not as a share of the leg',
            headMorph(HEAD_MORPH_MS / 2 / 3000, 3000) === headMorph(HEAD_MORPH_MS / 2 / 1500, 1500));
        check('a short hop is still an arrow somewhere', headMorph(0.5, 120) === 1);
        let prev = 0, jump = 0;
        for (let i = 1; i <= 400; i++) { const v = headMorph(i / 400, 900); jump = Math.max(jump, Math.abs(v - prev)); prev = v; }
        check('...and the change is gradual', jump < 0.05, jump.toFixed(3));
    }
    // ---- a jump fades what it replaced ------------------------------------
    // A restart took a finished route back to step one on a single frame; the
    // flight now remembers where it was so both surfaces can fade the old
    // picture out instead.
    {
        const { advanceFlight, newFlight, CUT_FADE_MS } = coursePaths;
        const t = { ...fakeTrace([0, 0], [0.3, 0], [0.6, 0], [0.9, 0]), projectId: 7 };
        const f = newFlight();
        const firstLook = advanceFlight(f, t, null, 16, false);
        check('a course just picked has nothing to fade', firstLook.cut === null);
        const restarted = advanceFlight(f, t, 1, 16, false);
        check('a restart from the finished path fades it', restarted.cut?.from === 4 && restarted.cut.p === 0 && restarted.landed === 1);
        let last = restarted;
        for (let ms = 0; ms < CUT_FADE_MS + 40; ms += 16) last = advanceFlight(f, t, 1, 16, false);
        check('...and the fade ends', last.cut === null);
        const g = newFlight();
        advanceFlight(g, t, null, 16, false);
        check('a reader who asked for less motion gets a cut, not a fade',
            advanceFlight(g, t, 1, 16, true).cut === null);
        const h = newFlight();
        advanceFlight(h, t, null, 16, false);
        advanceFlight(h, { ...t, points: new Map(t.points) }, null, 16, false);
        check('the same course fetched again is not a new course',
            h.target === 4 && advanceFlight(h, t, 1, 16, false).cut?.from === 4);
        const k = newFlight();
        advanceFlight(k, t, null, 16, false);
        check('another course is a new flight, not a fade',
            advanceFlight(k, { ...t, projectId: 8 }, 1, 16, false).cut === null);
    }
    check('the dials are clamped to the ends of their own sliders',
        stepDurationMs(mid, 2, { speed: 100, holdMs: 0 }) === stepDurationMs(mid, 2, { speed: SPEED_MAX, holdMs: 0 })
        && stepDurationMs(mid, 2, { speed: 0.001, holdMs: 0 }) === stepDurationMs(mid, 2, { speed: SPEED_MIN, holdMs: 0 }));
    ...
    // The page's own clock — which finish has been reached and when the next
    // one is — moved out of `AtlasView` when a second surface started replaying
    // courses: the render dialog films one, and a hand-written second copy of
    // these beats is a video that runs at a different speed than the map it is
    // a video of. The scans below read it where it lives.
    const clock = readFileSync(new URL('../src/components/atlas/useJourneyReplay.ts', import.meta.url), 'utf8');
    const stage = readFileSync(new URL('../src/components/atlas/AtlasRenderStage.tsx', import.meta.url), 'utf8');
    ...
    check('both clocks read the same beat', /stepDurationMs\(/.test(clock) && /stepDurationMs\(/.test(paths));
    check('...and neither keeps a beat of its own',
        !/REPLAY_STEP_MS/.test(view) && !/REPLAY_STEP_MS/.test(map));
    // ONE clock, used by both places a course replays. The page and the render
    // dialog each hold a surface and a course; neither may arm a timeout of its
    // own, or the film and the map are two replays that agree by coincidence.
    check('the page and the render both drive the one clock',
        /useJourneyReplay\(/.test(view) && /useJourneyReplay\(/.test(stage)
        && !/stepDurationMs\(/.test(view));
    ...
    check('the page waits the CURRENT leg\'s beat',
        /stepDurationMs\(trace, Math\.max\(1, at\), pace\)/.test(clock)
        && !/stepDurationMs\(trace, at \+ 1/.test(clock));
    check('a late arrow hurries instead of teleporting',
        /const behind = Math\.max\(1, want - flight\.at\)/.test(paths)
        && /behind \/ legFly/.test(paths));
    ...
    check('carrying on takes the next step at once, not after a beat',
        /s >= trace\.journey\.length \? 1 : s \+ 1/.test(clock));
}
...
if (colors) {
    ...
    // The shipped accents, plus deep violet, which a stored setting can still name.
...
// The replay's pace, the region names and the video's shape are stored as JSON
// in ONE settings row, which means every one of them arrives as text a
// different build may have written. A half-understood row must not produce a
// map that cannot be drawn: an unknown aspect has to fall back the way an
// unknown `atlas_surface` does, an absent switch has to be its default rather
// than `false`, and a frame has to come out at a size an encoder accepts.
let viewer = null;
try {
    const { build } = await import('esbuild');
    const outfile = join(scratch, 'atlasSettings.mjs');
    await build({
        entryPoints: [fileURLToPath(new URL('../src/components/atlas/atlasSettings.ts', import.meta.url))],
        bundle: true, format: 'esm', outfile, logLevel: 'silent',
    });
    viewer = await import(pathToFileURL(outfile).href);
} catch (err) {
    console.log(`SKIPPED: viewer settings — esbuild unavailable (${err.message.split('\n')[0]})`);
}

if (viewer) {
    const {
        normalizeAtlasSettings, DEFAULT_ATLAS_SETTINGS, isDefaultAtlasSettings,
        frameSize, ASPECTS, FPS_CHOICES, RESOLUTIONS, paceOf,
    } = viewer;
    const same = (a, b) => Object.keys(DEFAULT_ATLAS_SETTINGS).every(k => a[k] === b[k]);

    check('nothing stored is the shipped default',
        same(normalizeAtlasSettings(null), DEFAULT_ATLAS_SETTINGS)
        && same(normalizeAtlasSettings(undefined), DEFAULT_ATLAS_SETTINGS));
    check('so is a row that is not JSON at all',
        same(normalizeAtlasSettings('{ half a row'), DEFAULT_ATLAS_SETTINGS)
        && same(normalizeAtlasSettings('7'), DEFAULT_ATLAS_SETTINGS));
    // A build from the future knowing a fourth aspect and a 4K size must leave
    // this one with something it can draw, field by field — not with defaults
    // for the whole row, which would throw away the settings it DID understand.
    check('a value this build does not know falls back, and only it', (() => {
        const s = normalizeAtlasSettings(JSON.stringify({
            aspect: '21:9', resolution: 4320, fps: 7, speed: 1.75, holdMs: 750, regionNames: false,
        }));
        return s.aspect === DEFAULT_ATLAS_SETTINGS.aspect
            && s.resolution === DEFAULT_ATLAS_SETTINGS.resolution
            && s.fps === DEFAULT_ATLAS_SETTINGS.fps
            && s.speed === 1.75 && s.holdMs === 750 && s.regionNames === false;
    })());
    // `Number(null)` is 0, not NaN — which is what shipped a square app icon
    // once. An absent switch is its DEFAULT, and the default here is on.
    check('an absent switch is on, a stored false is off',
        normalizeAtlasSettings('{}').regionNames === true
        && normalizeAtlasSettings('{"regionNames":false}').regionNames === false);
    check('a speed that would hang the replay cannot be stored', (() => {
        const bad = ['{"speed":0}', '{"speed":-4}', '{"speed":"fast"}', '{"speed":1e9}'];
        return bad.every(raw => {
            const s = normalizeAtlasSettings(raw);
            return Number.isFinite(s.speed) && s.speed > 0
                && Number.isFinite(paceOf(s).speed) && paceOf(s).holdMs >= 0;
        });
    })());
    check('the dwell is a whole number of milliseconds inside its range', (() => {
        const s = normalizeAtlasSettings('{"holdMs":123456.7}');
        return Number.isInteger(s.holdMs) && s.holdMs > 0 && s.holdMs <= 4000;
    })());
    check('"is this the default" is asked of every field',
        isDefaultAtlasSettings(DEFAULT_ATLAS_SETTINGS)
        && Object.keys(DEFAULT_ATLAS_SETTINGS).every(k => !isDefaultAtlasSettings({
            ...DEFAULT_ATLAS_SETTINGS,
            [k]: typeof DEFAULT_ATLAS_SETTINGS[k] === 'boolean'
                ? !DEFAULT_ATLAS_SETTINGS[k] : DEFAULT_ATLAS_SETTINGS[k] + 1,
        })));

    // ---- the frame ----------------------------------------------------------
    // Every block-based encoder behind MediaRecorder works in macroblocks, and
    // an odd dimension is either refused or quietly rounded — which produces a
    // file whose size is not the size that was asked for.
    check('every offered frame has even edges', ASPECTS.every(a => RESOLUTIONS.every(r => {
        const f = frameSize(a.id, r);
        return f.width % 2 === 0 && f.height % 2 === 0 && f.width >= 2 && f.height >= 2;
    })));
    check('every frame is the shape it says it is', ASPECTS.every(a => RESOLUTIONS.every(r => {
        const f = frameSize(a.id, r);
        // Within a pixel of the ratio, which is all that rounding to even can
        // promise.
        return Math.abs(f.width / f.height - a.w / a.h) < 0.01;
    })));
    // Every option is its OWN WORKED EXAMPLE, not a formula restated: "the
    // resolution is an edge" is exactly the sentence that shipped a 16:9 "1080p"
    // of 1080×608 — a real frame, a valid file, and not the one anybody asked
    // for. The rule 1080p carries everywhere else is that 1080 is the SHORT
    // edge, and the only way to pin that is to write down what each button must
    // produce.
    check('1080p is the frame everyone means by 1080p', (() => {
        const shown = (a, r) => { const f = frameSize(a, r); return `${f.width}x${f.height}`; };
        return shown('16:9', 1080) === '1920x1080'
            && shown('16:9', 720) === '1280x720'
            && shown('9:16', 1080) === '1080x1920'
            && shown('9:16', 720) === '720x1280'
            && shown('1:1', 1080) === '1080x1080'
            && shown('1:1', 720) === '720x720';
    })());
    check('an aspect nobody offers still draws a frame', (() => {
        const f = frameSize('nonsense', 1080);
        return f.width > 0 && f.height > 0 && f.width % 2 === 0 && f.height % 2 === 0;
    })());
    check('the fps on offer are the ones a reader can pick',
        FPS_CHOICES.length > 0 && FPS_CHOICES.every(f => Number.isInteger(f) && f > 0));

    // ---- the GIF --------------------------------------------------------------
    // A GIF has its own frame and rate, whatever the video's controls say —
    // they are hidden while GIF is picked, so they must not reach it either.
    const { outputFrame, outputFps, GIF_RESOLUTION, GIF_FPS } = viewer;
    check('an output this build does not know is a video',
        normalizeAtlasSettings('{"output":"apng"}').output === 'video'
        && normalizeAtlasSettings('{"output":"gif"}').output === 'gif');
    check('a GIF is drawn at its own frame, whatever the video size says', (() => {
        const g = outputFrame({ ...DEFAULT_ATLAS_SETTINGS, output: 'gif', resolution: 1080, aspect: '16:9' });
        const v = outputFrame({ ...DEFAULT_ATLAS_SETTINGS, output: 'video', resolution: 1080, aspect: '16:9' });
        return `${g.width}x${g.height}` === '712x400' && GIF_RESOLUTION === 400 && v.width === 1920
            && outputFps({ ...DEFAULT_ATLAS_SETTINGS, output: 'gif', fps: 60 }) === GIF_FPS
            && outputFps({ ...DEFAULT_ATLAS_SETTINGS, output: 'video', fps: 60 }) === 60;
    })());
}
...
try {
    const { build } = await import('esbuild');
    const outfile = join(scratch, 'gifStream.mjs');
    await build({
        entryPoints: [fileURLToPath(new URL('../src/components/atlas/gifStream.ts', import.meta.url))],
        bundle: true, format: 'esm', outfile, logLevel: 'silent',
    });
    gifMod = await import(pathToFileURL(outfile).href);
} catch (err) {
    console.log(`SKIPPED: GIF stream — esbuild unavailable (${err.message.split('\n')[0]})`);
}

if (gifMod) {
    // The file is walked block by block, never scanned for byte patterns — LZW
    // data can contain any byte sequence, including a fake extension header.
    const walkGif = (b) => {
        ...
        let delay = 0;
        let localTables = 0;
        const skipSub = () => { while (b[p]) p += b[p] + 1; p++; };
        while (p < b.length && b[p] !== 0x3b) {
            if (b[p] === 0x21) {
                ...
                const packed = b[p + 9];
                p += 10;
                if (packed & 0x80) { localTables++; p += 3 * (1 << ((packed & 7) + 1)); }
                p++; skipSub();
                frames.push(delay);
            } else return null;
        }
        return { head, frames, localTables, trailer: b[p] === 0x3b };
    };
    const W = 64, H = 36;
    const paint = (fn) => {
    ...
    // The opening shot: two regions and a line.
    const ground = (x, y) => (y === 18 ? [240, 240, 240] : x < 32 ? [30, 60, 120] : [40, 110, 70]);
    const s = gifMod.createGifStream(W, H);
    for (let i = 0; i < 3; i++) s.push(paint(ground), 67);           // a still landing
    s.push(paint((x, y) => (x > 40 && x < 60 && y < 16 ? [240, 240, 240] : ground(x, y))), 67);
    // Something the opening palette never saw, over a big enough area to count.
    s.push(paint((x, y) => (x < 20 && y < 20 ? [230, 30, 200] : ground(x, y))), 67);
    const out = walkGif(s.finish());
    check('a GIF stream writes a whole, well-formed file',
        !!out && out.head === 'GIF89a' && out.trailer);
    check('a frame identical to the one before stretches it rather than being written again',
        !!out && out.frames.length === 3 && s.written === 3);
    // 3 × 67 ms = 201 ms → 20 cs; the carried remainder keeps the total honest.
    check('the merged frame lasts as long as the frames it replaced',
        !!out && out.frames[0] === 20
        && Math.abs(out.frames.reduce((a, d) => a + d, 0) * 10 - 5 * 67) <= 10);
    check('a colour the opening shot never had re-learns the palette once, and only from there on',
        s.relearns === 1 && !!out && out.localTables === 1);
}
if (viewer) {

    // ---- the surfaces obey the switch ---------------------------------------
    // Both of them, from the same flag, and by SKIPPING the pass rather than
    // painting it at zero alpha: measuring and wrapping every visible region's
    // name every frame is the cost a reader who switched them off asked not to
    // pay. The selected region keeps its name either way — a ring round an
    // anonymous blob is not a selection.
    const mapSrc = readFileSync(new URL('../src/components/atlas/AtlasMap.tsx', import.meta.url), 'utf8');
    const globeSrc = readFileSync(new URL('../src/components/atlas/GlobeMap.tsx', import.meta.url), 'utf8');
    check('both surfaces gate the region names on one flag',
        /if \(!showNames && region\.id !== selected\) continue;/.test(mapSrc)
        && /if \(!showNames && p\.region\.id !== selected\) continue;/.test(globeSrc));
    check('both surfaces take the reader\'s pace into the integrator',
        /advanceFlight\(flightRef\.current, tracing, stepNow, frameMs, reduceMotion, paceNow\)/.test(mapSrc)
        && /advanceFlight\(flightRef\.current, tracing, stepNow, frameMs, reduceMotion, paceNow\)/.test(globeSrc));
    // A `MediaRecorder` takes the track's dimensions at START and never
    // revisits them, so a canvas sized off its own box would film a video whose
    // size is whatever the dialog had room for.
    check('a captured canvas is sized to the FRAME, not to its box',
        /canvas\.width = capture \? capture\.width/.test(mapSrc)
        && /canvas\.width = capture \? capture\.width/.test(globeSrc));
    // One value for the scale, written by the sizing observer and read by the
    // draw: two computations of "how many canvas pixels is a CSS pixel" is a
    // map painted at a different scale than the one it was sized at.
    check('the draw scales by the ratio the canvas was sized with',
        /const dpr = dprRef\.current;/.test(mapSrc) && /const dpr = dprRef\.current;/.test(globeSrc)
        && !/const dpr = Math\.min\(2, window\.devicePixelRatio/.test(mapSrc)
        && !/const dpr = Math\.min\(2, window\.devicePixelRatio/.test(globeSrc));
}
