...
// --- the study surface has the FEED'S shape, not the workspace's -------------
//
// Study is the workspace tab that TEACHES, and what surrounded it was
// administrative furniture: the course's nav tree down the left of every
// lesson, and a sticky header repeating the project's name (already in the app
// header one row above) over a Back button that went to the Dashboard tab and
// a "Topics" button that went where the Tree tab goes.
//
// Both rules are read off the source because both are JSX structure: there is
// no value to assert, only "is this element still conditional". Each predicate
// is re-run against the shape it was written for — a gate that passes because
// it cannot see the fault is worse than no gate.
{
    // Named per MOUNT rather than by walking backwards from each `<Sidebar`:
    // the drawer's is ~700 characters below its guard, and a window wide
    // enough to reach it is also wide enough to be satisfied by the OTHER
    // mount's guard — which would pass a tree the drawer still draws. The
    // count is what stops a third mount arriving unguarded.
    const sidebarIsStudyFree = (src) =>
        /const studyView = workspaceView === 'study'/.test(src)
        && /!isMobile && !studyView && <Sidebar \/>/.test(src)
        && /\{isMobile && !studyView && \(/.test(src)
        && [...src.matchAll(/<Sidebar\b/g)].length === 2;
    const wsSrc = readFileSync(new URL('../src/components/Workspace.tsx', import.meta.url), 'utf8');
    okc('the nav tree is not drawn on Study (both mounts)', sidebarIsStudyFree(wsSrc));
    okc('…and the rule sees the pre-fix shape', !sidebarIsStudyFree([
        "const studyView = workspaceView === 'study';",
        '{!isMobile && <Sidebar />}',
        '{isMobile && (<div><Sidebar isDrawer /></div>)}',
    ].join('\n')));
    okc('…and a drawer left unguarded behind a fixed desktop mount', !sidebarIsStudyFree([
        "const studyView = workspaceView === 'study';",
        '{!isMobile && !studyView && <Sidebar />}',
        '{isMobile && (<div><Sidebar isDrawer /></div>)}',
    ].join('\n')));
    okc('no drawer handle either, so nothing opens what is not there',
        /\{!studyView && \(\s*<button/.test(wsSrc));

    const feedSrc = readFileSync(new URL('../src/components/feed/FeedView.tsx', import.meta.url), 'utf8');
    const headerIsTopicOnly = (src) => /const studyHeader = studying \?/.test(src);
    okc('a COURSE stream carries no header of its own', headerIsTopicOnly(feedSrc));
    okc('…and the rule sees the pre-fix shape',
        !headerIsTopicOnly('const studyHeader = scoped ? (<div className="sticky">…</div>) : null;'));
    // The two wrong destinations: Back landed on a tab the reader had never
    // opened, and on a topic stream it landed where the button beside it goes.
    okc('nothing in the stream navigates to the Dashboard tab',
        !/setWorkspaceView\('dashboard'\)/.test(feedSrc));
    okc('Back leaves the topic for the whole course, not for the topic details',
        /onClick=\{\(\) => studyProjectFeed\(currentProjectId \?\? 0\)\}/.test(feedSrc));
}
