...
// ---------------------------------------------------------------------------
// 13. The headers the download leaves through
// ---------------------------------------------------------------------------
//
// The deck built perfectly and the export still failed: `X-Export-Stats` is
// `JSON.stringify(stats)` and `stats.project` is the project's NAME, so «Нидер-
// ландский язык: с нуля до C1» threw `Invalid character in header content
// ["X-Export-Stats"]` — an error about a header, raised after every card and
// every clip had been read, on a course that is only ever Russian. The
// `Content-Disposition` beside it had been fixed for a Japanese deck long
// before; the advisory header had not, because nothing asserted it.
//
// Judged by NODE'S OWN validator rather than a regex of my own: `setHeader` is
// what threw, and `validateHeaderValue` is the function it calls. A rule
// re-implemented here could agree with itself and still disagree with the
// thing that runs in production.
{
    const { validateHeaderValue } = await import('node:http');
    const { contentDisposition, headerJson } = await import('../server/httpHeaders.js');

    const accepted = (name, value) => {
        try { validateHeaderValue(name, value); return true; } catch { return false; }
    };

    // The control: the shape that shipped. Without this the two checks below
    // could pass against a validator that accepts anything.
    const RU = 'Нидерландский язык: с нуля до C1';
    ok(!accepted('X-Export-Stats', JSON.stringify({ project: RU })),
        'the pre-fix header — a raw JSON.stringify of the stats — is what Node rejects');

    const stats = { project: RU, notes: 3126, skipped: 0, decks: 150, mediaFiles: 2, missingMedia: ['早い_ハヤ.mp3'] };
    const encoded = headerJson(stats);
    ok(accepted('X-Export-Stats', encoded), 'the encoded stats header is a legal header value');
    eq(JSON.parse(encoded), stats, 'and the client parses it back to exactly the stats that went in');

    // `missingMedia` is the half a fix aimed only at the project name would
    // miss: a Japanese clip's filename travels in the same header.
    ok(!/[^\x20-\x7e]/.test(encoded), 'every character in it is printable ASCII');

    const cd = contentDisposition(`${RU}.apkg`, { inline: false });
    ok(accepted('Content-Disposition', cd), 'the disposition is a legal header value too');
    ok(cd.includes(`filename*=UTF-8''${encodeURIComponent(`${RU}.apkg`)}`),
        'and the real name still travels in filename*, so the download is not saved as "_"');
    ok(/filename="[\x20-\x7e]+"/.test(cd), 'beside an ASCII filename an older client can read');

    // A name that is ENTIRELY non-ASCII must not collapse to an empty quoted
    // filename — the bundle export used to sanitise every such run to one `_`.
    ok(contentDisposition('日本語', { inline: false }).includes('filename="___"'),
        'a wholly non-Latin name keeps one placeholder character per character');
}
