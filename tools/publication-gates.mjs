 ...
 *
 *  • **A change note written into the prose.** "X used to be Y", "(fixed
 *    <date>)" — a sentence about a former state turns a specification into a
 *    changelog, and it ages next to the claim it sits beside. The reason
 *    stays; the history belongs in the changelog.
 *
...
const rules = [
    ...
    {
        name: 'a change note written into the prose',
        // "X used to be Y", "(fixed 2026-09-17)", "was the bug" — a phrasing
        // about a former state turns a specification into a changelog, and it
        // ages next to the claim it sits beside. State what is true now, with
        // the reason: the counterfactual ("a slideshow would show nothing
        // moving") carries the same why and does not go stale.
        re: /\bused to (?:be|have|live|sit|carry|serve|count|call|read|draw|work|run|make|go|say|cost|take)\b|\bstated (?:plainly|honestly)\b|\b(?:fixed|found|re-checked) 20\d\d-\d\d\b|\bwas a live regression\b|\bthe day it was found\b|\balready happened once\b|\bwas the bug\b|\bexpensive to find\b/i,
        // Two places where the match is the point: the GPL's own text, and the
        // `consumed_at` migration comment in `database.js`, which documents the
        // legacy shape the migration exists to rewrite.
        allow: [/^LICENSE$/, /^server\/database\.js$/],
        why: 'State what is true now. A sentence about a former state reads as a changelog entry, and it ages next to the claim it sits beside.',
    },
];
