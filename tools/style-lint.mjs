...
//   2. ACCENT MISUSE      violet/purple families used as an accent (ARCHITECTURE:
//                         "Never hardcode violet-* for accents — use accent").
...
//
// Not committed (see .gitignore `tools/`). Static & heuristic: it reads resting
// classes only, and the documented rainbow palettes are allow-listed, so a clean
// run means "no new inconsistencies", not "provably perfect".
...
const ALLOW = [
  ...
  'src/components/Settings.tsx',      // COLOR_MAP model-card rainbow (documented)
