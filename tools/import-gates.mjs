...
//    budget writing something that is thrown away on import. It used to live in
//    PromptToGenerateProjectJSON.md, a doc that duplicated the field tables; it
//    is now the string the app itself serves, so there is one copy to check.
//
...
// This one used to be silent, which is how a shared Dutch course ends up having
// every future lesson authored in whatever the local model guesses.
check('...and says so, because the silent version was the bug', badLang.warnings.length, 1);
