...
const r1 = updateMasteryFromAttempt(cold, 8, 10, 'mastery_check', { questionType: 'multiple_choice' });
...
// MIN_GATE_QUESTIONS answered at at least `checkPass`. Both are `>=`, and the
...
const assessed = (score, total) => {
    ...
    updateMasteryFromAttempt(n, score, total, 'mastery_check', { questionType: 'multiple_choice' });
    return checkMasteryEligibility(n, 0.85, CHECK_PASS);
};
...
check(`a score exactly at the pass mark (8/10 = ${CHECK_PASS}) counts`,
...
check('a topic with no evidence at all is not eligible on either clause',
    checkMasteryEligibility(mkNode('untouched'), 0.85, CHECK_PASS).eligible === false);
...
check('a perfect run of flashcard self-ratings never opens the assessment clause',
    checkMasteryEligibility(selfRated, 0.85, CHECK_PASS).passed_assessment === false);
