...
<p align="left">
  ...
  <img alt="twelve interface languages" src="https://img.shields.io/badge/i18n-12_languages-informational?style=flat-square">
</p>

<p align="center">
  <img src="docs/screenshots/feed-hero.png" alt="A Terramentor study feed for a topic on neural-network weights: the lesson's first part with inline formulas, and a chart the model wrote that draws a weight of 2 as a steep rising line and −2 as a steep falling one" width="820">
</p>

<p align="center"><sub>The study feed of one topic. The lesson, the formulas and the chart are written
for that topic by whichever model you point the app at; a question's answer key is solved
again, cold, by a second pass, and a question whose key does not survive that is never served.</sub></p>

<p align="center">
  <img src="docs/screenshots/terra-globe.png" alt="The atlas drawn as a globe: 18 topics from two courses grouped into six regions by meaning, the selected region painted darker for what is proven, its topics listed beside it" width="49%"><img src="docs/screenshots/review-session.png" alt="A flashcard review session beside the topic tree: the card shows the question and its answer, with Again, Hard, Good and Easy buttons naming the interval each will schedule" width="49%">
</p>

<p align="center">
  <img src="docs/screenshots/schedule-board.png" alt="The schedule board with one card per project: a filled bar for what is done with a notch for where the plan puts you today, days left, topics done, and pace badges" width="820">
</p>

<p align="center"><sub><b>Atlas · Terra</b>: everything you have studied, grouped by meaning rather than
by course, as a planet you turn. <b>Review</b>: the question stays on the answer side, and
each rating names the interval it schedules. <b>Schedule</b>: fill is what is done, the notch
is where the plan puts you today, and pace is measured against the schedule, not the calendar.</sub></p>

<details>
<summary>The whole lesson, top to bottom</summary>

<img src="docs/screenshots/feed-lesson.png" alt="The same study feed scrolled out in full: two lesson parts with worked formulas, a chart drawing ReLU as a blue kinked line and sigmoid as an orange S-curve, a numeric question, and a practice drill" width="820">

</details>

Most study tools help you *organise* material you already have. This one does
the whole job: it plans a curriculum against your deadline, writes the lessons,
quizzes you on what it taught, will not close a topic until you have proved you
know it, and brings back what you are forgetting.
...
It collects no telemetry, asks for no account, and is tied to no vendor. What
network traffic there is goes to the model endpoint you chose, to a resource
search while the AI drafts a project (on by default, one switch to turn off),
and to two features that ship off.
...
and visuals on top, and it is never a dependency.
...
Docker is the only prerequisite: the published image already contains the built
app, so nothing compiles on your machine.
...
published but never becomes `latest`. Pinning `:1.0` instead takes patches
and nothing else.
...
`.certs`), so it installs as a PWA, with no dev server in a window.
...
Your phone can use the same library, with the same review queue and the same
schedule, by talking to your own computer. Nothing is uploaded and there is no account.
...
In **Settings → General → About** you will find the version, the commit and how
it was installed, with a one-press **Copy details** that puts the same facts into
a bug report.
...
| **Generation is slow** | Expected on local hardware. Generation runs in the background; navigate away and it survives. |
