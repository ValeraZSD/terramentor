...
export type TutorAction = 'mastery_check' | 'next_topic';

const MARKERS: Record<string, TutorAction> = {
    'mastery-check': 'mastery_check',
    'next-topic': 'next_topic',
};

/** A complete marker: `[[mastery-check]]`. Tolerates case and inner spacing. */
const MARKER_RE = /\[\[\s*(mastery-check|next-topic)\s*\]\]/gi;
