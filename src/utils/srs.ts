 ...
 *
 * **`scheduled_days` is the wrong field to read.**
 * On a (re)learning step FSRS reports `scheduled_days: 0` and puts the real
 * answer in `due` — so `Math.max(1, scheduled_days)` does not merely round a
 * 10-minute step, it invents a day the scheduler never asked for, and does it
 * identically for every sub-day step. The interval is the distance to `due`,
 * and days are what the rest of this module speaks.
 */
