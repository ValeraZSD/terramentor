 ...
 *
 * A size FITTED per pair (`min(0.6, max(0.45, baseWidth / readingWidth))`: one
 * kana over one kanji at 0.6em, three kana at 0.45em) looks right on each pair
 * and wrong on the line they share. A reading's font-size decides its box
 * height as well as its width, so a sentence carrying several readings draws
 * them at several sizes AND at several heights: measured on the real card 11667
 ...
 * that overhang their base at a flat 0.5em are **the same 791** whose own fit
 * asks for more than 0.45em anyway, so uniformity buys the raggedness fix
 * without widening a single column that was not already wide.
 *
 ...
 *
 * The blunt rule "supporting lines are answer-side only, because an example
 * sentence contains the word and therefore hands over the answer" is half
 * right, and the half it gets wrong is the useful half. Anki shows the
 * sentence on the FRONT — without its
