    ...
    return (
        ...
        >
            {/* WHATEVER IS TYPED IS KEPT, including nothing.
                ...
                two ordinary guards make those states permanent. `if (v)`
                drops the empty value a partly typed segment produces, so the
                state never changes, so React restores the node to the last
                good date and the keystroke vanishes. The clamp
                (`v < from ? from : v`) rewrites any intermediate that falls
                outside the other end: typing 01122027 into the deadline snaps
                it to the START date on the third keystroke and pins it there
                for the rest of the sequence, and typing into the start field
                commits years like 0099 before snapping back. Measured, key by
                key, with `temp/schedule-typing-probe.mjs`.
