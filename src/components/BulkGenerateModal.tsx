...
import { api } from '../api';
import type { BulkCandidate, BulkEstimates, BulkJobStatus, BulkKind } from '../types';
import {
 ...
 *     questions ARE its Boss Fight bank (`MasteryGateModal` reads every
 ...
 *     is a Boss Fight here, sized to clear the gate, and the card says what it
 ...
 *  3. THE COST MUST BE VISIBLE BEFORE IT IS PAID, IN MINUTES. Calls are an
 *     abstraction; "about 14 minutes" is the decision. The server measures each
 *     call and the footer spends those measurements — hedged as "roughly" until
 *     this machine's own model has actually been timed.
 *
...
export default function BulkGenerateModal({ projectId, onClose }: { projectId: number; onClose: () => void }) {
    ...
    const [estimates, setEstimates] = useState<BulkEstimates | null>(null);
    const [max, setMax] = useState(60);
    const [selected, setSelected] = useState<Set<number>>(new Set());
    const [kinds, setKinds] = useState<BulkKind[]>(['flashcards', 'boss_fight']);
    ...
    const needs = useCallback((c: BulkCandidate) =>
        kinds.some(k => k === 'boss_fight' ? !c.gateReady : c.flashcards === 0), [kinds]);
    ...
    // The honest cost of pressing the button: one model call per topic per kind,
    // minus whatever "only what's missing" takes off, priced in minutes.
    const plan = useMemo(() => {
        if (!candidates) return { topics: 0, calls: 0, skipped: 0, ms: 0, measured: true };
        let calls = 0, topics = 0, skipped = 0, ms = 0, measured = true;
        for (const c of candidates) {
            if (!selected.has(c.nodeId)) continue;
            const todo = kinds.filter(k => !skipExisting || (k === 'boss_fight' ? !c.gateReady : c.flashcards === 0));
            ...
            for (const k of todo) {
                ...
                ms += est ? est.ms : 0;
                if (est && !est.measured) measured = false;
            }
        }
        return { topics, calls, skipped, ms, measured };
    }, [candidates, selected, kinds, skipExisting, estimates]);
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                                {t("Boss fights and flashcards for many topics at once")}
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto">
                    ...
                    <section className="px-5 py-4 space-y-3">
                        ...
                        <div className="grid sm:grid-cols-2 gap-3">
                            <KindCard
                                active={kinds.includes('boss_fight')}
                                onClick={() => toggleKind('boss_fight')}
                    ...
                    <section className="px-5 pb-2 space-y-3">
                        ...
                        <div className="flex items-center gap-2 text-xs overflow-x-auto rail-scroll -mx-1 px-1">
                            {([['missing', 'Missing material'], ['open', 'Still open'], ['overdue', 'Overdue'], ['all', 'Everything'], ['none', 'None']] as const)
                                .map(([p, label]) => (
                                    <button
                                        key={p}
                                ...
                                <div key={section} className="mb-1">
                                    <div className="flex items-center gap-2 px-2 sticky top-0 bg-white dark:bg-slate-800 z-10">
                                        ...
                                        <button
                                            ...
                                            aria-expanded={!shut}
                                            className="flex-1 min-w-0 flex items-center gap-1.5 text-left text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 hover:text-accent-fg transition py-2.5"
                                        >
                        ...
                        {plan.calls === 0
                            ...
                            : <>
                                <span className="font-medium text-slate-700 dark:text-slate-200">
                                    ...
                                    {plan.ms > 0 && <> · {plan.measured ? '~' : t("roughly")}{formatDuration(plan.ms)}</>}
                                </span>
            ...
            <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-slate-500 dark:text-slate-400 truncate">
                    {job.waiting
                        ...
                        : job.current
                            ? `${job.current.title} — ${job.current.kind === 'boss_fight' ? t("boss fight") : t("flashcards")}`
...
/**
 * One topic. The chips say what it HAS in the vocabulary of the gate — "boss
 * ready" means its Boss Fight can already run, which is the only fact that
...
function TopicRow({ c, section, on, kinds, skipExisting, onToggle }: {
    ...
    const covered = kinds.length > 0
        && kinds.every(k => k === 'boss_fight' ? c.gateReady : c.flashcards > 0);
    return (
        <label
            ...
            <span className="shrink-0 flex items-center gap-1.5 text-[11px]">
                {c.flashcards > 0 && (
                    <span className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 tabular-nums">
                        {t("{{flashcards}} cards", { flashcards: c.flashcards })}
                    </span>
                )}
                {c.gateReady ? (
