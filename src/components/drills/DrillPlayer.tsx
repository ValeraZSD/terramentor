...
import { answersMatch, buildOptions, closedOptionSet, shuffle } from './parseDrill';
 ...
 */
export default function DrillPlayer({ spec, onRoundComplete }: Props) {
    ...
    useEffect(() => {
        if (phase !== 'playing' || !currentItem || feedback || checking || !seconds) {
            setRemaining(seconds && phase === 'playing' && !feedback ? seconds : null);
            return;
        }
            ...
            <div className="py-2">
                ...
                <div className="mt-5 flex flex-wrap justify-center gap-2">
                    ...
                    <button
                        ...
                        className="px-4 py-2 min-h-11 rounded-xl border border-slate-200 dark:border-slate-600 text-sm font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed transition"
                    >
    ...
    const timerPct = seconds && remaining != null ? Math.max(0, (remaining / seconds) * 100) : null;
    ...
    return (
        <div className="py-1">
            ...
            <div className="mt-1 h-1 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
                <div
                    className={timerPct != null ? 'h-full bg-accent transition-[width] duration-100 ease-linear' : 'h-full bg-accent transition-all'}
                    style={{ width: `${timerPct != null ? timerPct : ((pos) / order.length) * 100}%` }}
                />
            </div>

            {spec.promptLabel && (
                <p className={`mt-5 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 ${promptLong ? 'text-left' : 'text-center'}`}><MathText content={spec.promptLabel} /></p>
            )}
