                ...
                {phase === 'preview' && preview && stats && (
                    <div className="p-5 space-y-5">
                        <p className="text-sm text-slate-700 dark:text-slate-200">
                            <span className="font-semibold text-slate-900 dark:text-white">{t("{{cards}} cards", { count: stats.cards, cards: num(stats.cards) })}</span>
                                ...
                                {samples.slice(shown, shown + 1).map((s, i) => (
                                    <div key={i} data-testid="anki-sample"
                                        ...
                                        <div className="px-3 pt-2.5 pb-2.5 text-slate-900 dark:text-slate-100 whitespace-pre-wrap break-words">
                                            <span className="mb-1.5 block text-[11px] font-semibold text-slate-500 dark:text-slate-400">{t("Question")}</span>
                                        ...
                                        <div className="px-3 pt-2.5 pb-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/40 text-slate-700 dark:text-slate-200 whitespace-pre-wrap break-words">
                                            <span className="mb-1.5 block text-[11px] font-semibold text-slate-500 dark:text-slate-400">{t("Answer")}</span>
                        ...
                        <details className="group rounded-lg border border-slate-200 dark:border-slate-700">
                            {/* The app's own chevron, not the browser's ▸ — which is
                                a different shape, weight and size in every engine and
                                half the size of every other disclosure's mark. */}
                            <summary className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-200 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden">
                                <ChevronRight className="w-4 h-4 shrink-0 text-slate-400 transition-transform group-open:rotate-90" aria-hidden="true" />
                                ...
                                <span className="min-w-0">
                                    {stats.decks.length === 1
                                        ...
                                        : t("{{length}} decks become topics", { count: stats.decks.length, length: stats.decks.length })}
                                </span>
                            </summary>
                                    ...
                                    <p className="text-sm text-slate-600 dark:text-slate-300">
                                        <span className="font-medium">
                                            {stats.skipped === 1
                                                ...
                                                : t("{{skipped}} notes will not become cards:", { count: stats.skipped, skipped: stats.skipped })}
                                        </span>{' '}
                        ...
                        <div className="space-y-2.5">
                            <label className="block">
                                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">{t("Project name")}</span>
                                <input
                                    value={projectName}
                        ...
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="font-medium text-slate-900 dark:text-slate-100">
                                    {t("{{imported}} cards imported into “{{name}}”.", { count: result.imported, imported: num(result.imported), name: result.name })}
                                </p>
                                {result.skipped > 0 && (
                                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                                        {t("{{skipped}} notes were left behind, as listed before the import.", { count: result.skipped, skipped: num(result.skipped) })}
                                    </p>
                                )}
                            </div>
                        </div>
