 ...
 *
 * There are FOUR themes on two axes (light · warm · dark · black), so a boolean
 * was never enough either — `warm` is a cream page and `black` is true black,
 * and both were being drawn against stock white/`slate-900`.
 *
...
export interface VisualPalette {
    /** Theme id (`light` | `warm` | `dark` | `black`) — the cache key for a render. */
    theme: string;
...
export function readVisualPalette(themeKey: string): VisualPalette {
    const dark = themeKey === 'dark' || themeKey === 'black';
