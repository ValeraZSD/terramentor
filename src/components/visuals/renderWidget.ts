...
function progressReporter(onProgress?: (message: string) => void) {
    return (evt: { status?: string; queuePosition?: number | null; thinking?: number; progress?: number }) => {
        ...
        if (typeof evt.thinking === 'number') onProgress(i18n.t("Designing the widget… {{chars}} chars of reasoning", { chars: num(evt.thinking) }));
        if (typeof evt.progress === 'number') onProgress(i18n.t("Building widget… {{chars}} chars", { chars: num(evt.progress) }));
    };
}
