 ...
 * drawing — and the bar used to be scaled against the BRIEF's length for both
 * of them. A drawing is several times longer than the brief that asks for it,
 * so the second call (much the slower one) crossed the whole remaining range in
 * its first few hundred characters and then sat on the 99% clamp for the rest
 * of its run. Worse, a reasoning model spends its opening stretch THINKING,
 * which produces no answer characters at all — so the bar was frozen at 99
 * while the only thing that could move it had not started writing yet.
 *
