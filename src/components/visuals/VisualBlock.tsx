...
const VisualBlock = memo(({ kind, language, code, live = false, autoRepair = false, autoBuild = autoRepair, onRepaired, surface, nodeId, messageId }: VisualBlockProps) => {
    ...
    // There are four themes on two axes, so `light → warm` and `dark → black`
    // both leave `isDark` unchanged while changing every surface colour under
    // the visual — a visual drawn for stock white sat unchanged on a cream page.
    ...
    const palette = useMemo(() => readVisualPalette(theme), [theme, accentColor]);
    ...
    // survive.
    const themeDep = SELF_THEMING_KINDS.has(kind) ? null : `${theme}:${accentColor}`;
        ...
        <div ref={wrapperRef} className="visual-block my-4 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            {sourceVisible && (
                <div className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                    {sourceIsBrief ? (
                        <>
                            <div className="px-4 pt-2 pb-1 text-[11px] font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                            ...
                            {drawnSpec ? (
                                <>
                                    <div className="flex items-center justify-between gap-3 px-4 pt-2 pb-1 border-t border-slate-200 dark:border-slate-700 text-[11px] font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
                ...
                <div className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 px-4 py-6 text-sm text-slate-500 dark:text-slate-400">
                    ...
                    {repairing
                        ...
                        : progressMsg
                            ...
                            : live
                                ? <>{tr("Generating {{label}}…", { label: label.toLowerCase() })}{' '}<span className="tabular-nums text-slate-500 dark:text-slate-300">{tr("{{length}} chars", { length: num(code.length) })}</span></>
