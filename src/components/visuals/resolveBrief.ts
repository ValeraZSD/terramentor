 ...
 * waiting for its one model call. It used to be thrown as an ordinary Error and
 * the shell drew it as an amber "Couldn't render this animation" — an error
 * card over the most normal state a brief can be in (every brief passes through
 * it while the reply is still streaming). The shell now tells this apart from
 * a real render failure by the class, and shows an offer instead of a warning.
 */
