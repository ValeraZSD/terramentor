...
function progressReporter(onProgress?: (message: string) => void) {
    return (evt: { status?: string; queuePosition?: number | null; thinking?: number; progress?: number }) => {
        ...
        if (typeof evt.thinking === 'number') onProgress(i18n.t("Planning the scene… {{chars}} chars of reasoning", { count: evt.thinking, chars: num(evt.thinking) }));
        if (typeof evt.progress === 'number') onProgress(i18n.t("Drawing… {{chars}} chars", { count: evt.progress, chars: num(evt.progress) }));
    };
}
