...
export interface VisualContext {
     ...
     * theme switch and nothing finer, but it is not enough on its own: there
     * are four themes on two axes, so `warm` and `black` were both being drawn
     * against stock white / `slate-900`. Everything that picks its own colours
     * — and everything that has to rescue colours a model picked — uses this.
     */
