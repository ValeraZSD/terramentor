 ...
 *
 * So: the input is still a real checkbox (focusable, announced, space toggles
 * it, forms and screen readers see it), and the visible box is a sibling that
 * follows it with `peer-*`. The tick is the same lucide `Check` the rest of the
 * app uses, at a stroke weight that survives an 18px box.
 *
 * The input is laid OVER the drawn box at zero opacity, not `sr-only`: an
 * sr-only input is a 1px square somewhere else, so a click on the box only
 * reached it when a `<label>` happened to wrap the whole thing, and the box
 * in the model-provider table (whose label is the next cell) did nothing
 * at all. The drawn box is `pointer-events-none` so the click lands on the
 * input under it; inside a label, a click on the control itself toggles once.
 *
...
export default function Checkbox({
    ...
    return (
        <span className={`relative inline-grid shrink-0 place-items-center ${className}`}>
            ...
            <span
                aria-hidden="true"
                className={`pointer-events-none grid h-[1.125rem] w-[1.125rem] place-items-center rounded-[0.3rem] border-2 transition-colors
