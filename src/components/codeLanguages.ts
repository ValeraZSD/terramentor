...
//
// It used to be "all of them, lazily": `prism-async-light` reaches every Prism
// grammar through a dynamic import, so the bundler has to account for all ~300.
// Nobody paid for that at runtime, but everybody paid for it at BUILD time —
// 299 of the 486 dependency bundles Vite pre-bundles for `npm run dev` were
// Prism grammars, which was most of the 19.5 s before the dev server could
// serve its first module (measured 2026-09-09), and ~300 of the 403 chunks the
// production build emitted. For `warpscript`, `unrealscript` and `splunk-spl`,
// in an app whose code blocks are written by a tutor about a curriculum.
//
