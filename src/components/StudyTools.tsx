...
export default function StudyTools({ nodeId, mode, onCountsChange }: Props) {
    ...
    const handleGenerateQuiz = async () => {
        ...
        try {
            ...
            // (the "Remember" loop). The Boss Fight gate deliberately does not.
    ...
    const GenerationCounter = () => {
        ...
        return (
            <p className="text-xs font-mono tabular-nums text-accent-fg text-center -mt-1">
                {progressChars > 0
                    ? t("Generating… ({{progressChars}} chars)", { progressChars: num(progressChars) })
                    : t("Reasoning… ({{thinkingChars}} chars)", { thinkingChars: num(thinkingChars) })}
            </p>
        );
    };
                                ...
                                >
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <div className="flex items-center gap-3 text-sm text-slate-500 dark:text-slate-400 mt-1">
                                            <span>{t("{{length}} questions", { length: quiz.questions.length })}</span>
