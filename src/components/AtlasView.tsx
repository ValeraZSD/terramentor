 ...
 * takes the height it is given and the list scrolls beside it. It used to be a
 * column of blocks in a `max-w-6xl` article — which, since `Layout`'s `main` is
 * `overflow-hidden` and this page never declared a scroll container of its own,
 * meant everything below the fold (the bridges, most of the region list) was
 * not merely hard to reach but unreachable.
 */
/**
...
export default function AtlasView() {
    ...
    //
    // That used to be a button ("Show the whole path"), which is a strange
    // thing to ask someone to press: the walk is over, the path is drawn, and
    // the reader is left parked inside whichever bubble the last topic lives
    // in. Ending on the wide shot is what the replay was for, and it frees the
    ...
    // time. Handing the ending to the brisk arm is what read as a snap.
    useEffect(() => {
                    ...
                    >
                        ...
                        <div className="flex w-full items-center gap-2 flex-wrap">
                            {/* The switch names what it CHANGES, which is where
                                ...
                                the whole map, or each course's. It used to say
                                "Proven / Course", which read as a choice
                                between showing proof and showing courses; both
                                modes show proof, in the same five steps, and
                                the one beside "Course" was the one with no
                                course in it. */}
