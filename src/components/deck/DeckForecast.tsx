    ...
    return (
        <div>
            ...
            <div className="flex items-stretch gap-1 sm:gap-1.5" style={{ height: 96 }}>
                {days.map((n, i) => {
                    const d = labelFor(i);
                    const label = `${d.toLocaleDateString(uiLocale(), { weekday: 'short', month: 'short', day: 'numeric' })}: ${t("{{fmt}} cards", { count: n, fmt: fmt(n) })}`;
                    return (
            ...
            <div className="mt-1.5 flex justify-between text-[11px] text-slate-500 dark:text-slate-400">
                <span>{t("Today")}</span>
                <span>{t("In {{length}} days", { length: days.length })}</span>
            </div>
