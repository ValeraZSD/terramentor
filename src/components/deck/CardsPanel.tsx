                ...
                {owed > 0 ? (
                    <>
                        ...
                        <button
                            ...
                            className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-accent px-5 py-3.5 text-base font-semibold text-white transition hover:bg-accent/90"
                        >
                    ...
                    <div className="text-center">
                        ...
                        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                            {t("Brings forward new cards and anything due in the next {{AHEAD_DAYS}} days. Answering early earns a shorter next interval.", { AHEAD_DAYS })}
                        </p>
                        <NewLimit {...newLimitProps} />
                    </div>
                )}
...
function NewLimit({ deck, editing, draft, onDraft, onEdit, onSave, onCancel }: {
    ...
    return (
        <button
            onClick={onEdit}
            className="mt-4 flex w-full items-center justify-center gap-1.5 border-t border-slate-100 pt-3 text-sm text-slate-500 transition hover:text-slate-700 dark:border-slate-700 dark:text-slate-400 dark:hover:text-slate-200"
        >
            <Settings2 className="h-3.5 w-3.5" aria-hidden="true" />
            {t("{{fmt}} new cards a day", { fmt: fmt(deck.newPerDay) })}
            {deck.introducedToday > 0 && t("· {{fmt}} met today", { fmt: fmt(deck.introducedToday) })}
        </button>
    );
}
