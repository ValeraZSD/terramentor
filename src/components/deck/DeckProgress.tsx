 ...
 *
 * A single 6px track with two nested fills (how much of it has been met at
 * all, and how much of it is mature) answers "how far along", and for a deck
 * with no structure of its own that is the ONLY thing on the row that differs
 * between "Stage 4" and "Stage 27": thirty rows of identical grey with
 * identical titles.
 *
 * Two resolutions:
 *
...
export function StateTicks({ map, height = 'h-2', gap }: {
    ...
    return (
        <div
            ...
            style={{ gap }}
            role="img"
            aria-label={t("{{length}} cards: {{describeStates}}", { count: cells.length, length: cells.length, describeStates: describeStates(counts) })}
        >
            {cells.map((ch, i) => {
