...
export function SegmentedControl<T extends string | number>({
    ...
    // in it says where one option ends and the next begins. So the segments
    // draw their own outline once, and only once, they have wrapped.
    //
    ...
    // Wrapped, the unselected ones are drawn as outlines — a button each,
    // with the chosen one filled among them. An outline rather than a second
    // fill: on the light theme a pale chip on a pale track is within a shade
    // of the white the SELECTION is made of, and the one thing this control
    // has to say would be carried by a shadow.
    const outlined = wrapped && 'ring-1 ring-slate-300 dark:ring-slate-600';
    ...
    return (
        <div
            ...
            className={cx(
                ...
                // Wrapped, the gap grows: two rows of outlined chips 2px apart
                // are a grid, not a group of buttons.
                'inline-flex max-w-full flex-wrap items-stretch rounded-lg p-1',
                wrapped ? 'gap-1' : 'gap-0.5',
                track && 'bg-slate-100 dark:bg-slate-700/70',
