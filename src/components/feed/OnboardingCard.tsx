 ...
 *
 * The stage it belongs to is deliberately NOT shown. It used to be, as a small
 * capital badge before each label and again as an arrow chain in the subtitle,
 * and that was the only place in the whole interface those five words appeared
 * — so the card named its own vocabulary twice to a reader with nothing to
 * attach it to, and the two namings were translated independently: in 7 of 12
 * languages they disagree, worst in Russian, where 1 of 5 matched ("ОБЗОР
 * Создать проект" over "цикл: Найти → Спланировать → …"). Two of the six steps
 * are the same stage anyway, so the column read "STUDY / STUDY". A checklist
...
const STEPS: { key: StepKey; label: string; hint: string }[] = [
    ...
    { key: 'prove', label: k("Complete a topic"), hint: k("Take its Boss Fight — or mark it done, or skip it honestly.") },
 ...
 * explanation of why it was choosing things, what a Boss Fight was, or what the
