...
import { Award, CheckCircle2, Loader2, ShieldAlert, ShieldCheck, SkipForward } from 'lucide-react';
 ...
 *
 * "Take the mastery check" opens the gate modal RIGHT HERE. It used to navigate
 * into the workspace and leave the learner staring at a node detail panel,
 * having to find the mastery check themselves — three taps and a lost scroll
...
export default function CheckpointCard({ card, done, onDone }: Props) {
    ...
    // workspace's mastery check modal.
    const skip = async () => {
        if (saving || done) return;
        setSaving(true);
        try {
            ...
            {done ? (
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {/* Not proven yet → proving it is the real next step, so the
                        mastery check leads. Already proven → completing is, and the
                        mastery check stays available for anyone who wants the rep. */}
