...
interface Props {
    ...
    inChapter?: boolean;
    /**
     * THE EYEBROW IS SAID ONCE PER RUN, not once per question.
     *
     * A topic that owns a bank is asked two questions after each lesson part,
     * and the end of a topic is questions and nothing else — so "Check your
     * understanding" came up three and four times down one card, in the
     * accent, above questions that were already obviously questions. A label
     * repeated over every member of a run stops being a label and becomes
     * texture. The hairline `ChapterSection` already draws is what separates
     * the second question from the first; the words are only needed where the
     * run STARTS, which is where the reader changes from reading to being
     * asked. False for every question whose neighbour above is one.
     */
    labelled?: boolean;
    /** The label's tick: every question in the run answered (defaults to `done`). */
    labelDone?: boolean;
}
 ...
 */
export default function QuestionCard({ card, done, onDone, inChapter = false, labelled = true, labelDone = done }: Props) {
    ...
    if (inChapter) {
        ...
        return (
            <div>
                {labelled && (
                    <p className="flex items-center gap-1.5 mb-3 text-[11px] font-semibold text-accent-fg">
                        {labelDone
