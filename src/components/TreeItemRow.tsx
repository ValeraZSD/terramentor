        ...
        <>
            ...
            >
                ...
                <div className={`flex items-center transition-opacity ${rowControls}`}>
                    <DropdownMenu
                        trigger={<button
                            aria-label={t("Actions for {{title}}", { title: node.title })}
                            /* A 14px glyph in a `p-1` box is a 22px target,
                               and on a phone this menu is where "Study this
                               topic" lives. The icon stays the same size —
                               the row is 32px tall and a bigger glyph would
                               shout — but the TARGET grows to the touch
                               floor, which costs the row nothing because the
                               button is centred in it. */
                            className="flex items-center justify-center p-1 min-w-8 min-h-8 touch:min-w-11 touch:min-h-11 hover:bg-slate-200 dark:hover:bg-slate-600 rounded text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
