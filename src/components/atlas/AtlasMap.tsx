...
import {
    CourseTrace, Flight, Leg, advanceFlight, flightPath, legAt, legParam, legUpTo, newFlight,
} from './coursePaths';
...
import {
    ...
    JOURNEY_HALO_ALPHA, ARROW_MIN_LENGTH, ARROW_SIZE, HEAD_ARROW_LENGTH, HEAD_ARROW_WIDTH,
    HEAD_GLOW_RADIUS, TRACE_REGION_DIM, TRACE_TOPIC_DIM, TRACE_DOT_MIN, LOOK_AHEAD, FIT_AHEAD,
...
export interface AtlasMapHandle {
     ...
     *
     * A replay used to be watched from the whole-journey framing, and at that
     * distance most of it is not visible: a run of topics finished in one part
     * of the library is a run of steps a few pixels long. Framing the route and
     * then watching it from there shows the shape of the course; it does not
     * show the journey. The course's shape is what picking it in the list
     * already draws.
     *
...
interface Props {
    ...
    dark: boolean;
    /**
     * The theme, not merely its light/dark axis.
     *
     * There are FOUR themes on two axes, and the surface this canvas paints is
     * decided by the flavour: `warm` is cream, `black` is true black. Keying the
     * palette on `dark` alone meant `light→warm` and `dark→black` changed
     * nothing the canvas could see — the flavour was read off the DOM INSIDE the
     * memo, so it was resolved once and never again — and the map went on
     * painting stock white inside a cream page, or slate inside a black one.
     * The same trap `VisualBlock` had, and the same fix: depend on the THEME.
     */
    theme: string;
 ...
 *
 * It used to be one threshold, and that is what "the small blobs' text appears
 * in an instant" was: the region names ride a zoom ramp a full 1.7× wide, but
 * every topic name in a region switched on together the moment its dots crossed
 * 4.5px. On a phone that crossing lands well after the opacity ramp has already
 * reached the top, so they arrived at full strength — a page of text appearing
...
// the map has.
//
// There used to be a second layer under it: the curriculum's own tree, a faint
// web of parent→child links. It was drawn as context and it read as clutter —
// on a real course, a hundred hairlines running off every edge of the screen,
// most of them between topics the learner has never opened, behind the one line
// that is about them. The map dissolves courses on purpose; what a reader wants
// back is not the syllabus, it is their own path through it, which is why that
// path now arrives a leg at a time and STAYS.
...
// ---- the flight -------------------------------------------------------------
//
// A replay used to be a slideshow: the path grew by one whole segment every
// time the clock ticked and the head marker was simply somewhere else. Nothing
// ever moved, so nothing read as travel — and travel is the only thing a
// journey has to say that a finished path does not.
//
...
const AtlasMap = forwardRef<AtlasMapHandle, Props>(function AtlasMap(
    {
        regions, selectedId, onSelect, onOpenTopic, matches, dark, theme, accent,
    ...
    const live = useRef({ regions, selectedId, matches, onSelect, onOpenTopic, trace, tracedRegions, journeyStep, colorMode, hues });
    live.current = { regions, selectedId, matches, onSelect, onOpenTopic, trace, tracedRegions, journeyStep, colorMode, hues };
    ...
    const palette = useMemo(() => {
        ...
        // The surface follows the theme flavour: warm is cream, black is near-black.
        const surface = theme === 'warm'
            ? '#f4eee4'
            : theme === 'black'
                ? '#000000'
                : dark ? '#0f172a' : '#f8fafc';
        const ring = theme === 'warm'
            ? '#faf6ef'
            : theme === 'black'
                ? '#0d0d0d'
                : dark ? '#0f172a' : '#ffffff';
        const ink = theme === 'warm'
            ? '#211a14'
            : dark ? '#e2e8f0' : '#0f172a';
        const grid = theme === 'warm'
            ? 'rgba(120,104,85,0.14)'
            : theme === 'black'
                ? 'rgba(148,163,184,0.10)'
                : dark ? 'rgba(148,163,184,0.13)' : 'rgba(100,116,139,0.14)';
        return {
            surface,
            grid,
            ring,
    ...
    const flyToCam = useCallback((next: Cam) => {
        followRef.current = false;
        followKRef.current = null;
        if (reduceMotion) {
            camRef.current = next;
    ...
    const frameOnCourse = useCallback((course: CourseTrace) => {
        frameOn(coursePoints(course));
    }, [frameOn]);
    ...
    drawRef.current = () => {
        ...
        const { trace: tracing, journeyStep: stepNow } = live.current;
        ...
        const flown = advanceFlight(flightRef.current, tracing, stepNow, frameMs, reduceMotion);
        ...
        const {
            ...
            colorMode: paint, hues: courseHue,
        } = live.current;
        ...
        //
        // A topic is drawn WHERE IT IS, at every zoom. The path used to run
        // from each topic's region centre out to its own place on the same
        // `detail` ramp the dots fade in on, so that zoomed out a course read
        // as a few region-to-region hops. What that actually does is make the
        // route a function of the camera: every leg swings between two
        // different shapes as you zoom, legs inside one bubble grow out of
        // nothing, and the line you were looking at is not the line you get
        // back. A map you can zoom is a map whose contents hold still — so the
        // journey is one fixed shape in world coordinates, and zooming only
        // ever makes it bigger. The traced course's own dots are drawn at every
        // zoom to go with it (see the topic layer below), or the ends of the
        // line would be places with nothing marking them.
        ...
        if (traced && flown) {
            ...
            if (landedPoint && landed > 0) {
                ...
                drawHead = () => {
                    ...
                    // everywhere else on this map. Under the arrow rather than
                    // a ring around it: a ring drawn around something moving
                    // reads as a second object being dragged along behind it.
                    const glow = ctx.createRadialGradient(head.x, head.y, 0, head.x, head.y, HEAD_GLOW_RADIUS);
                    glow.addColorStop(0, acc(0.55));
                    glow.addColorStop(0.55, acc(0.22));
                    glow.addColorStop(1, acc(0));
                    ctx.globalAlpha = 1;
                    ctx.fillStyle = glow;
                    ctx.beginPath();
                    ctx.arc(head.x, head.y, HEAD_GLOW_RADIUS, 0, Math.PI * 2);
                    ctx.fill();
                    ...
                    if (leg) {
                        ...
                        // Nose forward, tail notched, so the shape reads as
                        // flight rather than as a triangle that happens to be
                        // tilted.
                    ...
                    ctx.closePath();
                    // Outline first, under the fill: stroking a shape AFTER
                    // filling it spends half the line width eating into the
                    // shape, and this one has a fine point to lose.
                    ctx.strokeStyle = palette.surface;
                    ctx.lineWidth = 3.5;
                    ctx.lineJoin = 'round';
                    ctx.stroke();
        ...
        if (targetRef.current || fading || flying) requestDraw();
    };
    ...
    // ---- the floating card --------------------------------------------------
    // A pin beats a hover, and the order here is the whole bug. It used to be
    // `hover || pinned`, so the moment the pointer left a pinned dot to reach
    // the card's "Open topic" button it crossed the parent bubble, the region
    // won the card, and the button was replaced by the region's summary — which
    // also flipped `isPinnedTopic` false and so set the card to
    // `pointerEvents: 'none'`, putting it out of reach entirely. The only way
    // through was to cross the gap faster than a pointermove could fire. A
    // pinned card now holds the surface until it is dismissed (empty space,
