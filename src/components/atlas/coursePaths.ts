 ...
 *
 * Its own place and nothing else. A `TracePoint` used to carry its region's
 * centre too, and the renderers drew it somewhere between the two depending on
 * the zoom — so a course was a few region-to-region hops from far away and the
 * real topics up close. It reads well in a still frame and it makes the route a
 * function of the camera: zooming changes which shape the line has, and legs
 * inside one bubble grow out of a point. The journey is one shape now, in the
 * map's own coordinates, and the zoom only decides how big it is drawn.
 */
 ...
 *
 * The curriculum's own tree is deliberately NOT part of this. It used to be —
 * a faint web of parent→child links drawn under the path — and on a real course
 * it is a hundred lines running off the edge of the screen behind the one line
 * that is about the learner. The map already dissolves courses on purpose; what
 * this layer is for is the route somebody actually took through what is left.
 */
 ...
 */
export function stepDurationMs(trace: CourseTrace, step: number): number {
...
export const newFlight = (): Flight => ({ at: 0, target: null, rest: 0 });
 ...
 * racing across fifty topics nobody asked to watch. It used to be answered by
 * DISTANCE (a target more than 1.5 steps from the arrow was a cut), which
 * confused the two: a browser that skipped half a second of frames left the
 * arrow far enough behind to look exactly like a jump, so a stutter turned into
 * the map cutting straight from one topic to the next. Two missed beats is a
...
export function advanceFlight(
    ...
    frameMs: number, reduceMotion: boolean,
): Flown | null {
    if (!trace) {
        flight.at = 0; flight.target = null; flight.rest = 0;
        return null;
    }
    ...
    if (reduceMotion || want <= 1 || want < flight.at || !walked) {
        ...
        const legMs = stepDurationMs(trace, Math.floor(flight.at) + 1);
        ...
        flight.at = Math.min(want, flight.at + elapsed * (behind / (legMs * FLIGHT_SHARE)));
    ...
    const beatMs = stepDurationMs(trace, Math.max(1, want));
    const eyeAt = clamp(frac > 0
        ? landed + frac * FLIGHT_SHARE
        : landed - (1 - FLIGHT_SHARE) + Math.min(flight.rest / beatMs, 1 - FLIGHT_SHARE),
        1, full);
    return { landed, frac, eyeAt, inAir: frac > 0 && landed >= 1 && landed < full, flying };
}
