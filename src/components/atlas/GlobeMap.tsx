...
import { CourseTrace, Flight, advanceFlight, newFlight } from './coursePaths';
import {
...
interface Props {
    ...
    dark: boolean;
    /** The theme, not merely its light/dark axis — `warm` is cream, `black` is
     *  true black, and the planet is painted on that surface. */
    theme: string;
    accent: string;
...
const GlobeMap = forwardRef<GlobeMapHandle, Props>(function GlobeMap({
     ...
     * step.
     *
     * One direction per topic, not two. This used to carry the region's centre
     * as well and the journey was drawn somewhere between the two on the zoom's
     * own ramp, which made the route a function of the camera: the same course
     * had a different shape at every distance, and a leg between two topics of
     * one region grew out of a point as you came in. A place is a place.
     */
    ...
    const live = useRef({
        ...
        trace, journeyStep, tracePlaces, tracedRegions,
    });
    live.current = {
        placed, selectedId, matches, colorMode, hues, onSelect, onOpenTopic,
        trace, journeyStep, tracePlaces, tracedRegions,
    ...
    // The same four themes the flat map paints, and the same reason it depends
    // on the THEME rather than on `dark`: `warm` is cream and `black` is true
    // black, and a canvas keyed on the light/dark axis alone paints stock white
    // inside a cream page.
    const palette = useMemo(() => {
        ...
        const surface = theme === 'warm' ? '#f4eee4'
            : theme === 'black' ? '#000000'
                : dark ? '#0f172a' : '#f8fafc';
        const ink = theme === 'warm' ? '#211a14' : dark ? '#e2e8f0' : '#0f172a';
        // The planet's own body: the lit side and the side falling away from
        // the light. Two tones and nothing else — a globe with a texture on it
        // would be a picture of a planet, and this is a diagram.
        // The body is two tones — the side facing the light and the side falling
        // away from it — and BOTH are surfaces the mastery ramp gets drawn on,
        // which is the constraint that picks them.
        //
        // `atlas-gates.mjs` holds the ramp's palest rung to 1.5:1 against every
        // surface the flat map uses, and the globe adds two more the flat map
        // never had. The first try made the lit side of a dark planet lighter
        // than the page (#1e2d47) — a handsome ball, and the untouched end of
        // the ramp came out at 1.17:1 on it for the slate accent, below the
        // rule and visibly washed out. So on a dark theme the lit side IS the
        // page colour (#0f1829 is the lightest tone that still clears 1.5,
        // measured over all seven shipped accents by
        // `temp/globe-surface-check.mjs`) and the shading runs DOWN from there:
        // the sphere is read from its darkening limb, which is the cue that
        // says sphere anyway. On light themes the pale rung is the palest thing
        // the app draws on purpose and is no worse here than on the page, so
        // the body only has to stay off pure white — which is what the first
        // version got wrong in the other direction.
        const lit = theme === 'warm' ? '#efe6d7'
            : theme === 'black' ? '#181818'
                : dark ? '#0f172a' : '#f2f6fa';
        const shade = theme === 'warm' ? '#d4c4ab'
            : theme === 'black' ? '#050505'
                : dark ? '#060b14' : '#c9d5e3';
        const grid = theme === 'warm' ? 'rgba(120,104,85,0.22)'
            : theme === 'black' ? 'rgba(148,163,184,0.16)'
                : dark ? 'rgba(148,163,184,0.18)' : 'rgba(100,116,139,0.20)';
        const rim = theme === 'warm' ? 'rgba(120,104,85,0.35)'
            : dark ? 'rgba(148,163,184,0.30)' : 'rgba(100,116,139,0.28)';
    ...
    drawRef.current = () => {
        ...
        if (target) {
            // On a SPRING, and not on the exponential ease this used to be.
        ...
        // Both surfaces used to run the drawn place from the region's centre
        // out to the topic on the same `detail` ramp the dots fade in on, so
        // that from far away a course read as a few region-to-region hops. That
        // makes the route a function of the camera — one course, a different
        // shape at every distance, legs growing out of a point as you zoom —
        // and a map you can zoom has to hold still. The traced course's own
        // topics are drawn at every zoom to match (see the dot layer below).
                ...
                if (headV[2] > 0) {
                    ...
                    drawHead = () => {
                        ...
                        if (leg) {
                            ...
                            // flight rather than as a tilted triangle.
        ...
        if (moving || fading || flying) requestDraw();
    ...
    useEffect(() => { requestDraw(); },
        [placed, selectedId, matches, colorMode, hues, palette, spinning, requestDraw]);
    ...
    const turnTo = useCallback((
        u: Vec3, k?: number, ease?: { omega: number; kOmega: number },
    ) => {
    ...
    const takeInCourse = useCallback((
        course: CourseTrace, ease?: { omega: number; kOmega: number },
    ) => {
        ...
        followRef.current = false;
        followKRef.current = null;
        turnTo(middle, clamp(fit, MIN_K, FOLLOW_K), ease);
    }, [turnTo]);
