 ...
 *
 * **These are a STACK, not a map keyed by card.** A map is sound only while a
 * card can be rated at most once per session, and (re)learning steps make a
 * second rating of the same card the normal case: the second snapshot
 * overwrites the first, so undoing twice restores the state before the second
 * rating and, on the way back past the first, finds nothing to restore and
 * silently moves the cursor over a rating that still stands. One entry per
 * step, popped in step order, is the only
 ...
 *
 * Two hand-written copies of a 10px uppercase label drift apart quietly
 * (`tracking-widest` against `tracking-wider`), and they sit side by side on
 * the same row, the one place a reader does notice: the difference reads as
 * "these are two different kinds of thing" when they are the same kind of
 * thing, a one-word tag on this card.
 *
...
function CardChip({ tone, children }: { tone: ChipTone; children: React.ReactNode }) {
    return (
        <span className={`text-[11px] font-semibold px-2 py-1 rounded-md ${CHIP_TONE[tone]}`}>
            {children}
        </span>
    );
}
...
export default function GlobalFlashcardReview({ projectId, fetchCards, title, onClose, onComplete }: Props) {
    ...
    /**
     * Take back the last rating — the one control that makes a mis-tap
     * reversible. The rating is already on the server, so this
                ...
                <div className="max-w-sm w-full text-center space-y-5">
                    ...
                    <div>
                        ...
                        <p className="mt-2 text-slate-600 dark:text-slate-300">
                            {/* One sentence, one key: a count phrase plus
                                loose fragments ("still being learned and", a
                                bare "comes" or "come", the tail) cannot be
                                assembled by any language whose verb agreement
                                or word order differs from English's. */}
    ...
    if (sessionComplete) {
        ...
        const reviewedLine = t("You reviewed {{count}} flashcards", { count: finished })
            + (totalReviewed > finished ? t(" ({{answers}} answers)", { count: totalReviewed, answers: totalReviewed }) : '')
                ...
                <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 tabular-nums truncate">
                        ...
                        <span className="hidden sm:inline">
                            {t("{{min}} of {{length}} cards", { count: cards.length, min: Math.min(queue.reviewedCount + 1, cards.length), length: cards.length })}
                            {skipped > 0 && t("• {{skipped}} skipped", { skipped })}
