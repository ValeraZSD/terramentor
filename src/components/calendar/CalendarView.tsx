 ...
 *
 * There used to be two of these — `CalendarView` (one project, built from the
 * in-memory tree) and `GlobalCalendarView` (every project, fetched) — with the
 * same month/week/day grids, the same swipe carousel and the same overflow
 * badge written twice, drifting apart in exactly the ways duplicated views do:
 * the per-project one could show a topic as in-progress and the global one
 * could not, the global one hardcoded Monday, and each had a different idea of
 * what tapping a day should open. What actually differs between them is where
 * the tasks COME FROM and what a click MEANS; the rest was duplication.
 *
 ...
 *
 * Two behaviours were unified rather than merely picked: status colouring (the
 * global chips only knew "done or not" — a cross-project view has no reason to
 * be blinder than the per-project one), and the "+N" overflow badge, which
 * globally spent a whole extra row announcing that a row had been dropped and
 * now sits inline beside the last visible task, as the project one always did.
 *
 * The Mon-Sun / Sun-Sat toggle is GONE from this toolbar and lives in Settings →
 * General. It is a preference you set once, it was component state so it reset
 * to Monday on every navigation, and on a phone it was 100px of a toolbar that
 * still has to hold the month, the mode and the arrows.
 */
