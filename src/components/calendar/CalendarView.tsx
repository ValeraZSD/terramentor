...
export default function CalendarView({ scope }: { scope: CalendarScope }) {
    ...
    /**
     * A chip is drawn once, on the first day of its span this period shows, so
     * the title alone no longer says when the topic actually runs. A multi-day
     * one hands that back on hover rather than spending chip width on it — the
     * cells are already too narrow for the titles.
     */
    const taskLabel = (task: CalendarDay['tasks'][number]) =>
        task.start === task.end
            ? task.title
            : `${task.title} · ${dateRangeLabel(parseDate(task.start), parseDate(task.end))}`;
        ...
        <div ref={setRootEl} className="h-full flex-1 flex flex-col overflow-hidden">
            ...
            {visibleProjects.length > 0 && (
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 px-3 sm:px-4 py-2 border-b border-slate-200 dark:border-slate-700 shrink-0 max-h-[22vh] overflow-y-auto">
                    {visibleProjects.map(p => (
                        <span key={p.id} className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 min-w-0" title={p.name}>
                            <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: p.color }} aria-hidden="true" />
                            {/* No width cap. A flat 180px truncated "VWO Mathematics B —
                                Boswell-Bèta / CCVX…" on a 2560px screen with half the row
                                empty beside it. The row WRAPS, so an entry is laid out at
                                its natural width and only ever truncates when one name is
            ...
            {viewMode === 'month' && (
                <div className="grid grid-cols-7 border-b border-slate-200 dark:border-slate-700 shrink-0">
                    {dayHeaders.map(h => (
                        <div key={h} className="py-1.5 text-center text-[11px] font-semibold text-slate-500 dark:text-slate-400">
                            {h}
                        </div>
                    ))}
                </div>
