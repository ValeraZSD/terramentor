...
import { CardProposals, CaptureProposals, CheckButtons } from './AssistantProposals';
 ...
 */
/** Re-follows of a running turn after a dropped socket, and the pause per try. */
const MAX_REATTACH = 3;
const REATTACH_DELAY_MS = 1500;
 ...
 *
 * All four branches follow the INTERFACE language. The two words were bare
 * English literals no extractor could see, and the two formatters passed
 * `undefined` as the locale — which is the device's regional settings, not the
 * language the app is being read in, so a Russian interface on an English
 * machine printed "Today", "Yesterday" and "Tuesday" above Russian chrome.
 * Same fault `locale.ts` settled for every other date label in the app.
 *
 * No `timeZone` here on purpose, unlike the study-date labels: a chat message
 * is a real instant and these blobs mark the reader's own calendar days, which
 * is also what `dayKey` groups by.
 */
function dayLabel(iso: string, t: TFunction): string {
    ...
    if (days > 1 && days < 7) return d.toLocaleDateString(uiLocale(), { weekday: 'long' });
...
export default function AssistantDrawer({ open, onClose, onCapture, docked, width, onResize }: Props) {
    ...
    // bubble (a task that settled while unwatched) — and the card says the same.
    const [turnError, setTurnError] = useState<{ message: string; text: string; keptIn?: 'composer' | 'chat' } | null>(null);
    const [streaming, setStreaming] = useState(false);
    const [streamed, setStreamed] = useState('');
    // Whether the assistant may look something up, read for the line under the
    // composer and for nothing else: the decision is the model's, taken
    // mid-answer, and the permission is the Settings switch.
    const webEnabled = useStore(s => s.aiWebSearch);
    ...
    /** Re-follows after a dropped socket, per task id (bounded by MAX_REATTACH). */
    const reattachTriesRef = useRef<Map<string, number>>(new Map());
    /** `open` for async continuations, which must not see the value their effect captured. */
    const openRef = useRef(open);
    openRef.current = open;
    // Bubbles spliced into the transcript for a question whose row the server
    // dropped (a failed background turn), synthetic id → text. Sending that
    // same text again — the retry button above all — must retire the splice,
    // or the question appears twice: the kept bubble AND the new send's own
    // optimistic bubble, identical, one under the other.
    const splicedUserIdsRef = useRef<Map<number, string>>(new Map());
    ...
    const loadHistory = useCallback(async () => {
        ...
        setMessages(rows);
        // The turn in flight keeps its trace: it lives under a synthetic id no
        // stored row carries, and the panel's first-open load can land AFTER
        // an attach has replayed the reasoning into it.
    ...
    const attachToTask = useCallback(async (taskId: string) => {
        ...
        try {
            for await (const evt of api.attachTask(taskId, controller.signal)) {
                ...
                // The lookups, as the device that asked sees them — the server
                // replays the latest list on attach and every change after.
                if (Array.isArray(evt.actions)) trackActions(evt.actions as AiAction[]);
                ...
                if (evt.done || evt.cancelled) { terminal = true; break; }
            }
        } catch {
            // A bare throw here — the socket dying mid-stream, say — is not a
            ...
            if (!terminal && !failed && !controller.signal.aborted) {
                const tries = (reattachTriesRef.current.get(taskId) ?? 0) + 1;
                reattachTriesRef.current.set(taskId, tries);
                if (tries <= MAX_REATTACH) {
                    window.setTimeout(() => attachedRef.current.delete(taskId), REATTACH_DELAY_MS * tries);
                }
            }
            // A FINISHED answer goes into the list under the same id before the
            // history comes back to replace it, so it does not blink out. A
            // partial one never does: it would read as the whole answer.
            if (terminal && full.trim()) {
                const shown = full;
                const looked = actionsRef.current;
                setMessages(prev => [...prev, {
                    id: assistantId, role: 'assistant', content: shown,
                    actions: looked.length ? looked : null,
            ...
            setStreamed('');
            setStreamingId(null);
            // The settled row carries its own stored list (`chat_messages.actions`).
            trackActions([]);
            ...
            // conversation no longer contains. Stop (aborted, panel still open)
            // settles too; a close or unmount does not.
            if (!controller.signal.aborted || openRef.current) {
                const rows = await loadHistory().catch(() => null);
                if (failed && rows) {
                    // Put the question back and raise the same inline card a
                    // direct send that failed raises — the dock chip alone told
                    // a learner reading this panel nothing about the turn they
                    // were watching. The task's meta carries the text the turn
                    // was sent with; when the LAST user row already says it, the
                    // turn was partially answered and its row survived, so
                    // nothing is spliced and the card sits under it.
                    const question = useStore.getState().aiTasks
                        .find(t => t.id === taskId)?.meta?.message ?? '';
                    const lastUser = [...rows].reverse().find(m => m.role === 'user');
                    if (question && (!lastUser || lastUser.content !== question)) {
                        const bubbleId = -Date.now() - 2;
                        splicedUserIdsRef.current.set(bubbleId, question);
                        setMessages(prev => [...prev, {
                            id: bubbleId, role: 'user', content: question,
                            created_at: new Date().toISOString(),
                        } as ChatMessage]);
                    }
                    setTurnError({ message: failed, text: question, keptIn: 'chat' });
                }
            }
        }
    }, [appendReasoning, noteContentStarted, loadHistory, trackActions]);
    ...
    //
    // NO per-run cleanup: `aiTasks` is a new array on every dock snapshot,
    // several a second while a turn thinks, so a cleanup cancelled the attach
    // before the history request returned, every time. The continuation checks
    ...
    // unanswered turn), so without this the panel simply shows the previous
    // conversation and the only trace of the turn is the dock's chip — the
    // silence this panel was called out for. Surface it exactly as a watched
    // failure is: question restored, inline card, retry hands the text back.
    // The dock keeps finished tasks briefly; that is the window this reads, so
    ...
    useEffect(() => {
        ...
        // been retried since, and its card would read as news that no longer
        // holds.
        const failed = failures.reduce((a, b) => (a.createdAt > b.createdAt ? a : b));
        attachedRef.current.add(failed.id);
        // A LATER turn asking the same question that COMPLETED is the retry of
        // this failure: the transcript already carries the question and its
        // answer, so raising the card would fault a turn that went through —
        // a reload showed one clean exchange sitting under a red card saying
        // it never arrived.
        const latestQuestion = failed.meta?.message ?? '';
        const superseded = !!latestQuestion && aiTasks.some(t =>
            t.id !== failed.id
            && t.kind === 'today_chat'
            && t.createdAt > failed.createdAt
            && t.meta?.message === latestQuestion
            && t.status === 'done');
        if (superseded) return;
        // Guarded by refs, not a per-run cleanup (see above): closed, or a new
        // Send under way, and the card is not raised.
        loadHistory()
            .catch(() => { })
            .then(rows => {
                if (!openRef.current || abortRef.current || !rows) return;
                const question = failed.meta?.message ?? '';
                // The last user row already saying it means the turn was
                // partially answered and its row survived — splice nothing,
                // the card sits under the existing question.
                const lastUser = [...rows].reverse().find(m => m.role === 'user');
                if (question && (!lastUser || lastUser.content !== question)) {
                    const bubbleId = -Date.now() - 2;
                    splicedUserIdsRef.current.set(bubbleId, question);
    ...
    const send = useCallback(async (text: string) => {
        ...
        // second identical question (the retry button hit this: "Hello",
        // "Hello", answer). Resolved from the ref, not inside the updater, so a
        // StrictMode double-invocation cannot undo the removal.
        const retiring = [...splicedUserIdsRef.current]
            .filter(([, spliced]) => spliced === message).map(([id]) => id);
        if (retiring.length) {
            retiring.forEach(id => splicedUserIdsRef.current.delete(id));
            setMessages(prev => prev.filter(m => !retiring.includes(m.id)));
        }
        ...
        try {
            ...
            const dropped = !terminal && !controller.signal.aborted && taskId != null;            if (dropped && taskId) {
                ...
                // Read NOW, not inside the updater: React runs that later, after
                // the `trackActions([])` below has emptied the ref, and the
                // finished answer then arrived with no lookup rows at all until
                // something re-read the history.
                const looked = actionsRef.current;
                setMessages(prev => [...prev, {
    ...
    // `streaming` is the flag the separate streamed bubble used to pass to the
    // marker splitters (mid-stream a marker may be half-arrived), while the
    ...
    // The in-flight turn is spliced into the list under its reserved id, so
    // when the stream ends the same slot/key hosts the final message and React
        ...
        >
            ...
            <header className="flex h-14 items-center justify-between gap-2 px-4 border-b border-slate-200 dark:border-slate-700 shrink-0 select-none">
                    <div className="flex items-center gap-2 min-w-0">
                        {/* 20px in a 28px chip (was 16 in the same chip): the
                            detailed cut's grid needs ~20px to read as lines. */}
                        <span className="p-1 rounded-lg bg-accent/10 shrink-0">
                            <BrandMark grid={iconDetailed} className="w-5 h-5 text-accent-fg" />
                        </span>
            ...
            >
                ...
                {displayMessages.map(m => {
                    // The spliced in-flight turn, or a settled row — the flag is
                    // false for every message loaded from history.
                    const isStreaming = streaming && m.id === streamingId;
                    // One body pipeline for a streamed turn and its settled row:
                    // `isStreaming` is the flag the separate bubble used to pass.
                    const body = renderBody(m.content, isStreaming);
                    ...
                    const prepared = writes.cards.length + writes.captures.length > 0;
                            ...
                            {m.role === 'user' ? (
                                ...
                                <>
                                    {/* What the model worked through before it
                                        answered. Not a character count: a count
                                        says a long turn is alive and nothing
                                        more, and it is the one thing you cannot
                                        read. */}
                                    <ReasoningPanel
                                        text={reasoningByMsg[m.id] || ''}
                                        open={!!reasoningOpen[m.id]}
                                        live={isStreaming && !m.content}
                                        follow={isStreaming}
                                        onToggle={() => toggleReasoning(m.id)}
                                    />
                                    {/* What this turn looked up, UNDER the
                                        reasoning and above the answer, because
                                        that is where it happened: the model reads
                                        the question, works out that it cannot
                                        answer from memory, and goes and looks.
                                        Drawn above the reasoning it read as
                                        something the app did before the model was
                                        asked — which is what it WAS, until the
                                        model got the tools itself. */}
                                    <AiActions actions={m.actions} />
                                    {body.trim() ? (
                                        <Markdown
                                            content={body}
                                        ...
                                        // Mid-stream the turn has said nothing YET
                                        // — no absence to report there.
                                        <p className="text-sm italic text-slate-500 dark:text-slate-400">
                                            {reasoningByMsg[m.id]
                                                ? tr("The model reasoned for {{chars}} characters and gave no answer.", { count: reasoningByMsg[m.id].length, chars: num(reasoningByMsg[m.id].length) })
                                                : tr("The model gave no answer.")}
                                        </p>
                                    )}
                                    {/* Everything below lands at settle, inside the
                                        surviving element — the streamed turn never
                                        showed these rows while it ran, and showing
                                        them mid-stream would parse half-arrived
                                        markers. */}
                                    {!isStreaming && (
                                        <>
                                            ...
                                            <SettingChangeChips
                                                ...
                                                // The TURN, not the row: the provisional
                                                // id and the stored one name the same
                                                // turn, or a history reload counts it as
                                                // a second one and its Undo stops working.
                                                messageKey={String(dbIdRef.current.get(m.id) ?? m.id)}
                                            ...
                                            />
                                            {/* What it PREPARED for the learner to press:
                                                a mastery check, cards, a note for the
                                                Inbox (utils/assistantWrites.ts). Nothing
                                                here happens until they press it. */}
                                            <CheckButtons checks={splitChecks(m.content).checks} labels={labels} />
                                            <CardProposals cards={writes.cards} labels={labels} turnKey={turnKey} />
                                            <CaptureProposals
                                                captures={writes.captures}
                                                turnKey={turnKey}
                                                onOpen={(projectId, nodeId) => { openProjectNode(projectId, nodeId); if (!docked) onClose(); }}
                                            />
                ...
                {/* A turn with NOTHING in it yet — no reasoning, no answer — has
                    nothing to reconcile, so the spinner stands alone here until
                    the first frame splices it into the list above. From there
                    the reasoning panel, the answer and its visuals all settle in
                    place when the turn ends, instead of being torn down and
                    redrawn. Or a model that does not report its thinking, where
                    the spinner IS the whole of what can honestly be said. A web
                    search happens BEFORE the model starts and takes seconds, so
                    it says so rather than spending them under a generic
                    "Thinking…". */}
                {streaming && streamingId != null && !streamed && !reasoningByMsg[streamingId] && (
                    <>
