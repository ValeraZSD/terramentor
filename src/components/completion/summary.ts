...
export function completionStory(data: ProjectCompletion, text: SummaryText): string {
    ...
    if (span) {
        if (span.calendarDays <= 1) {
            ...
            parts.push(t('{{days}} days from {{first}} to {{last}} — and you studied on every one of them.', {
                days: num(span.calendarDays), first: date(span.firstDay), last: date(span.lastDay),
            }));
        } else {
            parts.push(t('{{days}} days from {{first}} to {{last}}, and you studied on {{studyDays}} of them.', {
                days: num(span.calendarDays), first: date(span.firstDay), last: date(span.lastDay),
    ...
    if (schedule) {
        ...
        if (schedule.daysEarly > 0) {
            parts.push(t('You finished {{days}} days before the date you set.', { days: num(schedule.daysEarly) }));
        } else if (schedule.daysEarly < 0) {
            parts.push(t('That is {{days}} days past the date you set.', { days: num(-schedule.daysEarly) }));
        } else {
...
export function completionCaption(data: ProjectCompletion, text: SummaryText): string {
    ...
    if (span.bestDay.count > 0 && span.studyDays > 1) {
        ...
        parts.push(t('Busiest day {{date}} — {{done}} things done', { date: shortDate(span.bestDay.date), done: busiest }));
    }
    if (span.longestStreak > 1) {
        parts.push(t('{{days}} days in a row at the longest', { days: num(span.longestStreak) }));
    }
    return parts.join(' · ');
}
