...
import { KIND_LABEL } from '../utils/taskPlace';
import { useTranslation } from 'react-i18next';
import i18n, { k } from '../i18n';
import { useAccentVars } from '../hooks/useAccentVars';

/**
 * A CHIP IS COLOURED BY ITS OWN PROJECT, never by the page it is floating over.
 *
 * The dock is `position: fixed` at the bottom of the screen, so it inherits
 * `--accent-rgb` from wherever it happens to sit in the tree — and that is not
 * the same place on every route. On a project's dashboard it lands inside the
 * subtree `Layout` scopes to the open project, so every chip took that
 * project's colour whoever it belonged to; on the library feed it lands outside
 * it and took the app's. The same "Preparing feed lessons" chip was therefore
 * blue on one screen and orange on the next, which is the opposite of what the
 * dot is for: the colour is the only thing on a chip that says WHICH course the
 * work is for, and it was tracking the reader instead of the work.
 *
 * So each chip scopes the accent to its own task. A task with no project (a
 * library-wide sweep) falls back to the app accent read from the STORE rather
 * than to `var(--accent-rgb)` — the variable is the ambient one, which is the
 * bug, and the store's value is the same answer on every screen.
 */
function useTaskAccent(projectColor: string | null | undefined) {
    const appAccent = useStore(s => s.accentColor);
    const color = projectColor || appAccent;
    const vars = useAccentVars(color);
    return {
        vars: vars as React.CSSProperties,
        // The DOT keeps the colour as the project wears it — it carries no text,
        // so it has no reason to be darkened for a label that is not there, and
        // the undarkened swatch is the one a reader matches against the card.
        dot: color,
    };
}
 ...
 * shows every background AI generation (tutor turns, mastery checks, flashcards,
...
function progressText(task: AITaskSummary): string {
    ...
    if (p.content > 0) return i18n.t("{{chars}} chars", { count: p.content, chars: formatChars(p.content) });
...
/** The collapsed pill's one dot: the running task's project, by the same rule
 *  the chips follow. Its own component because the pill is drawn after an early
 *  return and a hook cannot be. */
function RunningDot({ projectColor }: { projectColor?: string | null }) {
    const { dot } = useTaskAccent(projectColor);
    return (
        <span
...
const TaskChip = memo(function TaskChip({ task, onHold, onDismiss }: TaskChipProps) {
    ...
    // The task's own record: when it started, how far it has got, how long it
    // has left, and the controls that go with those. Opened by the progress
    // badge on any chip, and by the chip itself when the task has no screen to
    // go to — which is most of the background kinds.
    const [showDetail, setShowDetail] = useState(false);
    const cancelAiTask = useStore(s => s.cancelAiTask);

    const active = task.status === 'running' || task.status === 'queued';
    const failed = task.status === 'error';
    const { vars: accentVars, dot: dotColor } = useTaskAccent(task.projectColor);
    ...
    useEffect(() => {
        onHold(task.id, hovered || showFailure || showDetail);
        return () => onHold(task.id, false);
    }, [hovered, showFailure, showDetail, task.id, onHold]);
    ...
    return (
        <div
            ...
            // Scoped here, on the chip's own root, so everything accented
            // inside it — the progress token, the focus rings — says which
            // project this work is for. See `useTaskAccent`.
            style={accentVars}
            title={tooltip}
        ...
        >
            <button
                type="button"
                // Where the work IS, whenever the task has somewhere to be:
                // the topic, the project, the feed, the assistant, the settings
                // panel that runs it. A failure has no output to look at, so
                // it opens the diagnosis instead; a task with nowhere to go
                // opens its own record rather than swallowing the press.
                onClick={() => {
                    if (failed) setShowFailure(true);
                    else if (!openAiTask(task)) setShowDetail(true);
                }}
            ...
            {/* The state, as its own tinted token: at a glance, which of these
                is still working and which is finished — and a CONTROL, because
                the number is what a reader is looking at when they wonder how
                long is left, and that answer does not fit on a chip. A sibling
                of the chip's button rather than inside it: a button inside a
                button is not a thing, and the press would belong to whichever
                one the browser decided it belonged to. */}
            <button
                type="button"
                onClick={() => setShowDetail(true)}
                aria-label={tr("Details of {{label}}: {{detail}}", { label: task.label, detail })}
                title={tr("Details")}
                className={`shrink-0 px-1.5 py-0.5 rounded text-[11px] font-medium tabular-nums transition outline-none focus-visible:ring-2 focus-visible:ring-accent hover:brightness-95 dark:hover:brightness-110 ${
            ...
            {showDetail && (
                <TaskDetailModal
                    ...
                    onDismiss={onDismiss}
                    onShowFailure={task.failure ? () => { setShowDetail(false); setShowFailure(true); } : undefined}
                />
            )}
        </div>
    );
    ...
    return x.id === y.id
        ...
        // Not drawn on the chip, but the detail dialog is mounted INSIDE it and
        // counts from these — a comparator listing only what the chip PAINTS
        // freezes the dialog's clock and its time-left at whatever they were
        // when it opened.
        && x.startedAt === y.startedAt
        && x.finishedAt === y.finishedAt
        && x.progress.etaMs === y.progress.etaMs
        && x.label === y.label
    ...
    if (collapsed) {
        return (
            ...
            <div ref={dockRef} className="fixed bottom-3 left-1/2 z-40 w-max" style={DOCK_CENTRE}>
                ...
                >
                    {running ? (
                        <RunningDot projectColor={running.projectColor} />
                    ) : (
