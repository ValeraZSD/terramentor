...
import { Bot, Send, Square, Trash2, RefreshCw, Loader2, Check, FileText, ShieldCheck, ArrowRight } from 'lucide-react';
...
export default function AIPanel({ active = true }: AIPanelProps) {
    ...
    // The TaskDock's live task list, which is how an OPEN tutor learns that a
    // turn on its topic was started on another device.
    ...
    // retry (see ChatTurnError). `keptIn` says where the question went — the
    // composer, or the transcript as its own bubble (a task that settled while
    // unwatched) — and the card says the same.
    ...
    // Whether the tutor may look something up, read for the line under the
    ...
    /** Task ids this panel has already followed or started, so none is attached twice. */
    ...
    useEffect(() => {
        ...
        if (selectedNodeId) {
            loadChatHistory().then(() => { resumeActiveTask(selectedNodeId); surfaceFailedTask(selectedNodeId); });
    ...
    // Follow a turn on THIS topic that another device started while this panel
    // was already open. `resumeActiveTask` only runs when the topic is opened, so
    // an open tutor used to show nothing of it until the learner left and came
    // back. The dock's task list is kept live by its own feed, so reading it
    // costs no polling. Only while the panel is visible and idle, each task at
    // most once (this panel's own sends and resumes are marked followed), and
    // with no per-run cleanup: `aiTasks` is a new array on every dock snapshot,
    // several a second while a turn thinks, and a cleanup there cancelled the
    // attach before history came back (the drawer's lesson, same shape).
    useEffect(() => {
        if (!active || !selectedNodeId || loading || streaming || abortRef.current) return;
        const nodeId = selectedNodeId;
        const task = aiTasks.find(t => t.kind === 'chat' && t.nodeId === nodeId
            && (t.status === 'running' || t.status === 'queued'));
        if (!task || followedRef.current.has(task.id)) return;
        followedRef.current.add(task.id);
        // History first: the server writes the question before calling the
        // model, and an answer arriving under no question reads as a reply to
        // the one before it.
        loadChatHistory().then(() => {
            if (prevNodeIdRef.current === nodeId && !abortRef.current) attachToChatTask(task, nodeId);
            else followedRef.current.delete(task.id);
        });
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [active, selectedNodeId, loading, streaming, aiTasks]);

    // A tutor turn that failed while nobody was watching this node: history has
    // settled by the time this runs (the caller loads it first), so restore the
    // question the way a watched failure is and raise the inline card. The dock
    // keeps finished tasks briefly — that is the window this reads, so a node
    // visited later shows nothing, which is right: the turn is over.
    const surfaceFailedTask = async (nodeId: number) => {
        try {
            const all = await api.getTasks();
            const failures = all.filter(t =>
                t.kind === 'chat' && t.nodeId === nodeId && t.status === 'error');
            if (!failures.length || prevNodeIdRef.current !== nodeId) return;
            // Several can be listed at once (each lives ~3 min after it
            // settles); the LATEST failure is the one still true — an earlier
            // one may have been retried since.
            const failed = failures.reduce((a, b) => (a.createdAt > b.createdAt ? a : b));
            const question = failed.meta?.message ?? '';
            // A LATER turn asking the same question that COMPLETED is the
            // retry of this failure: the transcript already carries the
            // question and its answer, and the card would fault a turn that
            // went through.
            const superseded = !!question && all.some(t =>
                t.id !== failed.id
                && t.kind === 'chat' && t.nodeId === nodeId
                && t.createdAt > failed.createdAt
                && t.meta?.message === question
                && t.status === 'done');
            ...
            setMessages(prev => {
                ...
                // partially answered and its row survived — splice nothing, the
                // card sits under the existing question.
                if (!question || lastUser?.content === question) return prev;
                splicedUserIdsRef.current.set(bubbleId, question);
                return [...prev, {
                    id: bubbleId, node_id: nodeId, role: 'user' as const,
    ...
    const attachToChatTask = async (task: AITaskSummary, nodeId: number) => {
        ...
        let failedFromFrame: string | null = null;
        try {
            for await (const evt of api.attachTask(task.id, controller.signal)) {
                ...
                // What the turn looked up, replayed on attach and live after.
                if (Array.isArray(evt.actions)) trackLookups(evt.actions as AiAction[]);
            ...
            // Only a bare throw — the socket dying mid-stream — stays a toast:
            // the task is still running, the dock keeps tracking it. A failed
            // task (the frame above) is settled below, in the panel.
            if (!controller.signal.aborted && !failedFromFrame) addToast('error', tr("AI Error"), error.message);
        } finally {
            ...
            setStreamingContent('');
            // The settled row carries its own stored list.
            trackLookups([]);
            if (!controller.signal.aborted && failedFromFrame) {
                // The server drops an unanswered turn, so settle from the DB
                // first — which takes the question off the screen — then put it
                // back and raise the same inline card a direct send that failed
                // raises. A toast alone rolled away and left a question with no
                // answer and no trace left in the conversation.
                await loadChatHistory();
                const question = task.meta?.message ?? '';
                const bubbleId = Date.now();
                splicedUserIdsRef.current.set(bubbleId, question);
                ...
                setTurnError({ message: failedFromFrame, text: question, keptIn: 'chat' });
            }
        }
    };
    ...
    const send = async (rawText: string) => {
        ...
        // A spliced bubble saying exactly what is being sent now IS that
        // send's text — retire it so the real send replaces it instead of
        // stacking a second identical question (the retry button hit this).
        // Resolved from the ref, not inside the updater, so a StrictMode
        // double-invocation cannot undo the removal.
        const retiring = [...splicedUserIdsRef.current]
            .filter(([, spliced]) => spliced === userMessage).map(([id]) => id);
        if (retiring.length) {
        ...
        try {
            ...
            for await (const chunk of api.streamChat(selectedNodeId, userMessage, useRag, controller.signal, meta => {
                ...
                followedRef.current.add(taskId);
            }, { onNote: setNote, onActions: trackLookups })) {
                setNote('');
                if (!contentStartedRef.current) {
    ...
    return (
        <div className="flex-1 min-h-0 flex flex-col">
            ...
            <div className="flex-shrink-0 flex items-center justify-between gap-2 px-3 py-2 border-b border-slate-200 dark:border-slate-700 select-none">
                {/* The badge yields the space, not the switch: in a detail
                    panel ~380px wide the toggle wrapped onto two lines
                    ("Use / docs") while the model name held its width. A model
                    name is information; the switch is the control. */}
            ...
            >
                ...
                {(streaming && streamingMsgId != null && (streamingContent || reasoningByMsg[streamingMsgId])
                    ...
                    return msg.role === 'user' ? (
                        ...
                        <div key={msg.id} className="min-w-0">
                            ...
                            {/* What this turn looked up, UNDER the reasoning and
                                above the answer, because that is where it
                                happened: the model works out mid-thought that it
                                cannot answer from memory and goes and looks. Drawn
                                above the reasoning it read as something the app
                                did before the model was asked — which is what it
                                WAS, until the model got the tools itself. */}
                            <AiActions actions={msg.actions} className="mb-2" />
                            {body && (
                            ...
                            {!isStreaming && actions.length > 0 && (
                                <div className="mt-3 flex flex-wrap gap-2">
                                    {actions.includes('mastery_check') && selectedNodeId != null && (
                                        <button
                                            onClick={async () => {
                                                // The tutor's button used to open an ENFORCED check
                                                // whatever the setting said, hiding "Mark done anyway".
                                                let advisory = true;
                                                try { advisory = (await api.getSettings()).mastery_gate_mode !== 'enforced'; } catch { /* the setting's default is advisory */ }
                                                openMasteryGate(selectedNodeId, selectedNode?.title ?? '', advisory);
                                            }}
                ...
                {turnError && (
                    <ChatTurnError
                        ...
                        /* A surfaced failure whose question text did not survive in
                            the task's meta has nothing to retry — an empty send would
                            silently do nothing. */
                        retryDisabled={loading || streaming || !turnError.text}
            ...
            >
                ...
                <AiDisclosure web={webEnabled} className="mt-2" />
            </div>
        </div>
    );
}