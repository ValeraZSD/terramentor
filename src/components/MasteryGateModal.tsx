...
import { api } from '../api';
import { X, ShieldCheck, AlertTriangle, CheckCircle, XCircle, Brain, Check } from 'lucide-react';
import { QuizQuestion, DrawnQuestion } from '../types';
...
export default function MasteryGateModal({
    ...
    // How many questions the topic's bank holds, when it holds more than the
    // check asks — so the learner can see this is a draw, not the whole bank.
    const [bankSize, setBankSize] = useState(0);
    // Bumped by "Retry": the draw is redone server-side, never re-asked from
    // the answers the review screen just showed.
    ...
    // The topic was already completed when the check opened: this is a
    // RETAKE. The score is evidence like any other, but the topic stays
    // completed either way, so nothing here may skip it or finish it again.
    const [retake, setRetake] = useState(false);
    // The server refused: not a topic (a section, a note, a deck's stage).
    const [notTopic, setNotTopic] = useState(false);
    const offerMarkAnyway = advisory && !!onMarkAnyway && !retake && !notTopic;
    const offerSkip = !!onSkip && !retake && !notTopic;
    ...
    useEffect(() => {
        ...
        const key = `${isOpen}:${nodeId}:${drawSeq}`;
        ...
        (async () => {
            try {
                // The server draws the check from every quiz row the node owns:
                // never-asked first, then least recently asked, ghosts (other
                // topics' review questions) never. Empty means no bank yet.
                const drawn = await api.drawMasteryCheck(nodeId);
                if (cancelled) return;

                let allQuestions: DrawnQuestion[] = drawn.questions;
                setBankSize(drawn.bankSize);
                setRetake(drawn.status === 'completed');
                ...
                if (allQuestions.length === 0) {
                    ...
                    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
                        ...
                        try {
                            ...
                            // character count. The mastery check never includes ghost
                            // questions (it must stay a pure per-node assessment).
                            // Runs as a background task (masteryCheck labels it in the
                            ...
                            if (generated && generated.questions) {
                                allQuestions = generated.questions.map((q: QuizQuestion, index: number) => ({ ...q, index }));
                                // The generation saved a quiz row; draw from it so
                                // the questions carry their place in the bank.
                                try {
                                    const again = await api.drawMasteryCheck(nodeId);
                                    if (again.questions.length) { allQuestions = again.questions; setBankSize(again.bankSize); }
                                } catch { /* the generated list stands */ }
                            }
                            genErr = null;
                            break;
                        } catch (err: any) {
                ...
                // A drawn list is already the check's size and order; only a
                // generated list that could not be re-drawn is cut here.
                const drawnList = allQuestions.some(q => q.quizId != null)
                    ? allQuestions
                    : [...allQuestions].sort(() => Math.random() - 0.5).slice(0, 10);
                setQuestions(drawnList);
                setLoading(false);
                setGenerating(false);
            } catch (err: any) {
                if (!cancelled) {
                    if (err?.data?.code === 'not_a_topic') setNotTopic(true);
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            // Which bank entries this check asked, so the next draw asks the rest.
            const asked = questions
                .filter(q => q.quizId != null)
                .map((q, i) => ({ quizId: q.quizId, index: q.index, correct: !!results[i]?.correct }));
            const result = await api.submitMasteryCheck(projectId, nodeId, correct, total, questions, asked);
            ...
            // The mastery check already recorded raw evidence, so eligibility holds.
        } catch (err: any) {
            setError(err.message || 'Failed to submit mastery check');
        } finally {
            setLoading(false);
        }
    ...
    // z-[55]: above the assistant's phone overlay (z-50, later in the tree),
    // which offers this check and would otherwise cover it; below the toasts.
    return (
        <div className="fixed inset-0 z-[55] flex items-center justify-center p-3 sm:p-4 bg-black/60 dark:bg-black/80">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            {questionPhase && bankSize > questions.length && (
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                    {t("{{n}} of the topic's {{bank}} questions, the ones you have not been asked first", { count: bankSize, n: questions.length, bank: num(bankSize) })}
                                </p>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col items-center justify-center py-12">
                            ...
                            {charCount === 0 && thinkingChars > 0 && (
                                <span className="mt-2 text-xs font-mono tabular-nums text-accent-fg">
                                    {t("{{thinkingChars}} characters of reasoning so far", { count: thinkingChars, thinkingChars: num(thinkingChars) })}
                                </span>
                            )}
                            {charCount > 0 && !verifying && (
                                <span className="mt-2 text-xs font-mono tabular-nums text-accent-fg">
                                    {t("{{charCount}} characters generated", { count: charCount, charCount: num(charCount) })}
                                </span>
                            )}
                            {verifying && (
                                <>
                                ...
                                return (
                                    <div>
                                        <div className="text-slate-800 dark:text-slate-100 font-medium mb-4">
                                            <QuestionStem content={q.question} nodeId={nodeId} surface="mastery-check" media={q.media} mediaSize="review" />
                                        </div>
                    ...
                    {submitted && result && (
                        <div className="py-6">
                                ...
                                {result.passed ? (
                                    <>
                                        ...
                                        <p className="text-sm text-emerald-500 dark:text-emerald-400 mt-1">
                                            {retake
                                                ? t("You proved it again — the topic stays completed.")
                                    ...
                                    <>
                                        ...
                                        {retake && (
                                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                                                {t("The topic stays completed. This score goes into its mastery estimate.")}
                                            </p>
                                        )}
                                    </>
                                )}
                            ...
                            {/* Even on a pass, a few answers may be wrong — surface them
                                ...
                                the mastery check having closed the gaps, not just cleared it. */}
                                                    ...
                                                    <div key={i} className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700">
                                                        <div className="flex items-start gap-3">
                                                            ...
                                                            <div className="flex-1 min-w-0">
                                                                <div className="font-medium text-slate-700 dark:text-slate-200"><QuestionStem content={q.question} nodeId={nodeId} surface="mastery-check-review" media={q.media} /></div>
                            ...
                            <div className="mt-8 flex gap-3 justify-center flex-wrap">
                                {result.passed ? (
                                    <button
                                        // A retake has nothing to finish: the topic is already completed.
                                        onClick={retake ? onClose : onPassed}
                                    ...
                                    <>
                                        ...
                                        <button
                                            // A fresh draw, never the same ten with their
                                            // answer keys just shown on the review screen.
                                            onClick={() => setDrawSeq(s => s + 1)}
                                            className="px-4 py-2 text-sm rounded-lg bg-accent text-white hover:bg-accent/90 transition-colors"
                                        >
                                            {t("Retry the mastery check")}
                                        </button>
                                        {offerMarkAnyway && (
                                            <button
                                                onClick={onMarkAnyway}
                        ...
                        <div className="flex items-center justify-between gap-3">
                            ...
                            {currentQ < questions.length - 1 ? (
                                ...
                                >
                                    ...
                                    {t("Submit mastery check")}
                                </button>
                            )}
                        </div>

                        {(offerSkip || offerMarkAnyway) && (
