...
// Why a recovery attempt produced nothing, in words the user can act on.
const RECOVERY_FAIL_REASONS: Record<string, string> = {
    'no-vision-or-ocr': 'No vision model was available and OCR produced nothing — connect a vision model in Settings → AI & Models and re-run.',
    'no-gain': "Re-reading the rendered pages didn't beat the existing text layer.",
};
    ...
    const handleFiles = async (fileList: FileList | null) => {
        ...
        if (files.length > BULK_CONFIRM_THRESHOLD) {
            const confirmed = await showConfirm({
                title: tr("Add {{length}} files?", { length: files.length }),
                message: tr("Add {{length}} documents to this project's vault. Each is text-extracted and indexed for AI context.", { length: files.length }),
                confirmLabel: tr("Add {{length}} files", { length: files.length }),
                variant: 'info',
            });
            if (!confirmed) return;
        }
