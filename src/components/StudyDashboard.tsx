    ...
    return (
        ...
        <div className="flex-1 overflow-auto bg-slate-100 dark:bg-slate-900">
            <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
                ...
                <section className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                    ...
                    {heroTask ? (
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                            ...
                            <button
                                onClick={() => selectNode(heroTask.id)}
                                className="flex items-center gap-2 px-5 py-2.5 bg-accent text-white rounded-xl transition-all font-medium shadow-sm shrink-0 hover:brightness-90 active:brightness-75"
                            >
                        ...
                        {(dailyPlan.overdueTasks.length > 0 || dailyPlan.todayTasks.length > 0) ? (
                            <div className="space-y-4 mb-4">
                                ...
                                {dailyPlan.overdueTasks.length > 0 && (
                                    <div>
                                        <div className="flex items-center gap-1.5 mb-2 text-xs font-semibold uppercase tracking-wider text-red-500 dark:text-red-400">
                        ...
                        <div className="flex flex-wrap gap-2 text-xs">
                            ...
                            {dailyPlan.summary.dueFlashcardCount > 0 && (
                                ...
                                >
                                    ...
                                    {tr("{{dueFlashcardCount}} cards due", { dueFlashcardCount: dailyPlan.summary.dueFlashcardCount })}
                                </button>
                            )}
                ...
                {/* The per-project "Daily Briefing" used to live here: a 180s
                    thinking call that re-narrated the numbers already on this
                    page. The assistant answers the same question better (it
                    knows the deadline, the days left, the vault, and what is on
                    screen) and costs nothing until asked — so this is a door to
                    it, not a second AI surface. The question is pre-filled, not
                    sent: the learner still decides. */}
                ...
                <section className="grid gap-3 grid-cols-[repeat(auto-fit,minmax(16rem,1fr))]">
                    ...
                    >
                        ...
                        <span className="min-w-0 flex-1">
                            ...
                            <span className="block text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                {tr("Boss fights and flashcards for many topics at once")}
                            </span>
                        </span>
                        <ArrowRight className="w-4 h-4 text-slate-400 flex-shrink-0 group-hover:text-accent-fg transition" aria-hidden="true" />
                    </button>
                    {/* Pass two of the chat-model authoring route. It used to be
                        reachable only from the grid's ⋮ menu, which is the wrong
                        place: the learner decides a topic is thin while looking
                        at the project, not while looking at a list of them. */}
                    <button
                                        ...
                                        >
                                            ...
                                            {milestone.segments && milestone.segments.length > 0 ? (
                                                ...
                                                <div
                                                    ...
                                                    {/* An empty track and a 0% fill used to be the same
                                                        invisible colour, so a phase with no leaves drew
                                                        nothing at all. The track now carries the shape and
                                                        only real progress is inked. */}
                                                    <div
