...
function buildSamples(t: TFunction, accent: string): KindSpec[] {
    ...
    // The flock is drawn in the accent too — it was a literal blue, the one
    // sample on the card that did not follow the learner's colour. The sketch
    // interpolates toward grey when switched off, so it needs the channels.
    const [ar, ag, ab] = hexToRgb(accent) ?? [37, 99, 235];
    return [
        {
            id: 'mermaid',
            ...
            //
            // THREE boxes, not four. A left-to-right chain is shrunk to the
            // card's width, and four boxes (with "Ask") drew their labels at
            // 10.7px in a 376px card — under the 12px floor every other visual
            // is held to (measured 2026-09-24, and 7.6px on the website).
            sample: `flowchart LR\n  A[${q(t("Observe"))}] --> B[${q(t("Experiment"))}]\n  B --> C[${q(t("Conclude"))}]`,
        },
        {
            id: 'vega-lite',
        ...
        },
        {
            ...
            //
            // A ball going round and round a static picture is what a plain
            // looping build draws, and switching the kind on rewinds that — a
            // restart, which is not an arrival: the one kind whose subject is
            // TIME would be the one whose card says nothing about what turning
            // it on gets you. The build is `fill="freeze"` and does not repeat,
            // so the rewind the switch does (`setCurrentTime(0)`,
            // sampleReveal.ts) replays it in full while the idle card never
            // dismantles itself — a loop that cleared the whole scene would
            // leave the card half-erased for a quarter of a second in every
            // cycle.
            //
            ...
            //
            // The flight is the APP'S ACCENT. A model's scene keeps the hue it
            // chose — that is the whole of `palette.ts`'s second rule, because
            // the red vector is not the blue one — but this scene is the app's
            // own, describing itself in a grid where every other card draws in
            // the accent, and a red ball among them reads as a warning rather
            // than as the one thing that moves.
...
// SWITCHED OFF, THE FLOCK ALSO SLOWS DOWN. Grey alone says "not available";
// half speed says it as well, in the one channel a card full of stills cannot
// use, and it is what makes switching the kind back on read as the thing
// waking up rather than as a colour change. Eased, never set: a step in speed
// is a teleport in miniature — the same fault that made the nudge move the
// birds instead of pushing them. About a second either way (0.045 a frame is
// ~95% in 58 frames at 60fps), deliberately slower than the 300ms colour, so
// what the eye catches on switching on is the acceleration and not the fade.
const MUTED_SPEED = 0.5, SPEED_EASE = 0.045;
let speedMix = 1;
function setup() {
...
function draw() {
  ...
  fill(lerp(${ar}, 140, mutedMix), lerp(${ag}, 140, mutedMix), lerp(${ab}, 140, mutedMix));
  speedMix += ((muted ? MUTED_SPEED : 1) - speedMix) * SPEED_EASE;
  const speed = SPEED * speedMix;
...
function Sample({ kind, code, muted, reveal }: { kind: Kind; code: string; muted: boolean; reveal: number }) {
    ...
    const theme = useStore(s => themeKeyOf(s.theme, s.themeTint));
...
function WidgetMock({ muted, play, reveal }: { muted: boolean; play: boolean; reveal: number }) {
    ...
    useEffect(() => {
        const paint = (angle: number, hand: { x: number; y: number; on: number; press: number } | null) => {
            ...
            // WHOLE PIXELS, AND NO CSS TRANSITION. Both halves are the same
            // bug: two hairline spikes standing above the bars, left over from
            // an angle the slider had passed through several seconds earlier
            // (photographed at 25°, with the bars flat and the 68° edges still
            // drawn). A percentage height lands on a subpixel, so each bar is
            // painted with a half-covered column down each side; a `height`
            // TRANSITION hands the change to the compositor, whose dirty
            // rectangle does not always cover that column — so when the bar
            // shrinks the antialiased edge of the taller bar is simply never
            // repainted. The rAF here already writes a height every frame, so
            // the transition was 100ms of lag chasing its own target as well.
            const heights = barHeights(angle);
            const box = barBoxRef.current?.clientHeight || 0;
            barsRef.current.forEach((bar, i) => {
                if (bar) bar.style.height = box ? `${Math.round(heights[i] * box / 100)}px` : `${heights[i]}%`;
            });
    ...
    return (
        ...
        >
            <div ref={barBoxRef} className="flex items-end gap-1.5 h-16 px-1">
    ...
    return (
        <>
            <h2 className={`${GROUP_CAPTION} flex items-center gap-2`}>
                <Sparkles className="w-4 h-4 text-accent-fg" /> {tr("Visuals")}
            </h2>
                            ...
                            <span className="flex items-start gap-3 px-4 py-3 border-t border-slate-100 dark:border-slate-700/70 transition-colors can-hover:group-hover:bg-accent/5">
                                ...
                                <span className="min-w-0">
                                    <span className={`block text-sm font-medium ${enabled ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                                        {name.title}
                                        {spec.heavy && (
                                            <span className="ml-2 align-middle text-xs font-medium px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
            ...
            <div className="mt-3 bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm">
                ...
                <div id="visual-any-model-why" className="pl-[1.875rem]">
                    <SettingNote>
