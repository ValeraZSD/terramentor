...
export const useStore = create<AppState>((set, get) => ({
    ...
    setTheme: async (theme) => {
        set({ theme });
        applyThemeToDom(theme, get().themeTint);
        // Accent text brightness depends on the light/dark axis, so re-apply it.
        applyAccentVars(get().accentColor, theme, get().themeTint);
    ...
    setThemeTint: async (tint) => {
        ...
        applyAccentVars(get().accentColor, get().theme, hex);
        await api.setSetting('theme_tint', hex);
    },
    ...
    setAccentColor: async (hex) => {
        set({ accentColor: hex });
        applyAccentVars(hex, get().theme, get().themeTint);
    ...
    setAtlasSettings: async (patch) => {
        const next = normalizeAtlasSettings({ ...get().atlasSettings, ...patch });
        set({ atlasSettings: next });
        try { await api.setSetting(ATLAS_SETTINGS_KEY, JSON.stringify(next)); } catch { }
    },

    resetAtlasSettings: async () => {
        set({ atlasSettings: { ...DEFAULT_ATLAS_SETTINGS } });
        try { await api.setSetting(ATLAS_SETTINGS_KEY, JSON.stringify(DEFAULT_ATLAS_SETTINGS)); } catch { }
    },
    ...
    loadSettings: async () => {
        try {
            ...
            set({ theme, themeTint, sidebarWidth, detailPanelWidth, accentColor, appIcon, uiScale, weekStartDay, atlasColorMode, atlasSurface, atlasSettings, numberFormat, uiLanguage, aiProvider, aiModel, aiWebSearch });
    ...
    loadFeed: async (scope = null) => {
        ...
        try {
            ...
            if (scopeKey(get().feedScope) !== key) return;
            set({
                feedCards: res.items,
    ...
    openAiTask: (task) => {
        ...
        switch (place.go) {
            ...
            case 'creation':
                if (!nav) return false;
                nav(place.route!);
                // Reopen the creation screen. After a reload the store starts
                // cold — aiCreationActive is only set once ProjectsGrid mounts
                // and its restore check resolves, which is AFTER this click —
                // so the signal goes out unconditionally and the grid opens the
                // modal once the restore makes it real.
