...
export interface Node {
    ...
    cards?: number;
    seen?: number;
    /** 1 when the topic has something to TEACH (an overview of reading length,
     *  a note child, or a question bank) — such a topic weighs one unit of its
     *  course however many cards it carries. Mirrors `topicWeight` in
     *  server/progress.js. */
    taught?: number;
    position: number;
...
export interface CalendarTask {
    ...
    status: Node['status'];
    color: string;
    /** The task's OWN window. A chip is drawn once per period, so the day it
     *  sits on is not necessarily the day the topic starts — the dates are how
     *  a multi-day topic still says where it runs. */
    start: string;
    end: string;
}
...
export interface AiAction {
    /** Registry name: `search_web`, `find_in_library`, `project_state`. Unknown ones still render. */
    tool: string;
    /** What was looked for — the model's own words, never the learner's verbatim. */
    arg: string;
    /** What the tool actually looked at, when the argument is not it — the
     *  project's name for `project_state`, where the model usually passes an id. */
    label?: string;
    state?: 'running' | 'done';
...
export interface AITaskProgress {
    ...
    message: string | null;
    /**
     * Milliseconds left, where the JOB itself can measure it (bulk generation
     * times its own calls on this machine's model). Null everywhere else — the
     * dock then derives an estimate from `percent` and elapsed time, which is
     * all that can be done from outside, and says that it is an estimate.
     */
    etaMs?: number | null;
}
...
/**
 * `mastery_check` is what "quiz" always was: a node's saved questions ARE the pool
 * the mastery gate draws from, so generating them ahead of time is gate prep,
 * not a separate practice artifact.
 */
export type BulkKind = 'mastery_check' | 'flashcards';
...
export interface BulkCandidate {
    ...
    quizzes: number;
    /** Non-ghost questions the mastery check would actually find on this node. */
    questions: number;
...
export interface TodayActivitySummary {
    ...
    /** Assessments taken somewhere other than the feed (mastery check, paper…). */
...
export interface AtlasTopic {
    ...
    depth: number;
    /** A topic the course counts, not a section heading (absent on an older cached payload). */
    leaf?: boolean;
...
/**
 * THREE SCOPES, ONE STREAM — mirrors `composeFeed` in server/feed.js.
 * `null` is the whole library (the home feed); `project` is one course, read
 * by the same urgency rules; `node` is one topic or section, schedule
 * ignored. It lives here rather than in the store so `api.ts` can name it
 * without importing the store back.
 */
export type FeedScope = { kind: 'project' | 'node'; id: number } | null;
/** Scopes are compared by KEY, never by identity — a fresh object is a
 *  different object, and a slow load for an old scope must not land on a new one. */
export const scopeKey = (s: FeedScope): string => (s ? `${s.kind}:${s.id}` : '');
...
export interface FeedResponse {
    ...
    nodeId?: number | null;
    /** Set when the stream was scoped to one course. */
    projectId?: number | null;
...
/** A question drawn from a bank, naming where it lives so the sitting can report what it asked. */
export interface DrawnQuestion extends QuizQuestion {
    quizId?: number;
    index: number;
}

/** What a sitting reports back: which bank entries it asked, and how they went. */
...
export interface QuizAttempt {
    ...
    created_at: string;
    /**
     * The learner's own pass mark (`mastery_check_pass`), returned by the submit
     * route so the results screen praises on the same number the gate judges on.
     * Absent on a row read back out of the table — only the submission carries it.
     */
...
export interface DrillSpec {
    ...
    /** Which drill on the topic this is (`drillKeyOf`), so its last full round can be shown again after a reload. */
    key?: string;
}

/** The last FULL round of one drill, as the server recorded it. */
