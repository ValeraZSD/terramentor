...
Every serious learner eventually hits the same wall. You decide to learn something real — machine learning, organic chemistry, systems programming, constitutional law — and the tools available to you were designed for something else entirely.
...
- Cares about **actually knowing** the material, not just checking boxes
...
They don't need gamification. They need **architecture** — a structure that makes their self-directed effort actually work.
...
All data stays on your machine. SQLite, not the cloud. A local model by default — and if you point the app at a cloud model instead, that is a choice you make in Settings, with the app telling you what it sends. Your notes, your quiz answers, your study habits — these are yours. The app will never phone home, never train on your data, never require an account.
...
- **Their data is theirs.** No cloud, no accounts, no tracking. SQLite on their disk. Ollama on their GPU.
...
- **Their model is theirs.** Run Llama locally for privacy. Switch to a cloud model for heavy lifting. Swap the "brain" without changing the workspace.
